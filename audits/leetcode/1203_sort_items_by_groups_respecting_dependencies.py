...
def sortItems(n: int, m: int, group: List[int], beforeItems: List[List[int]]) -> List[int]:
    ...
    groups_order = topologicalSort(successors_group, predecessors_count_group, m)  # Topological sort of groups
    items_order = topologicalSort(successors_item, predecessors_count_item, n)  # Topological sort of items
