...
`sifr run` compiles your program and runs the resulting binary in a single step. It reuses compatible native artifacts after checking current inputs and Cargo freshness. Ordinary build/run uses the development application profile; add `--release` for release optimization. Compiler optimization is independent.
...
| `--release` | Optimize the generated application with the release profile. |
...
Compiling hello.sifr (single-file)
  ...
  Codegen      4.3ms
  Link         9.7ms
Finished development build in 16.9ms
Binary: ./hello
```
