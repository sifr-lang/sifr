...
<Warning>
  Generated-Rust output is not part of the packaged `0.1.0` GA-qualified
  surface. Its cold first-run qualification exceeded the deterministic bound
  through both `sifr emit` and the VS Code preview action. The follow-up must
  pass before stable release notes advertise this packaged capability.
</Warning>
