...
The protected stable activation publishes and verifies the exact qualified
`sifr.sifr-vscode` version `0.2.0` in the VS Code Marketplace without rebuilding
it. After activation, install that governed package by its Marketplace
identifier:
...
The extension automatically locates `sifr` on your `PATH` and launches
`sifr lsp --stdio` when you open a `.sifr` file.

To point the extension at a custom binary:

```json
{
  "sifr.lsp.path": "/path/to/sifr"
}
```
...
If your editor cannot find `sifr`, configure the full binary path explicitly
with `sifr.lsp.path` in VS Code or the command/path option shown in the other
editor examples above.
