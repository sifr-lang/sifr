...
collection-mutating method calls. Verified non-`Copy` values are cloned into
the retained closure's isolated construction scope, so the surrounding binding
remains usable after attachment and on later loop iterations. That clone
snapshots the value when the nested handler is declared; rebinding the
surrounding name before attachment does not change the retained value.
Attachment consumes the handler binding, so attaching or calling that same
nested handler again is an ownership error. This certifies only the exercised
...
provenance, and rejects assignment-target, deletion, and mutating-method
capture use that would require `FnMut`. Callback
