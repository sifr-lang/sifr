...
SHA-256 identity are validated. Sifr forces `SQLX_OFFLINE=true` and removes
`DATABASE_URL` from signature probes and generated builds; missing or stale
metadata is rejected as `SIFR-RUST-CARGO-0001` without contacting the armed
database sentinel. This is `supported-through-bridge` for the exact package
and does not claim arbitrary Axum/tower-http APIs, live SQLx database
