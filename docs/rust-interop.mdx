...
Directly declared nested retained handlers own their verified captures, and
the compiler checks sibling nested-function capture chains transitively.
Callback attachment checks remain active through imported functions, aliases,
re-exports, and imported methods; an unsafe capture is rejected with
`SIFR-RUST-CB-0001` before Cargo probing regardless of the declaration's
module.
