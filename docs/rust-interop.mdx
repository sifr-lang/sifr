...
`bridge_v2_structural_calls` is a planned, future-owned runtime deferral. Bridge
version 2 is not supported until its separate implementation milestone replaces
both planned evidence directions with passing construction, projection,
callback, and rejection evidence. That milestone replaces bridge version 1;
version 1 is removed rather than retained as a compatibility mode or fallback.
