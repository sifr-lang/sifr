...
usable after attachment and on later loop iterations. This certifies only the
exercised local protocol frames and package bridge, not general WebSocket,
Redis, or filesystem event semantics.
...
loop iterations. The compiler checks sibling nested-function capture chains
transitively and rejects callable-valued captures whose own captures lack
compiler-known provenance. Callback attachment checks remain active through
imported functions, aliases, re-exports, and imported methods; an unsafe or
unverifiable capture is rejected with `SIFR-RUST-CB-0001` before Cargo probing
regardless of the declaration's module.
