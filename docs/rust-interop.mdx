...
Cargo probing. Verified non-`Copy` values are cloned into the retained
...
surrounding captured names remain available. The compiler checks sibling
nested-function capture chains transitively and rejects callable-valued
captures whose own captures lack compiler-known provenance. Callback
