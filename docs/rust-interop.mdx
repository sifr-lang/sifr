...
contract-only. The remaining runtime-observed deferral,
`advanced_data_runtime_matrix`, is future-owned, planned, and unadvertised
until its runtime evidence passes.
...
`memmap2`, `bytemuck`, and `zerocopy`. An owned Sifr byte buffer is moved into
`bytes::Bytes` without changing its allocation, and a sliced alias remains
valid after the original Rust owner binding is dropped. The same package
...
`zerocopy` views. The compiler binds `view=` to the actual returned Rust type,
directly probes its declared `Send` and `Sync` obligations, rejects mutable
