...
requires `Fn`, not `FnMut`. Verified non-`Copy` values are cloned into the retained
closure's isolated construction scope, so the surrounding binding remains
usable after attachment and on later loop iterations. That clone snapshots the
value when the nested handler is declared; rebinding the surrounding name
before attachment does not change the retained value. Attachment consumes the
handler binding, so attaching or calling that same nested handler again is an
ownership error. This certifies only the exercised local protocol frames and
package bridge, not general WebSocket, Redis, or filesystem event semantics.
...
method calls. The compiler checks sibling nested-function capture chains
transitively, rejects callable-valued captures whose own captures lack
compiler-known provenance, and rejects capture mutation that would require
`FnMut`. Callback
