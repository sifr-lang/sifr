...
<Note>
The `PYIMP`, `PYCALL`, `PYCONV`, `PYRES`, `PYCB`, and `PYZC` families are active compiler-emitted Python interop diagnostics. `PYZC` currently covers typed buffer declaration, ownership, layout, and hidden-copy errors; additional codes remain reserved for Arrow and DLPack declaration activation. Runtime failures at active boundaries are returned as structured `PythonError` values.
</Note>
