...
The `WORKSPACE` family covers errors in workspace configuration: malformed manifests, a source root that escapes the workspace boundary, and an invalid source root value. It also includes import resolution codes that the current `IMPORT` family replaces.
