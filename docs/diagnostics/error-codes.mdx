...
<Note>
The `PYIMP`, `PYCALL`, `PYCONV`, `PYRES`, and `PYCB` families are active compiler-emitted Python interop diagnostics. `PYZC` remains reserved for the later zero-copy declaration milestones. Runtime failures at active boundaries are returned as structured `PythonError` values.
</Note>
