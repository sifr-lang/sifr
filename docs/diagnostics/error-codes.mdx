...
<Note>
The `PYIMP`, `PYCALL`, `PYCONV`, `PYRES`, `PYCB`, and `PYZC` families are active compiler-emitted Python interop diagnostics. `PYZC` covers typed buffer and Arrow declaration, ownership, layout, transfer, and hidden-copy errors; additional codes remain reserved for DLPack declaration activation. Runtime failures at active boundaries are returned as structured `PythonError` values.
</Note>
