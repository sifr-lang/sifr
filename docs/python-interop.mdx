...
`schema=parameter(name)` instead names a required keyword-only borrowed
`python.ArrowSchema` parameter. Sifr passes that exact schema capsule to the
...
- `SIFR-PYZC`: active buffer and Arrow declaration, ownership, layout,
  certification, capsule-validation, transfer, and hidden-copy diagnostics;
  later codes remain reserved for DLPack declaration activation.
