...
generic shapes without a Sifr contract, missing annotations, and unresolved
types. It never substitutes a dynamic `Object`. `bind --check` is frozen and
read-only: it reruns resolution and generation in memory and rejects typing,
environment, distribution, source, or generated-file drift. Ordinary package
check/build also validates checked-in outputs and consumed local typing hashes
and includes binding identity in its build cache key.
