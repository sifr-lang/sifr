...
This is not Python source compatibility and not a generic `dlopen` layer. Prefer `sifr.*` for Sifr code; use `sifr.python` only when you intentionally cross into Python packages. {/* rust-interop-rejected */}
...
- DLPack capsules for NumPy and PyTorch CPU tensor paths.
