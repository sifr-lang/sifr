...
A library-only package is valid to inspect without a root Python environment.
Its required imports and target probes are reported as deferred to the final
application. A package containing one or more runnable applications must
resolve the root environment and trust policy immediately; failures match
ordinary `sifr check` diagnostics for the same snapshot.
