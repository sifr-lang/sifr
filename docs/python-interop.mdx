...
Sifr source examples for Redis, Postgres, a Kafka-compatible broker, and
LocalStack SNS/SQS interop before running the matching testcontainers-backed
Python client examples. If Docker is not running locally, the service cases
report `structured-skip` after source checks pass; with Docker running they must
report `live-passed`.
