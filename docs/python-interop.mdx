...
writable declarations must use a producer callable that returns a fresh
exporter because a shared opaque receiver cannot be exclusively frozen while
the view exists. `access=read | write` for producers and
