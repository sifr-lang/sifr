...
`root` is one relative directory. It defaults to `src`, so you can omit the
section for the standard layout. Set `root` only when the package uses another
source directory.

<Warning>
`[exports]`, `[exports].modules`, and Sifr manifest `[[bin]]` tables are not
supported. Use `src/__init__.sifr` for a library API. Use `src/main.sifr` and
`src/bin/*.sifr` for app targets.
</Warning>
...
<Warning>
Scripts do not execute shell strings or external programs. A script may only expand a single Sifr command. Nested script expansion — calling one script from another — is rejected with [`SIFR-PACKAGE-0714`](/errors/SIFR-PACKAGE-0714).
</Warning>

If an app target and a script share the same name, Sifr reports [`SIFR-PACKAGE-0605`](/errors/SIFR-PACKAGE-0605) and requires you to disambiguate with `--bin` or `--script`.
...
`bridges` lists package-local Rust bridge roots that Sifr may reference through
the `bridge` decorator root. The bridge contract is selected by the compiler;
there is no manifest version selector. Files such as
`src/bridges/tokenizer.rs` are user-owned; Sifr-managed projection files are
checked with `sifr repair --check` and regenerated with `sifr repair`.
