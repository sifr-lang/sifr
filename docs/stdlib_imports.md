...
Bare CPython stdlib module names such as `math`, `json`, `os`, `heapq`, and `collections` are not aliases for Sifr stdlib modules. Sifr uses Python syntax and follows CPython behavior where that fits the safety model, but Sifr source is not Python-source-compatible.
