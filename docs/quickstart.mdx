...
<Steps>
  ...
  <Step title="Inspect the generated Rust">
    `sifr emit` prints generated Rust without producing a binary. This command
    is documented for development builds, but generated-Rust output is not in
    the packaged `0.1.0` GA-qualified surface because its cold first-run
    qualification exceeded the deterministic bound.
