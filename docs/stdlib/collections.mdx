...
description: "Use sifr.collections for Counter, deque, defaultdict, and frozenset. Built-in collection indexing returns Option instead of crashing."
---

Sifr's `sifr.collections` module provides familiar collection types such as `Counter`, `deque`, `defaultdict`, and `frozenset`. Built-in `set[T]` is a first-class language type and needs no import. Indexing operations that could fail return `Option` instead of raising `KeyError` or `IndexError`, so every missing-value case is visible in the type system.

## Importing

```python
from sifr.collections import Counter, from_list, deque, defaultdict, frozenset
```

<Note>
Do not use bare `import collections` — that triggers [`SIFR-IMPORT-0008`](/errors/SIFR-IMPORT-0008). Always use `from sifr.collections import ...`.
</Note>
...
`set[T]` is a built-in generic mutable collection. Construct it directly and use its methods:

```python
left: set[int] = set([1, 2, 3])
right: set[int] = set([3, 4, 5])

union_set: set[int] = left.union(right)
inter_set: set[int] = left.intersection(right)
...
def collect_set_and_counter_checks() -> list[bool]:
    ...
    left: set[int] = set([1, 2, 3])
    right: set[int] = set([3, 4, 5])
    checks.append(len(left.union(right)) == 5)
    checks.append(len(left.intersection(right)) == 1)
