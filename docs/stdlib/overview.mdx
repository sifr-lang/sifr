...
| `sifr.hashlib` | Bytes-native MD5, SHA-1, SHA-2, and BLAKE2 digest objects. |
...
| Module | Description |
|---|---|
| `sifr.collections` | `Counter`, `deque`, `defaultdict`, and `frozenset`. Generic mutable `set[T]` is a built-in language type. Safe indexing returns `Option` instead of raising `KeyError` or `IndexError`. |
...
| `sifr.env` | Read process environment variables as explicit typed values. Process-wide mutation is intentionally unsupported because it is not sound in multithreaded native programs on all supported platforms. |
