...
- `sifr.env` is read-only. The former `setenv` and `unsetenv` functions are removed because process-wide environment mutation cannot satisfy Rust 2024 safety requirements in a concurrent native program. Configure the environment before starting Sifr instead.
