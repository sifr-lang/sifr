    ...
    for source_file in iter_source_files(repo_root):
        ...
        for line_number, line in enumerate(
            ...
            if "rustfmt::skip" in line:
                violations.append(
                    RustfmtSkipViolation(source_file.rel_path, line_number)
                )
    return violations
...
def assert_violation(
    repo_root: Path, expected_rel: str, expected_category: str
) -> None:
    violations = collect_violations(repo_root)
    if len(violations) != 1:
...
def assert_paths_are_included(paths: Sequence[str]) -> None:
    ...
    if missing:
        ...
        raise AssertionError(
            f"guardrail patterns do not include legacy paths:\n{formatted}"
        )
...
def run_self_test() -> None:
    ...
    excluded_oversized = (
        ...
        "verification/areas/python_interop/.venv/lib/python3.14/site-packages/vendor.py",
...
def main() -> int:
    ...
    if violations or rustfmt_skip_violations:
        ...
        for violation in rustfmt_skip_violations:
            print(
                f"- {violation.rel_path}:{violation.line}: rustfmt::skip is not allowed"
            )
        return 1
