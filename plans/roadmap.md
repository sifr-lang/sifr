...
| 39 | Rust Interop | completed, audited | [39_rust_interop.md](./phases/39_rust_interop.md), [rust_interop_architecture.md](../internal_docs/rust_interop_architecture.md) | Declaration-level Cargo integration for Rust-backed Sifr packages completed through PRs #2702-#2728 and closeout [PR #2729](https://github.com/sifr-lang/sifr/pull/2729) with final local `create-pr` validation and Opus closeout review. Verification-matrix hardening completed through PRs #3018, #3019, #3020, #3022, and #3023 with a satisfied final review and passing local closure gates recorded in its [archive](issues/archive/rust-interop-verification-matrix-hardening.md); runtime/ecosystem certification is unblocked to begin [certification_0](issues/active/rust-interop-runtime-ecosystem-certification.md). |
...
| ALG-CORPUS | Ad Hoc Algorithmic Full-Corpus Pre-Existing Failures | active, non-blocking | [ad-hoc-algorithmic-full-corpus-preexisting-failures.md](./issues/active/ad-hoc-algorithmic-full-corpus-preexisting-failures.md) | Keep the complete pinned corpus blocking in nightly while focused remediation closes the 20 preserved failures; Phase 40 release qualification retains a blocking representative subset and taxonomy self-test. |
| GENC-NAN | Ad Hoc Generated NaN Constant Clippy Quality | active, non-blocking | [adhoc_generated_nan_constant_clippy_quality.md](./issues/active/adhoc_generated_nan_constant_clippy_quality.md) | Keep the complete generated-code suite blocking in nightly while release requires the exact `clippy::zero_divided_by_zero` failure through an indexed divergence expiring 2026-10-31; restore canonical non-finite constants and remove the divergence. |
