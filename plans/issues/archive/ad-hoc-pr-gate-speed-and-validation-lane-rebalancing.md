...
2. `check_diagnostic_source_canonicalization_rules.py` is a focused diagnostic contract implemented as about 42 separate `cargo run -q -p sifr -- ...` CLI invocations. It should be an in-process Rust or Python harness over one built compiler binary or frontend API, not repeated Cargo-launched checks.
