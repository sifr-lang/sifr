...
| M7. Process Family | in progress | PR #2877 · sha=de07b23 · M7a sync process output migrated through `_sifr.process` and `sifr_stdlib::process`; PR #2879 · sha=0603eec · M7b sync child/pipe leaves migrated through `_sifr.process` and `sifr_stdlib::process`; M7c async run/output/shell migration ready for review |
...
- M7c: migrate async run/output/run-timeout/output-timeout/shell leaves through
  ...
  E2E fixtures, and migration guardrails passed; reviewer round 1 pending.
