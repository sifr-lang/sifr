...
| 4C | pending | Mutation-tail witness continuation closure | Refreshed witnesses use region-scoped continuations and current typed failure semantics across nested and straight-line mutation tails. |
...
- [ ] A witness refresh nested under another loop or branch derives control
  flow from its current structured region, never from an outer witness's stored
  `break`, `continue`, return, or fallback payload.
- [ ] Straight-line mutation tails cannot silently skip following statements or
...
- [ ] Simple and structured loop lowering share one canonical break/loop-else
  marker constructor.
- [ ] Native and codegen regressions cover outer `while ... else` with nested
...
- Active item: Item 4C, mutation-tail witness continuation closure, based on
  Item 4B merge `67c1804df84d0367e380ebef1ee14845ec1971fb`.
- Item 4B state: merged with async-for guard restoration, loop-kind refresh
  actions, canonical structured-loop break markers, precise invalidation
  diagnostics, and native ordinary/closable async plus loop-else evidence.
- Item 4C scope is limited to the first review's straight-line mutation-tail
  debt and the sole remediation review's new nested-region action leak. It also
  unifies the duplicated simple-loop break constructor and adds direct
  condition-refresh marker evidence; unrelated loop-tail E0317 and diagnostic
  UX remain Item 8-owned.
- Next action: implement Item 4C completely without testing, then run focused
  nested-loop, straight-line, codegen, and native validation, the bounded
  exact-SHA Opus review, and the single exact candidate gate sequence before
  Item 5 starts.
