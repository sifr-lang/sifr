...
focused remediation waves are merged, all 411 pinned fixtures now pass both the
canonical suite and a complete native build/run audit on pre-integration
compiler `eee55b9f94`, and the integrated-head native audit remains in
progress. Closeout restores the full corpus plus taxonomy self-test to release
qualification. The temporary release-profile divergence is removed rather
than renewed.
