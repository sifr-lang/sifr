...
| Wave 5: order-independent `defaultdict` declaration inference | implemented; review pending | function-level inference models unseeded `defaultdict(int/list/set)` aliases with inference-only unknown key/value slots, collects later subscript and `set.add`/`list.append` shapes before declaration lowering, and adopts a fully concrete hint only for one unshadowed direct declaration in the current lexical block; declaration and constructor-call HIR receive the same concrete alias type; conflicting concrete keys and set elements retain deterministic `SIFR-TYPE-0014` (`TYPE_CONTAINER_ELEMENT_CONFLICT`) diagnostics; focused lowering is 6/6, focused codegen is 2/2, full lowering is 914 passed with one ignored, full codegen is 941/941, affected Clippy/rustfmt/maintainability/file-size checks pass, the capability e2e passes, `0036_valid_sudoku` checks/builds/runs without a fixture change, and the complete native e2e suite passes 679/679 |
