...
| DX14-F5 | Reuse effectiveness boundaries | Positive edited-body reuse currently covers flat legacy source graphs reached from a manifestless CLI invocation; ordinary package graphs retain only exact-context reuse. Editor restoration also requires the same process cwd as the saved CLI semantic context. Keep these limits explicit; any expansion must retain package, trust, toolchain and resolver authority. |
...
| DX14-F10 | Future interface-proof expansion | Document the transitivity obligation of chained restored-success records before widening the body-erasure class. Each current hop must continue to prove semantic stability and check changed code successfully. |
