...
| M1. Sysroot Identity and Resolver Skeleton | implemented, review satisfied, pending PR | `crates/sifr_sysroot` adds manifest parsing, layout validation, resolver precedence, digest canonicalization, development source-tree sysroot resolution, and CLI `sifr --print sysroot` support. |
...
- M1 sysroot identity/resolver: review satisfied, PR pending.
