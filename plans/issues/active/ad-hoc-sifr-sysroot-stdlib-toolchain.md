...
| M7b. Tooling Sysroot Diagnostics and Synthetic Origins | in progress | Implementation branch adds proactive LSP sysroot diagnostics, structured `sifr/sysroot` mismatch reporting with observed paths, development CLI/LSP root/toolchain comparison coverage, and generated source-map metadata for real production Rust output tagged as `GeneratedSupport` and `CompilerSynthetic`. Focused validation has passed once; Opus review pass 1 blockers are being addressed. |
