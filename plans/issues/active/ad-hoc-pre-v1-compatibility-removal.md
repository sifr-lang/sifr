...
| SQL template-string M1 base-integration validation | The core-language inventory lacks a row for the pre-existing `method_receiver_conventions_and_source_ranges` HIR snapshot. The snapshot existed at M1 base `f428c0d2c62a7f7914ebc6424e32ad4ba79fbf36`; M1 does not own receiver semantics. | receiver semantics and `compiler/core-language` | Add the active snapshot to `lowering_layer_inventory.json` with its permanent owner, normalizers, and profile assignment. Do not add it to SQL M1. |
