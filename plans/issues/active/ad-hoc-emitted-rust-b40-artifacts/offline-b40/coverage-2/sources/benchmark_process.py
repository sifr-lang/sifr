...
LIFECYCLE = None  # Owned B39 diagnostic copy; original process-group policy retained.
...
def run_owned_process(
    ...
    with LIFECYCLE.section('long-lived-debugger-spawn'):
        process = subprocess.Popen(command, cwd=cwd, text=True, errors="replace",
            stdout=subprocess.PIPE, stderr=subprocess.PIPE, start_new_session=True)
        LIFECYCLE.inventory.pids.add(process.pid)
        LIFECYCLE.inventory.groups.add(process.pid)
        LIFECYCLE.record(kind='debugger', state='spawned', pid=process.pid, argv=list(command))
    timed_out = False
    try:
        ...
        if group_exists(process.pid):
            finish_group(process)
    with LIFECYCLE.section('long-lived-debugger-reaped', retirement=True):
        LIFECYCLE.record(kind='debugger', state='reaped', pid=process.pid, exit=process.returncode)
