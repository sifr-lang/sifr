...
LIFECYCLE = None  # B39 owned diagnostic copy; set before admission, never shared.
...
def capture_host_snapshot(
    *, include_calibration: bool = True, control_mode: str = "latency",
) -> dict[str, Any]:
    if LIFECYCLE is None:
        raise HostControlError("diagnostic lifecycle not configured")
    with LIFECYCLE.section('host-snapshot'):
        begin = time.monotonic()
        result = _capture_host_snapshot(include_calibration=include_calibration,
                                        control_mode=control_mode)
        result['producer_begin_monotonic'] = begin
        result['producer_end_monotonic'] = time.monotonic()
        return result
...
class HostActivityMonitor:
    ...
    def __exit__(self, _exc_type: object, _exc: object, _traceback: object) -> None:
        ...
        if self._thread is not None and self._thread.is_alive():
            self.error = 'host monitor did not join'
        if self.error is None and LIFECYCLE.failure is None:
            try:
                self.snapshots.append(capture_host_snapshot(
                    include_calibration=False, control_mode=self._control_mode))
    ...
    def rejection_reasons(self) -> list[str]:
        reasons: set[str] = set()
        if self.error or LIFECYCLE.failure:
            reasons.add('producer lifecycle failed: ' + str(self.error or LIFECYCLE.failure))
...
def command_output(argv: list[str]) -> str:
    try:
        completed = LIFECYCLE.run(argv, text=True, capture_output=True, timeout=5)
