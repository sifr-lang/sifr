...
def observer_tests(suite):
    ...
    for mode in ('handshake-only','terminal-only','late-handshake','wrong-run','partial-json','no-target-terminal'):
        def launch(mode=mode):
            ...
            path=d/('observer-result.json' if mode in ('terminal-only','no-target-terminal') else 'inferior.json')
            record={'pid':123,'run_id':run_id(0),'flags':134,'pgid':123,'state':'INCONCLUSIVE'}
            if mode=='terminal-only':record.pop('pgid')
            if mode=='no-target-terminal':record={'run_id':run_id(0),'target_launches':0,'state':'INCONCLUSIVE'}
            if mode=='wrong-run':record['run_id']='foreign'
            if mode=='partial-json':path.write_text('{')
            else:save(path,record)
            inv.collect_launch_records(out,run_id)
            if mode=='no-target-terminal':
                require(not inv.errors and inv.pids=={99} and not inv.groups,'prelaunch failure invented target identity')
            elif mode in ('wrong-run','partial-json'):
