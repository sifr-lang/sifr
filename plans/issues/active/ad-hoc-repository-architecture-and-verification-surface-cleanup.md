...
| PR 9 Project Workspace Area Migration | In progress | This branch |
...
| `project_workspace` | Deleted project row from `verification/suites/manifest.json`; moved `crates/sifr/tests/verification/project/` fixtures | `verification/areas/project_workspace/` | `uv run --project verification --locked python -m sifr_verify areas run --area project_workspace --suite baselines`; merge/nightly/release hardening dispatches project baselines through this area | Area schema validation, project area baseline execution, legacy hardening dispatch equivalence | In progress in PR 9; project fixture source cut over, validation contract matrices still pending area migration; Opus review passed after shared-adapter hardening |
