...
| `developer_tooling` | Deleted `verification/tooling/`; moved the large LSP corpus submodule and wrapper under `verification/areas/developer_tooling/corpora/`; routed tooling, TypeScript-Go, and diagnostic tooling contracts through the developer tooling area | `verification/areas/developer_tooling/` | `uv run --project verification --locked python -m sifr_verify areas run --area developer_tooling --suite typescript-go-m1`; `--suite diagnostic-contracts`; profile facade dispatches `static`, `formatter`, `analysis`, `lsp-smoke`, `editor-release`, `lsp-stress`, `phase-closeout`, and `full` through this area | Area schema validation and developer tooling suite equivalence pending this PR | In progress in PR 15 |
