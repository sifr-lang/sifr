...
| `generated_code_quality` | Deleted `verification/generated_code_quality/` and shell wrappers | `verification/areas/generated_code_quality/` | `uv run --project verification --locked python -m sifr_verify areas run --area generated_code_quality --suite smoke`; `--suite representative`; `--suite full`; generated-code profile dispatch in `scripts/run_all_tests.sh` uses the area runner | Area schema validation and generated-code quality area smoke/representative equivalence pending this PR | In progress in PR 13 |
