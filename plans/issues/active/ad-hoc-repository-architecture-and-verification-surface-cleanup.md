...
| `ecosystem_compatibility` | Moved OSS manifests, local projects, and gate policy | `verification/areas/ecosystem_compatibility/` plus `verification/policy/ecosystem_compatibility.md` | `uv run --project verification --locked python -m sifr_verify areas run --area ecosystem_compatibility --suite oss-curated`; `--suite ecosystem-broader` | In progress in implementation branch | Implementation in progress |
