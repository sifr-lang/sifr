...
| --- | --- | --- |
| DXF2-F1 | Suggestion / storage documentation | The `Store::prune` comment still implies all unused results survive deletion of predecessors. Eviction intentionally retains only bounded recent history. Correct this stale source comment when DXF.3 touches the storage owner; architecture documentation already states the actual contract. |
