...
One housekeeping note: both `reviews/ad-hoc-repository-architecture-and-verification-vocabulary-agent-review-pass-{1,2}.md` contain only a model-error line ("There's an issue with the selected model (agent-high)…") — they're dead artifacts and should be deleted or replaced with this pass rather than archived.
