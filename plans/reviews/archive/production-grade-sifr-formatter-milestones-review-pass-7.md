...
| M3: sifr_format core | route through Ruff, `PyFormatOptions` conversion, stable Sifr diagnostics | Ruff-backed core, Sifr wrapper tests | `cargo test -p sifr_format`, `check_formatter_rules.py` dual pass | one formatter core, no fallback whitespace path |
...
- **Guardrails**: `check_formatter_ast_coverage.py` design is fully specified (6 requirements). `check_formatter_rules.py` dual-pass is already in the codebase and is referenced for M3 validation.
