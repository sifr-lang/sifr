...
- **External-lock verification**: `artifact_self_update_receipt_rules.sh:82-101` pre-creates the lock, runs the installer with `SIFR_INSTALL_LOCK_HELD=1`, then asserts the caller-owned lock survives and the binary still installs.
