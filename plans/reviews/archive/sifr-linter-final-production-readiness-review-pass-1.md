...
| **R4** | Parser-aware suppressions for non-line rules | ✅ SATISFIED | M3 implements `sifr_lint::suppression::ParserAwareSuppressions` as the required compile-time dependency for non-`physical-line` rule modules. `suppression_gate.json` transitions from `physical_line_only` to `parser_aware` on the M3 milestone marker. `check_linter_reuse_rules.py` asserts `ParserAwareSuppressions` dependency for statement-range, single-node, and symbol-workspace suppression complexity rules. M5 policy rules use parser-aware suppression; M5 post-fix pass confirmed the split-brain fix and rule/suppression contract. |
...
| `check_linter_reuse_rules.py` wired and documented | Present in `internal_docs/tooling_verification.md` § Linter Hardening Checks and `internal_docs/tooling_analysis.md` § Milestone 1–M7. Consistent. |
