...
- `crates/sifr/tests/e2e_support/harness_behavior_tests.rs:522` — `test_generate_cargo_toml_required_tokio_uses_runtime_features` pins the same features string, so the harness contract test continues to detect drift between helper and assertion.
