...
- `crates/sifr/tests/e2e_support/harness_contract_tests.rs:526` — `test_generate_cargo_toml_runtime_diagnostics_use_locked_tracing_spec` independently pins both code paths (module-driven and required-crate-driven) to the exact locked spec. This is the right level: it would catch a future drift on either side of the OR.
