# Review Adjudication: GPT Pro async-model review

Date: 2026-05-09
Reviewer: Claude (adjudication pass)
Source: reviews/async-model-review-gpt-pro.md
...
GPT Pro's review is structurally sound on the blocking issues and overstated on several of its rejections. The model has real semantic contradictions that must be fixed before implementation, but the overall direction is right.
...
**What GPT Pro says:** The opening example shows:
```sifr
a: str = await first
b: str = await second
```
...
**What GPT Pro says:** The proposal uses `async with task.scope() as scope` in milestones 2 and 3, but milestone 7 is where `async with` is implemented. `TaskScope` is an async context manager, but the syntax to use it doesn't exist until milestone 7.
...
**What GPT Pro says:** `channel.receive()` returns `Result[Option[T], ClosedError]`. `None` means closed-and-drained. `ClosedError` also means closed. Two different ways to say "closed" on receive.
...
**What GPT Pro says:** `sync.Lock` uses a synchronous Rust mutex. If contended, it blocks the OS thread, not just the async task. The proposal's fix (lock guards not crossing `await`) is necessary but not sufficient.
...
GPT Pro says to define `Coroutine[T, E]`, `Task[T, E]`, `TaskResult[T, E]` as separate types. The current model conflates them.
...
GPT Pro says: `try await task` — what happens when cancellation occurs? Can it propagate?
...
GPT Pro says these compositors lack exact result type signatures.
...
GPT Pro says the orphaned handle rule and the `__aexit__` safety backstop contradict each other. If handles must be consumed, the backstop is unnecessary. If the scope owns children, why require consumption?
...
GPT Pro says: "a handle moved into a collection is tracked only when the compiler can prove the collection is drained, consumed, or dropped before task-scope exit" is too complex for milestone 3.
...
GPT Pro says `SecondaryError` is mentioned repeatedly but its type is unspecified.
...
GPT Pro says: timeout wrapping arbitrary enclosed operations requires internal spawning, which imposes Send/ownership rules. The proposal acts like all forms are easy.
...
GPT Pro says: what if `T` contains interior mutability?
...
GPT Pro says: if blocking work continues after scope exit, it cannot borrow from the scope.
...
GPT Pro says: handle creation order is less intuitive than input order.
...
GPT Pro says: "cancellation waits for cleanup" can hang forever if cleanup loops.
...
GPT Pro says the mapping table (`Event -> Notify`, `Queue -> Channel`, etc.) overpromises.
...
## Rejections (GPT Pro is wrong or premature)

### Rejection 1: `E | CancellationError` requires a union type system

GPT Pro says: does Sifr actually have union types? If not, this notation is misleading.
...
GPT Pro says: Event is level-triggered, Notify is edge-triggered. They're not the same.
...
GPT Pro says: v1 should use owned-only captures for `spawn`.
...
The GPT Pro review identifies four real blockers:
...
Three of GPT Pro's items are rejected: the union type notation is already valid Sifr syntax; the Event -> Notify mapping is documented as an approximation with the divergence already addressable via refinement 12; and scoped borrows are already handled conservatively through Send/Sync checking.
