...
**Not ready for PR** solely because the create-pr validation gate hasn't been run (H1). Code-wise the removal is clean and internally consistent: the synthetic `sifr.http_transport` bootstrap seeder, the `HTTP_TRANSPORT_HARNESS_RUST` blob, the lowering escape hatch, the frontend override API, the e2e directive scan, the Cargo/dependency planning entries, and the three `.sifr` fixtures + create-PR manifest entries are all gone; SIFR-IMPORT-0009 rejection routes cleanly through the existing `unsupported_legacy_stdlib_module` path; and the new Rust fixture exercises HTTP/1 TCP, HTTP/2 h2c, HTTPS/H2+ALPN loopback with equivalent request/response assertions. Docs and traceability updated consistently. After running the create-pr profile, this can ship; carry M1/M2/M3 as follow-ups (best fit in M0e alongside architecture doc updates and source-origin privacy).
