...
7. **Untracked artifacts** in `plans/reviews/active/`: zero-byte `*.agent.log` files for passes 1–8/10/11 and a zero-byte pass-11 `.md`. Not part of the diff, so not a PR defect, but the recurring pass-1 §7 / pass-3 §7 / pass-6 §4 note stands.
