...
- `plans/reviews/active/m10-i18n-interop-agent-review-pass1.md` and `pass2.md` are 0-byte placeholders (only the corresponding `.stderr`/`.log` sidecars carry content). If those files are meant to hold review artifacts, they're empty; if they're just markers, ignore.
