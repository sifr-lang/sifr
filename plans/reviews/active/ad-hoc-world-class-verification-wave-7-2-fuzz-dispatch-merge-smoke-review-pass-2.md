...
- `crates/sifr_driver/src/bin/diagnostic_contract_harness.rs:119-187` now parses `--target <id> --seed <path>`; the per-target arm dispatches to `check_parser_seed_runtime_contract` (for `diagnostic_renderer_entrypoint`) or `check_project_seed_runtime_contract` (for `package_project_manifest_entrypoint`).
