...
1. **Tracker row** — M10 now reads `merged` with dual-PR evidence: `PR #2892 · sha=4a5b16c` (M10a signal native-boundary migration, agent round 6) and `PR #2894 · sha=330e277` (certification split, agent round 1). Both shas match the recent commit log (`4a5b16cf3` migrate, `330e27765` split). Ordering with M11/M12 `planned` is correct.
