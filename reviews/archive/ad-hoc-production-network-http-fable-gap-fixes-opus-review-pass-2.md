...
- `grep -n "Error::"` over the file returns only `:184` (the Fable amendment itself, which intentionally names the rejected variants).
...
- The `Error Taxonomy` table at `:182-195` keeps the 8-class flat list and carries the dated Fable amendment. No drift between amendment, taxonomy, and individual rule cells now.
...
1. Sweep the one residual `ProtocolError::UnsupportedExtensionFrame` in `network_http_substrate_inventory.json:113` — same wording fix as the `.md` cell at `:51`. Not in scope for this branch, not enumerated by Fable, but the only place an unhelpful reader can still derive a contradicted variant name in the inventory pair.
...
**PASS.** All six Fable blockers and the three pass-1 cleanup items are satisfied; tests pass; remaining residual is pre-existing on main and not a re-FAIL.
