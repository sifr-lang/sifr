# Review (Fable Pass 1): Production Network and HTTP Platform Substrate — TLS Full-Duplex Disposition
...
- **TCP split/half-close.** Unchanged since Opus pass 2; still internally consistent (`split()` infallible, unsplit `shutdown_write()` non-consuming, typed write-after-shutdown, evidence from socket state).
...
- `TlsReadHalf.close()` / `TcpReadHalf.close()` disposition while the sibling half is live remains "according to the final affine-handle rules" — a known M0 day-one decision (carried from Opus pass 2).
