...
- [x] `milestone_diag_9` slice 8 implementation complete and reviewer-satisfied: attached primary ranges to match diagnostics (`SIFR-MATCH-0001` through `SIFR-MATCH-0003`), split match-statement lowering out of `statements.rs` to satisfy HIR maintainability guardrails, and added HIR primary-range assertions plus e2e column anchors for match fixtures. PR: pending.
