...
- [x] `milestone_diag_11` expression scalar/conversion builtin raw HIR diagnostic migration slice implementation complete and reviewer-satisfied: migrated `abs`/`hash`/`round`/`repr`/`int`/`bigint`/`float`/`bool` raw diagnostics to structured call/type diagnostics with primary ranges. PR: pending.
