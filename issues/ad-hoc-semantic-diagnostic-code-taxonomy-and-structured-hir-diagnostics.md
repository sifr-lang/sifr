...
- [x] `milestone_diag_11` expression iterator-builtin raw HIR diagnostic migration slice implementation complete and reviewer-satisfied: migrated `reversed`/`enumerate` raw arity, keyword, duplicate-argument, iterable/type-shape, start-type, and reversible-bound diagnostics to structured call/type/protocol diagnostics with primary ranges, and moved the lowering into `expression_iter_builtins.rs`. PR: pending.
