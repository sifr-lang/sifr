...
- [x] `milestone_diag_11` expression min/max raw HIR diagnostic migration slice implementation complete and reviewer-satisfied: migrated `min`/`max` raw keyword, missing-argument, and single-argument non-iterable diagnostics to structured call/type diagnostics with primary ranges. PR: pending.
