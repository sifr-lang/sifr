...
- [x] `milestone_diag_11` expression-call raw HIR diagnostic migration slice implementation complete and reviewer-satisfied: migrated non-simple call target and `iter`/`next`/`pow` raw diagnostics to structured call/type diagnostics with primary ranges. PR: pending.
