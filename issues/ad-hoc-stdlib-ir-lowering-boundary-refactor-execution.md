...
- M1 implementation review: `reviews/ad-hoc-stdlib-boundary-m1-review-1.md` -> `READY`; reviewer confirmed the `sifr_stdlib` contract crate owns intrinsic signatures and embedded source inventory, driver still owns bootstrap compilation, no `sifr_lowering` stdlib shim remains, and dependency direction is clean.
...
- M1: local validation passed.
  ...
  - `cargo test -p sifr_lowering name_import_diagnostics_tests` -> PASS.
...
- M3: local validation passed.
  ...
  - `cargo tree -p sifr_codegen --depth 2 --edges normal` -> PASS; normal dependency tree contains `sifr_ir` and no `sifr_lowering`, parser, syntax, frontend, or lowering edge through IR.
  - `cargo tree -p sifr_lint --depth 1 --edges normal` -> PASS; direct dependency tree contains `sifr_ir` and no direct `sifr_lowering` lowering dependency.
  - `rg "sifr_lowering" crates/sifr_codegen/src --glob '!**/*tests.rs' --glob '!**/tests.rs' --glob '!**/lib_codegen_tests/**'` -> PASS; no production codegen imports from lowering crate.
  - `rg "sifr_lowering" crates/sifr_lint/src -g '*.rs'` -> PASS; no lint source imports from lowering crate.
  - `cargo test -p sifr_lint` -> PASS.
  - `cargo test -p sifr_lowering cfg` -> PASS.
  - `cargo test -p sifr_lowering flow_graph` -> PASS.
...
- M4: local validation passed.
  - `test ! -d crates/sifr_hir` -> PASS; old `crates/sifr_hir` directory removed.
  - `rg "sifr_hir|crates/sifr_hir" Cargo.toml Cargo.lock crates scripts verification docs internal_docs issues/ad-hoc-stdlib-ir-lowering-boundary-refactor.md issues/ad-hoc-stdlib-ir-lowering-boundary-refactor-execution.md -g '*'` -> PASS; only intentional migration-plan references remain in the active phase contract.
  - `cargo metadata --no-deps --format-version 1 | jq -r '.packages[].name' | rg '^sifr_(hir|lowering|ir|stdlib)$'` -> PASS; workspace contains `sifr_stdlib`, `sifr_ir`, and `sifr_lowering`, with no `sifr_hir` crate.
  - `cargo tree -p sifr_codegen --depth 2 --edges normal` -> PASS; normal dependency tree contains `sifr_ir` and no lowering/parser dependency path through IR.
  - `cargo tree -p sifr_lint --depth 1 --edges normal` -> PASS; direct dependency tree contains no direct lowering dependency.
  - `cargo check -p sifr_lowering` -> PASS.
  - `cargo check --workspace` -> PASS.
  - `cargo test -p sifr_lowering` -> PASS (`535 passed; 0 failed; 1 ignored`).
  - `cargo test -p sifr_frontend` -> PASS (`43 passed`).
  - `cargo check -p sifr_codegen --tests` -> PASS.
  - `cargo test -p sifr_lint` -> PASS.
  - `python3 scripts/check_hir_maintainability_guardrails.py` -> PASS.
  - `python3 scripts/check_file_size_guardrails.py` -> PASS.
  - `cargo fmt --check` -> PASS.
