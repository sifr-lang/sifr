...
- [ ] INT-1 runtime `SifrInt` and ownership semantics
  ...
  - [ ] Continue the broader `Type::Int` codegen migration beyond direct helper/local expression rewrites: lexical shadowing and legacy-emission paths need scope-safe exact-int coverage, unsupported augmented assignment/fallible `//` and `%` still need exact-int runtime/codegen support, and function argument/return boundaries still need uniform `SifrInt` lowering instead of legacy `i64`.
