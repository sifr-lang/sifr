...
- [ ] INT-1 runtime `SifrInt` and ownership semantics
  ...
  - [x] `SifrInt`-shaped `//=` and `%=` augmented assignments with syntactically non-zero integer literal right-hand sides now rewrite to plain assignments through compiler-proven non-zero floor division/modulo runtime helpers, preserving promoted exact-int locals without Rust `/=` or `%=` fallback: PR #1856.
  - [ ] Continue the broader `Type::Int` migration beyond direct helper/local expression rewrites: HIR `Result[int, DivisionError]` / `SIFR-INT-0005` proven-nonzero integration still needs support for unproven exact-int division and modulo.
