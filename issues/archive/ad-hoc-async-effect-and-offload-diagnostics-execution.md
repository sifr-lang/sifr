...
- 2026-05-12 fake async rejection ([#2098](https://github.com/sifr-lang/sifr/pull/2098)): `SIFR-ASYNC-0001` rejects async functions without a suspension source and `SIFR-ASYNC-0002` rejects awaiting same-task coroutines with transitive no-suspend summaries. Claude reviewer approval is recorded in `reviews/adhoc_reject_fake_async_review.md`.
- 2026-05-12 workload annotation enforcement ([#2099](https://github.com/sifr-lang/sifr/pull/2099)): `@blocking_io` and `@cpu_heavy` are sync-only annotations; direct annotated workload calls in async contexts now fail while cheap sync helpers remain allowed. Claude reviewer approval is recorded in `reviews/adhoc_enforce_workload_annotations_review.md`.
- 2026-05-12 blocking offload target restriction ([#2100](https://github.com/sifr-lang/sifr/pull/2100)): `task.spawn_blocking()` and `ThreadPoolExecutor.submit()` now require classified `@blocking_io`, `@cpu_heavy`, or known stdlib offload targets and preserve existing arity/sendability/result diagnostics. Claude reviewer approval is recorded in `reviews/adhoc_restrict_blocking_offload_targets_review.md`.
...
- 2026-05-12 final phase closure review: Claude reviewer approved closure in `reviews/adhoc_async_effect_and_offload_diagnostics_final_review.md`.
