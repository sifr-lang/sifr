...
- Rust ecosystem-first expansion:
  - Result: accepted; locked production crates are Tokio, Tokio Util, Futures Util, Crossbeam Channel, Rayon, targeted Rustix, tracing, metrics, thiserror, Serde, and Postcard, all hidden behind Sifr APIs with exact version/feature plans in the phase doc. Tokio remains `current_thread`; blocking I/O parallelism uses Tokio's blocking pool and CPU parallelism uses Rayon. Flume, async-channel, futures-channel, direct Parking Lot, new Once Cell, Scopeguard, production tracing-subscriber, IPC Serde JSON, Bincode, Signal Hook, Nix, direct Mio/Bytes/DashMap, Anyhow, Eyre, and bespoke replacements are not used in this phase.
...
| Rust ecosystem choices | `internal-only` | `internal-only` | Use only the locked Rust Ecosystem Decisions table from the phase doc: Tokio with `current_thread`, Tokio Util, Futures Util, Crossbeam Channel, Tokio `sync` wrappers including MPSC/semaphore/notify, std sync/OnceLock, Rayon, Tokio process/std process, targeted Rustix, Tokio signal, tracing, metrics, thiserror, Serde, and Postcard with exact feature plans. Reject Flume, async-channel, futures-channel, direct Parking Lot, new Once Cell, Scopeguard, production tracing-subscriber, IPC Serde JSON, Bincode, Signal Hook, Nix, direct Mio/Bytes/DashMap, Anyhow, Eyre, and bespoke replacements. | N/A | Phase-doc decision table plus M0 ledger verification |
