...
- Current cancellation started mostly abort-based through Tokio task handles and timeout aborts. The closed v1 decision keeps abort-backed task-handle cancellation, accepts compiler-recognized `task.timeout(...)` as the same-task cooperative scope, and rejects any public cancellation-token layer; see the cancellation contract closure decision below.
...
- Ring 2 generated-runtime core: `tokio`, conditional `tokio-util`, conditional `futures-util`, and `tracing`, each feature-gated by the Sifr runtime capability that requires it.
...
| Ring 2 | cooperative cancellation helper and Tokio I/O utilities | `tokio-util` | add `tokio-util = 0.7.18` only if implementation proves it is needed, with `default-features = false` and features `rt`, `io-util`, and `time`; in tokio-util 0.7.18 `rt` exposes `tokio_util::sync::CancellationToken`, and there is no separate `sync` feature; do not enable `full`, `net`, `codec`, `compat`, or `join-map` | M1, M4 | Conditional generated-runtime helper for internal cancellation or process pipe helpers only if needed. The closed v1 model did not add a public `CancelScope`, Tokio/tokio-util token, or unconditional `tokio-util` cancellation dependency. |
...
- Add or close the production `sifr.task` surface:
  ...
  - `timeout`
  - `deadline`
  - compiler-recognized `async with task.timeout(duration)` same-task timeout scope
  - `join_all`
  - `race`/`select`
...
- `timeout` wraps one awaited operation and returns the operation's normal typed outcome plus typed timeout evidence. Inner errors are preserved; timeout is a distinct error/evidence variant. `deadline` uses an absolute monotonic deadline. The planned broader cancellation-scope surface closed as compiler-recognized `async with task.timeout(duration)` same-task timeout scopes; no separate public `cancel_scope` API, `CancelScope` type, Tokio token, or tokio-util token is exposed. See the cancellation contract closure decision above.
...
- Add or close the production `sifr.runtime` and `sifr.parallel` offload surface:
  - `spawn_blocking`
  - `task.spawn_cpu(fn: Callable[[], T]) -> BlockingTask[T, WorkerRuntimeError]`
  - `task.spawn_cpu(fn: Callable[[], Result[T, E]]) -> BlockingTask[T, WorkerError]`
  ...
  - `JoinSet.join_all().await -> list[TaskResult[T, E]]`, consuming the set and returning results in submission order
...
- Require sendable owned items for `parallel.map`/`try_map`; non-send items are compile-time diagnostics. M3 v1 accepts top-level named sync workers only: nested worker functions with non-send captures are compile-time ownership diagnostics, and nested worker functions with otherwise sendable captures are rejected until Sifr carries capture ABI metadata through worker function values.
...
- Map task, worker, foreign/runtime boundary, and panic-like runtime failures into typed evidence. In M3's first implementation waves, `WorkerError` is the public non-generic worker wrapper used by `parallel.try_map`, `Pool.try_map`, and fallible `task.spawn_cpu`; infallible `task.spawn_cpu` uses `WorkerRuntimeError`. `JoinSet`/scoped owner offload keep the stricter `WorkerError[E]` target if the type system can carry the user error parameter without erasing it.
- M3 JoinSet implementation note: `task.JoinSet[T, E]` currently accepts `add(Task[T, E] | BlockingTask[T, E])`, `spawn_blocking(Callable[[], Result[T, E]])`, and `spawn_cpu(Callable[[], Result[T, UserE]])` for `JoinSet[T, WorkerError]`. `join_all().await` returns `list[TaskResult[T, E]]` to preserve existing task failure/cancellation evidence, and `cancel_all().await` returns `list[CancelOutcome]`. Live JoinSet bindings cannot be rebound or dropped after accepting work; they must be consumed by awaiting `join_all()` or `cancel_all()` directly, or by binding that terminal awaitable and awaiting the bound value before scope exit. Base `JoinSet` usage emits only Tokio-backed task/`spawn_blocking` support; Rayon is enabled only when `JoinSet.spawn_cpu` or top-level `task.spawn_cpu` is used. The stricter `list[Result[T, WorkerError[E]]]` target remains coupled to the generic `WorkerError[E]` follow-up.
...
| Stable task APIs | Stable production APIs are `TaskHandle`, `TaskGroup[E]`, `spawn_scoped`, `sleep`, `timeout`, `deadline`, compiler-recognized `async with task.timeout(duration)` same-task timeout scopes, `join_all`, `race`, and `select`. `TaskHandle[T, E]` is the public affine observation handle. `TaskGroup[E]` is error-homogeneous and result-heterogeneous through individual handles. `race`/`select` result containers are M0 public API boundary artifacts. No separate public `cancel_scope` API or `CancelScope` type exists. Raw runtimes, event loops, callback transports, detached tasks, and Tokio handles are not public. |
| Scoped process spawn return type | M0 chose a distinct `ProcessHandle` for `TaskGroup.spawn_process`; it owns child status observation plus stdin/stdout/stderr pipe access while binding child lifetime to the parent scope. `TaskHandle[Status, SubprocessError]` would lose first-class pipe ownership, and returning `Child` alone would blur supervised lifetime with ordinary sync child observation. |
| TaskGroup offload error binding | M0 is the binding gate for `TaskGroup[E].spawn_blocking` and `TaskGroup[E].spawn_cpu`: M0 must choose `Result[T, E]` with runtime offload errors representable in `E`, `TaskGroup[WorkerError[E]]`, or an explicit mapper closure, then update this row before M3's first implementation PR. This decision must also be reconciled with the M3 `JoinSet.join_all().await` surface. Current M3 returns `list[TaskResult[T, E]]`; generic `WorkerError[E]` remains a recorded follow-up instead of an implemented promise. |
...
| Diagnostics and warnings | Runtime diagnostics use structured `tracing` spans/events plus fixed-schema `metrics` counters behind Sifr diagnostics types. Duration histograms remain gated on future accepted duration-bearing runtime event schemas. No Python global `warnings` filter adapter ships in this phase. |
...
| `task.spawn_cpu` handle API | Module-level `task.spawn_cpu` is valid only inside async functions, requires a named zero-argument sync worker classified as `@cpu_heavy`, returns an affine `BlockingTask` handle, and uses private Rayon pools. Infallible workers produce `BlockingTask[T, WorkerRuntimeError]`; fallible workers produce `BlockingTask[T, WorkerError]` with user errors formatted as typed worker evidence until `WorkerError[E]` lands. |
