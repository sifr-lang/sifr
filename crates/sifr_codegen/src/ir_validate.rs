...
fn validate_stmt(stmt: &RustStmt, issues: &mut Vec<IrValidationIssue>, in_function: bool) {
    match stmt {
        ...
        RustStmt::RawCode(code) => {
            validate_raw_code_braces(code, "statement RawCode", issues);
        }
    }
}
...
fn validate_expr(expr: &RustExpr, issues: &mut Vec<IrValidationIssue>, in_function: bool) {
    match expr {
        ...
        RustExpr::RawCode(code) => {
            validate_raw_code_braces(code, "expression RawCode", issues);
        }
    }
}
...
fn validate_type(ty: &RustType, issues: &mut Vec<IrValidationIssue>) {
    match ty {
        ...
        RustType::RawCode(code) => {
            validate_raw_code_braces(code, "type RawCode", issues);
        }
    }
}

fn validate_raw_code_braces(code: &str, context: &str, issues: &mut Vec<IrValidationIssue>) {
    if !has_balanced_braces(code) {
        issues.push(IrValidationIssue {
            kind: IrValidationKind::UnbalancedRawCodeBraces,
            message: format!("{context} has unbalanced braces"),
        });
    }
}
...
mod tests {
    ...
    #[test]
    fn catches_unbalanced_raw_code_braces() {
        let items = vec![RustItem::RawCode("fn broken() {".to_string())];
        let issues = validate_items(&items);
        assert!(issues
            .iter()
            .any(|issue| issue.kind == IrValidationKind::UnbalancedRawCodeBraces));
    }
