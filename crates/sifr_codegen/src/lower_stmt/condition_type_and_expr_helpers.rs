...
fn string_concat_capacity_expr(parts: &[&HirExpr]) -> RustExpr {
    ...
    for part in parts {
        let len_expr = if let HirExpr::StringLiteral(value) = part {
            RustExpr::compiler_fragment(format!("{}usize", value.len()))
            ...
            RustExpr::compiler_fragment("0usize".to_string())
        };
    ...
    let mut capacity = iter
        .next()
        .unwrap_or_else(|| RustExpr::compiler_fragment("0usize".to_string()));
    for part in iter {
