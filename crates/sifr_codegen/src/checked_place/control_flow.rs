use super::{
    ...
    expr_mentions_name, CheckedPlaceFailureKind,
};
...
impl RustEmitter {
    ...
    pub(crate) fn try_lower_checked_dict_if_for_ir(
        ...
        if self.checked_read_failure_type(CheckedPlaceFailureKind::Key).is_some() || !elif_clauses.is_empty() {
            return Ok(None);
        }
    ...
    pub(crate) fn try_lower_checked_sequence_if_for_ir(
        ...
        if self.checked_read_failure_type(CheckedPlaceFailureKind::Index).is_some() || !elif_clauses.is_empty() {
            return Ok(None);
        }
    ...
    pub(crate) fn checked_sequence_loop_guards_for_ir(
        ...
        if self.checked_read_failure_type(CheckedPlaceFailureKind::Index).is_some() { return Ok(Vec::new()); }
        let mut guards = Vec::new();
    ...
    pub(crate) fn try_lower_checked_dict_exit_guard_for_ir(
        ...
        if self.checked_read_failure_type(CheckedPlaceFailureKind::Key).is_some() { return Ok(None); }
    ...
    pub(crate) fn try_lower_checked_sequence_exit_guards_for_ir(
        ...
        if self.checked_read_failure_type(CheckedPlaceFailureKind::Index).is_some() { return Ok(None); }
