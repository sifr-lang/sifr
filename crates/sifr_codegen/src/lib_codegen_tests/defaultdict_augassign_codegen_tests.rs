...
fn variable_string_key_defaultdict_counter_keeps_entry_default_codegen() {
    ...
    assert!(
        rust_code.contains("counts.entry(task).or_insert(SifrInt::from_i64(0))"),
        "{rust_code}"
    );
...
fn variable_integer_key_defaultdict_counter_keeps_entry_default_codegen() {
    ...
    assert!(
        rust_code.contains("counts.entry(item.clone()).or_insert(SifrInt::from_i64(0))"),
        "{rust_code}"
    );
    assert!(rust_code.contains("*__elem += SifrInt::from_i64(1);"));
}
...
fn nested_scalar_shadow_is_not_retyped_as_defaultdict() {
    ...
    assert!(rust_code.contains("let mut counts: HashMap<String, SifrInt> = HashMap::new();"));
    assert!(rust_code.contains("let counts: SifrInt = SifrInt::from_i64(7);"));
    assert!(!rust_code.contains("HashMap<String, SifrInt> = SifrInt::from_i64(7)"));
}
