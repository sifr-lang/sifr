    ...
    fn lower_operator_method_body(&mut self, func: &HirFunction) -> Vec<RustStmt> {
        ...
        for stmt in &func.body {
            ...
            match try_lower_simple_stmt_with_scope_result(
                ...
                Ok(None) => {
                    if let Some(lowered) = self.try_lower_operator_stmt_ir(stmt) {
                        self.lowering_stats.expr_candidate_total += 1;
                        self.lowering_stats.expr_candidate_structured += 1;
                        self.lowering_stats.stmt_structured += 1;
                        self.lowering_stats.stmt_candidate_structured += 1;
                        ...
                        panic!(
                            "structured operator method statement lowering missing for IR-first emission: {stmt:?}"
                        );
                    }
                }
                Err(_) => {
                    ...
                    panic!(
                        "structured operator method statement lowering failed for IR-first emission: {stmt:?}"
                    );
                }
            }
        }
