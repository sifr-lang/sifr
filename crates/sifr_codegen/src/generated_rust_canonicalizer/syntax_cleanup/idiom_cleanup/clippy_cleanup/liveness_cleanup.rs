...
pub(super) fn remove_proven_owned_clones(body: &mut syn::Block, owned: &HashSet<String>) {
    ...
    remove_clones_from_return_expressions(body, owned);
}
...
impl VisitMut for LastUseCloneRemover<'_> {
    ...
    fn visit_expr_mut(&mut self, expression: &mut syn::Expr) {
        ...
        let syn::Expr::Path(path) = call.receiver.as_ref() else { return; };
        let Some(root) = path.path.get_ident().map(ToString::to_string) else {
            return;
        };
