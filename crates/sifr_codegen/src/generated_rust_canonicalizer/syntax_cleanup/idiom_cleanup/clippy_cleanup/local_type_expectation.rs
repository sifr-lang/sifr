pub(super) fn add_complex_local_type_expectation(local: &mut syn::Local) {
        ...
        && type_contains_large_nested_result_tuple(&typed.ty)
    {
        let reason = syn::LitStr::new(
            "language necessity: this generated carrier preserves nested typed Sifr error and tuple structure; owner emitted-Rust quality; remove when the carrier representation changes",
