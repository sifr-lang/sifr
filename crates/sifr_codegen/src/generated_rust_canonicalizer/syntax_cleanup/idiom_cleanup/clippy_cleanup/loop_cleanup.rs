...
pub(super) fn remove_unnecessary_owned_iteration(
        ...
        && into_iter.args.is_empty()
    {
                    ...
                    if mapped.method == "map"))
        {
            let reason = syn::LitStr::new(
                "language necessity: this generated Rust materializes cloned mapping entries before typed loop mutation; owner emitted-Rust quality; remove when ownership lowering proves direct iteration safe",
        ...
        && iterated.args.is_empty()
    {
        ...
        if path_receiver {
            let reason = syn::LitStr::new(
                "language necessity: generated Rust borrows this typed Sifr iteration source; owner emitted-Rust quality; remove when direct IntoIterator preserves the same source lifetime",
                proc_macro2::Span::call_site(),
            );
