...
#[cfg(test)]
pub(crate) use authority::LoweredMethod;
pub(crate) use authority::{
    is_in_place_collection_method, lower_method, lower_method_with_context,
};
...
mod tests {
    ...
    fn lowers_string_methods_via_registry() {
        ...
        let list_cloned = lower_method(&Type::List(Box::new(Type::Int)), "cloned", "xs", &[])
            .expect("list cloned lowers");
        assert_eq!(render_expr(&list_cloned.expr), "xs.clone()");
