...
pub(super) fn lower_builtin_open(args: &[String]) -> Option<RustExpr> {
    ...
    code.push_str(&str_ref(&args[0]));
    code.push_str("; let __mode = ");
    code.push_str(&str_ref(&args[1]));
...
pub(super) fn lower_file_write(args: &[String]) -> Option<RustExpr> {
    ...
    Some(RustExpr::RawCode(format!(
        "(|| -> Result<(), IOError> {{ use std::io::Write; let __hid = ({}); let __data = ({}).as_ref(); let mut __handles = __SIFR_FILE_HANDLES.lock().unwrap(); match __handles.get_mut(&__hid) {{ Some(SifrFileHandle::TextWrite(ref mut __w)) => {{ __w.write_all(__data.as_bytes()).map_err(__io_err)?; Ok(()) }}, _ => Err(IOError {{ message: \"file not open for writing\".to_string(), kind: \"Other\".to_string() }}) }} }})()",
        args[0], args[1]
    )))
}
