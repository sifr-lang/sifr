...
fn utf8_only_guard_expr(encoding_expr: RustExpr, surface: &str, success_expr: RustExpr) -> RustExpr {
    RustExpr::Block {
        stmts: vec![
            RustStmt::Let {
                mutable: false,
...
pub(super) fn lower_str_encode_utf8_result(args: &[RustExpr]) -> Option<RustExpr> {
    if args.len() != 1 {
        return None;
    }
...
pub(super) fn lower_str_encode_utf8_result_with_encoding(args: &[RustExpr]) -> Option<RustExpr> {
    if args.len() != 2 {
        return None;
    }
    let encoded = lower_encode_utf8(&[arg_expr(args, 0)])?;
...
pub(super) fn lower_decode_utf8_with_encoding(args: &[RustExpr]) -> Option<RustExpr> {
    if args.len() != 2 {
        return None;
    }
...
pub(super) fn lower_bytes_with_size(args: &[RustExpr]) -> Option<RustExpr> {
    if args.len() != 1 {
        return None;
    }
    Some(RustExpr::Block {
        stmts: vec![
            ...
            RustStmt::If {
                ...
                then_body: vec![RustStmt::Return(Some(err_value_expr(RustExpr::MethodCall {
...
pub(super) fn lower_bytes_from_ints(args: &[RustExpr]) -> Option<RustExpr> {
    if args.len() != 1 {
        return None;
    }
    Some(RustExpr::Block {
            ...
            RustStmt::For {
                ...
                body: vec![
                    ...
                    RustStmt::Expr(RustExpr::MethodCall {
                        ...
                        method: "push".to_string(),
                        args: vec![RustExpr::Deref(Box::new(RustExpr::Field {
                            expr: Box::new(RustExpr::Ident("__pair".to_string())),
                            field: "1".to_string(),
                        }))],
                    }),
