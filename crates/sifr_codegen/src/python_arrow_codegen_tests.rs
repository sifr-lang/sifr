...
#[test]
fn arrow_requested_schema_is_borrowed_and_not_forwarded_to_producer_call() {
    let rust = generate(&format!(
        "{ERROR}\n@python.arrow(pkg.make_stream, schema=parameter(requested))\ndef acquire(*, requested: python.ArrowSchema) -> Result[python.ArrowStream, PythonError]: ...\n"
    ));
...
fn requested_schema_can_precede_forwarded_keyword_parameters() {
    let rust = generate(&format!(
        "{ERROR}\n@python.arrow(pkg.make_stream, schema=parameter(requested))\ndef acquire(*, requested: python.ArrowSchema, limit: int) -> Result[python.ArrowStream, PythonError]: ...\n"
    ));
