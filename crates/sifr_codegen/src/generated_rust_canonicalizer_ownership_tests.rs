...
#[test]
fn qualified_string_calls_and_returns_preserve_unknown_abis() {
    let source = r#"
        mod known {
            pub struct Reader;
...
#[test]
fn string_callbacks_and_scalar_trait_methods_keep_declared_abis() {
    let source = r#"
...
#[test]
fn project_type_facts_follow_declared_module_imports() {
...
#[test]
fn borrow_plans_do_not_match_unrelated_qualified_callees() {
    let source = r#"
        mod local {
            pub struct Reader;
            impl Reader {
...
#[test]
fn generic_self_constructor_keeps_optional_scalar_borrow_plan() {
    let source = r#"
...
#[test]
fn final_project_slice_plan_preserves_lexical_loop_item_borrows() {
    let mut source = r#"
...
#[test]
fn tail_empty_collection_keeps_element_type_evidence() {
    let source = r#"
        fn main() {
...
#[test]
fn branch_argument_keeps_value_live_for_following_argument() {
    let source = r#"
...
#[test]
fn owned_callback_string_conversion_keeps_the_owned_abi() {
    let source = r#"
...
#[test]
fn external_runtime_namespace_does_not_imply_a_borrowed_string_abi() {
...
#[test]
fn constructor_borrows_follow_declared_reexports_only() {
    let source = r#"
        mod definitions {
            pub struct Reader;
...
#[test]
fn cleanup_keeps_effectful_initializer_lookalikes() {
    let source = r#"
...
#[test]
fn macro_struct_construction_aligns_fields_and_retains_effects() {
    let source = r#"
    ...
    let compact = canonical
        ...
        .collect::<String>();
    assert!(!compact.contains("live:"), "{canonical}");
    assert!(compact.contains("structConfig{effect:i64,}"), "{canonical}");
    assert!(
        compact.contains("Self{effect:make_number()}"),
        "{canonical}"
    );
}

#[test]
fn closed_binary_removes_stale_exact_support_function_imports() {
    let source = r#"
...
#[test]
fn residual_clippy_shapes_are_rewritten_inside_macros_and_control_flow() {
    let source = r#"
...
#[test]
fn replacement_collapse_keeps_sequential_semantics_when_replacement_matches() {
    let source = r#"
        fn main() {
...
#[test]
fn project_string_rewrites_use_local_signatures_without_cross_module_guesses() {
    let mut borrowed = r#"
        fn resolve(_: &str) {}
...
#[test]
fn boolean_cleanup_preserves_pattern_conditions_and_shadowed_types() {
    let source = r#"
        fn consume(_: &str) {}
        fn main() {
...
#[test]
fn borrowed_loop_cleanup_preserves_clone_from_shadow_contract() {
    let source = r#"
...
#[test]
fn math_capture_keeps_the_resolved_import_without_fallback_materialization() {
    let source = r#"
...
#[test]
fn api_cleanup_preserves_receiver_abi_and_module_visibility() {
    let source = r#"
        struct RootPayload;
        mod generated_union {
...
#[test]
fn string_clone_cleanup_uses_exact_binding_type_facts() {
    let source = r#"
...
#[test]
fn recursive_sifr_int_cleanup_preserves_callable_abi() {
    let source = r#"
...
#[test]
fn scalar_borrow_plans_are_module_and_owner_qualified() {
    let source = r#"
        use sifr_runtime::SifrInt;
        mod first {
            use super::SifrInt;
...
#[test]
fn scalar_borrowing_preserves_callback_abis_and_let_chain_bindings() {
    let source = r#"
        use sifr_runtime::SifrInt;
...
#[test]
fn assignment_cleanup_folds_recomputed_character_cache() {
    let source = r#"
...
#[test]
fn closed_binary_prunes_unreferenced_marker_trait_components() {
    let source = r#"
        trait Marker {}
        impl Marker for i64 {}
        fn main() { println!("ok"); }
