...
use environment::{EnvironmentSnapshot, resolve_package_python_environment};
use serde_json::Value;
use sifr_analysis::FileId;
use sifr_analysis::{DiskSourceProvider, SourceProvider};
use sifr_compiler_services::{
    PythonInteropPlan, PythonInteropPlanDiagnostic, PythonTargetInspection, PythonTargetProbeStatus,
};
...
impl Session {
    ...
    pub(crate) fn python_declaration_snapshot(
        ...
        let mut checked_external_fingerprint = None;
        if let Some(entry) = self.python_declarations.entries.get(&cache_key) {
            if entry.graph_revision == graph_revision && entry.source_revision == source_revision {
                ...
                let current_fingerprint =
                    package_root.as_deref().map_or_else(String::new, |root| {
                        package_input_fingerprint(root, &mut provider)
                    });
                if entry.external_fingerprint == current_fingerprint {
                ...
                checked_external_fingerprint = Some(current_fingerprint);
            }
        }
        self.python_declarations.stats.misses += 1;
        let external_fingerprint = checked_external_fingerprint.unwrap_or_else(|| {
            self.python_declarations.stats.external_fingerprint_runs += 1;
            package_root.as_deref().map_or_else(String::new, |root| {
                package_input_fingerprint(root, &mut provider)
            })
