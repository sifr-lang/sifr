...
Phase 37 records Sifr feature metadata and carries Cargo feature data through normalized metadata and command plans. Full source-level feature activation is reserved for the package-aware CLI integration phase, but the Phase 37 model fixes the ownership boundary.

## Ownership Boundary

| Feature surface | Owner | Phase 37 behavior |
| --- | --- | --- |
...
- If a future milestone maps Sifr feature names to Cargo package features, it must validate missing Cargo packages/features with stable package diagnostics before invoking Cargo build commands.
...
| --- | --- |
| Stable Cargo feature argument ordering | `milestone_37_4_tests::cargo_command_plans_preserve_lock_mode_and_feature_semantics` |
| Feature selectors in cache inputs | `milestone_37_4_tests::package_build_cache_digest_changes_with_lock_source_and_target_inputs` |
| Backend trust selected by Cargo graph | `milestone_37_4_tests::backend_trust_reports_untrusted_direct_backend_crate` |
| Package filter feature-mode placeholder | `milestone_37_5_tests::filters_select_dependency_and_dependent_closures_with_negation` |
