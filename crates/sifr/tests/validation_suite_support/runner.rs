...
pub(crate) fn run() -> Result<(), String> {
    ...
    println!(
        "[validation-suite] total_rows={} total_ms={}",
        total_rows,
...
fn run_suite(repo_root: &Path, suite: &Suite) -> Result<(), String> {
    for row in &suite.rows {
        ...
        println!(
            "[sifr-case-timing] bucket=validation_suite case={}/{} elapsed_ms={} status={}",
            suite.name,
            row.id,
...
fn temp_root(repo_root: &Path, suite_name: &str, row_id: &str) -> Result<PathBuf, String> {
    ...
    let root = std::env::temp_dir().join(format!(
        "sifr-validation-suites-{suite_name}-{row_id}-{}",
        std::process::id()
    ));
...
fn run_command(
    ...
    if result.exit_code != command.expected_exit {
        return Err(format!(
            "validation suite command '{}' expected exit {}, got {}\n  command: {}\n  stderr:\n{}",
            command.id,
    ...
    match assertion {
        Assertion::Contains {
            ...
            if !actual.contains(text) {
                return Err(format!(
                    "validation suite assertion failed: command '{}' {} missing {:?}\n  command: {}\n  actual {}:\n{}",
                    command_id,
                    stream_name(*stream),
                    text,
                    result.argv.join(" "),
        ...
        Assertion::EqualStreams {
            ...
            if left_text != right_text {
                return Err(format!(
                    "validation suite assertion failed: {} differed between '{}' and '{}'\n  left command: {}\n  right command: {}\n  left {}:\n{}\n  right {}:\n{}",
                    stream_name(*stream),
                    left_command_id,
                    right_command_id,
                    left.argv.join(" "),
