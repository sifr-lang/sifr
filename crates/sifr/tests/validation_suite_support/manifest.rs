...
const SUITE_FILTER_ENV: &str = "SIFR_VALIDATION_SUITE_FILTER";
const MANIFEST_PATH_ENV: &str = "SIFR_VALIDATION_SUITE_MANIFEST";
...
pub(crate) fn load(repo_root: &Path) -> Result<Vec<Suite>, String> {
    ...
    if manifest.suites.is_empty() {
        return Err("validation suite manifest contains no suites".to_string());
    }
    ...
    let suites = if let Some(filter) = suite_filter {
        ...
        if selected.len() != filter.len() {
            ...
            let missing = filter
                ...
                .join(", ");
            return Err(format!(
                "validation suite filter referenced unknown suites: {missing}"
            ));
        }
        selected
    } else {
    ...
    if suites.is_empty() {
        return Err("no validation suites selected".to_string());
    }
    Ok(suites)
}
...
fn manifest_path(_repo_root: &Path) -> Result<PathBuf, String> {
    ...
    let path = match from_env {
        ...
        None => {
            return Err(format!(
                "{MANIFEST_PATH_ENV} must point at an area-owned validation suite manifest"
            ));
        }
    };
    if !path.is_file() {
        return Err(format!(
            "validation suite manifest not found: {}",
            path.display()
        ));
    }
    Ok(path)
