//! TEMPORARY B50 diagnostic instrumentation. NEVER merge into production.
...
const N: usize = 12;
const NAMES: [&str; N] = [
    "command", "cache", "bootstrap", "inventory", "parse", "lowering",
    "lower_externals", "canonicalization", "exports", "emission", "metadata",
    "private_plan",
];

#[derive(Clone, Copy)]
pub enum Phase {
    Command, Cache, Bootstrap, Inventory, Parse, Lowering, LowerExternals,
    Canonicalization, Exports, Emission, Metadata, PrivatePlan,
}
...
pub fn run_command(run: impl FnOnce() -> i32) -> i32 {
    let output = std::env::var_os("SIFR_B50_PHASE_OUTPUT");
