...
impl RawAdapter<'_> {
    pub(crate) fn create_sequence(
        ...
        let mut statement = CreateSequenceStatement {
            ...
            if_not_exists: bool_field(body, "if_not_exists").unwrap_or(false),
    ...
    pub(crate) fn alter_sequence(
        ...
        let options = optional_array(body, "options");
        if options.len() != 1 {
            return Err(sequence_error(
                "ALTER SEQUENCE supports exactly one OWNED BY option; mixed options are unsupported",
            ));
        }
        let option = sequence_option(&options[0])?;
        if string_field(option, "defname") != Some("owned_by") {
            return Err(sequence_error("ALTER SEQUENCE supports only OWNED BY"));
        }
        let owned_by = sequence_owner(option)?;
        Ok(AlterSequenceStatement {
            name: relation_name(object_field(body, "sequence")?),
            if_exists: bool_field(body, "missing_ok").unwrap_or(false),
            owned_by,
        })
    }
}
