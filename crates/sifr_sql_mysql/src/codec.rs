use crate::types::MysqlServerSeries;
...
pub(crate) fn mysql_codec_registry_for_types(
    series: MysqlServerSeries,
...
fn contract(
    ...
    Ok(CodecContract {
        identity: CodecIdentity::new(format!("mysql.{name}.binary.v1"))?,
        database_type,
        sifr_type,
        server_profiles: BTreeSet::from([profile.to_string()]),
        encode_error: "MysqlEncodeError".to_string(),
        decode_error: "MysqlDecodeError".to_string(),
        null_behavior: NullCodecBehavior::PassThrough,
        wire_format: WireFormatIdentity::new(format!("mysql.binary.{name}.v1"))?,
