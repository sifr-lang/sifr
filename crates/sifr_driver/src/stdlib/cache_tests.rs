...
fn stdlib_cache_shared_bundle_outlives_cache_and_keeps_definitions_isolated() {
    let retained = {
        ...
        let first =
            get_or_init_stdlib_cache(&cache, compile_stdlib_uncached).expect("complete bootstrap");
        let second =
            get_or_init_stdlib_cache(&cache, || panic!("no rebuild")).expect("shared bootstrap");
