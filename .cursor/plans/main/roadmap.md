...
| 14 | Codegen Architecture | 6 (rust_ir_types → codegen_structural_passes) | completed | Structured Rust IR, pretty-printer, preamble/stmt/expr/intrinsic migration, dead-code elimination, clone optimization |
...
After the Codegen Architecture phase, the entire codegen pipeline uses a structured Rust IR instead of string templates. A purpose-built intermediate representation (`RustExpr`, `RustStmt`, `RustItem`, `RustType`) models the ~50 Rust constructs Sifr emits. A pretty-printer renders IR to semantically identical Rust source (same compilation and runtime behavior, whitespace differences allowed). The preamble (error types, `FileHandle` with its 10 methods, logging globals) is emitted via structured IR — the 500-650 character single-line string templates are gone. A `sifr_type_to_rust_type` helper maps Sifr types to structured `RustType` nodes. The `RustEmitter` god-object is decomposed: `CodegenContext` carries immutable compilation context, `ScopeContext` replaces temporal coupling flags with explicit parameters, and all new `lower_*` methods return `Result<_, CodegenError>`. The monolithic `lib.rs` (9,805 lines) is decomposed into focused modules (`lower_expr.rs`, `lower_stmt.rs`, `lower_item.rs`, `context.rs`, `preamble.rs`). All ~80 intrinsic function match arms and ~50 method call match arms produce structured IR bodies via a mandatory `HashMap`-based registry, organized into domain-specific modules (`intrinsics/io.rs`, `intrinsics/math.rs`, `methods/str_methods.rs`, etc.). The string-parsing dead-code eliminator (`filter_rust_code_to_needed`) is replaced by a structural IR pass (gated on zero `RawCode` nodes). Import collection is automatic (no more `needs_hashmap` boolean flags). Clone optimization is conservative — removing clones on literals and `Copy` types only; full ownership analysis is deferred to a future HIR pass. A clone optimization pass removes unnecessary `.clone()` calls. At least 20 of the 34 Clippy suppressions are removed. All future codegen (async, web, FFI) is built on structured IR from day one.
