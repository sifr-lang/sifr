# Phase 28: Async and Ecosystem Foundation
...
### milestone_28_1: Async Runtime Core
...
### milestone_28_2: Typed Serialization (Core)
...
**Depends on:** milestone_28_1 (async runtime must exist for async-compatible serde patterns; generics from Phase 13 enable `loads(s, T)`)
...
### Definition of Done (milestone_28_2)
...
### milestone_28_3: Async Synchronization Primitives
...
**Depends on:** milestone_28_1 (sync primitives are fundamental concurrency tools independent of networking; they depend only on the async runtime)
...
### Definition of Done (milestone_28_3)
...
### milestone_28_4: Advanced Async Features
...
**Depends on:** milestone_28_3 (sync primitives must exist for advanced patterns)
...
### Definition of Done (milestone_28_4)
...
- **milestone_28_1 first:** The async runtime must exist before anything else. This is the minimum viable async — `async def`/`await`, Tokio, basic task spawning.
- **milestone_28_2 second:** Typed serialization is web-independent and stays in this phase.
- **milestone_28_3 third:** Synchronization primitives (Lock, Channel, Semaphore) and Send/Sync checking depend only on the async runtime.
- **milestone_28_4 last:** Advanced features (async with, async generators, async comprehensions) build on everything above.
...
- Validation planning goals:
  - `milestone_28_1` (Async Runtime Core): validation goals cover `async def`/`await` lowering, Tokio auto-bundling, `sifr.task` spawn/sleep/timeout behavior, and try/except auto-unwrap across await points. Include negative-path goals that catch regressions against these guarantees.
  - `milestone_28_2` (Typed Serialization (Core)): validation goals cover auto-derive `Serialize`/`Deserialize`, typed `dumps`/`loads` behavior, and nested/union/optional collection roundtrip correctness. Include negative-path goals that catch regressions against these guarantees.
  - `milestone_28_3` (Async Synchronization Primitives): validation goals cover `sifr.sync.Lock`, `sifr.sync.Channel`, and `sifr.sync.Semaphore` semantics plus Send/Sync enforcement at spawn boundaries. Include negative-path goals that catch regressions against these guarantees.
  - `milestone_28_4` (Advanced Async Features): validation goals cover `async with` context manager flow, async generator semantics, and async comprehension compilation behavior. Include negative-path goals that catch regressions against these guarantees.
