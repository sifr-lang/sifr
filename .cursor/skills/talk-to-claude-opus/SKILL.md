...
nohup claude --dangerously-skip-permissions --setting-sources project --model opus --effort medium -p "$(cat <<PROMPT
${TOPIC_NAME}
PROMPT
...
- `--model opus` selects the latest available Claude Opus model; keep
  `--effort medium` as the default review effort.
