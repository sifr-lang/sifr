...
sifr/
  ...
  crates/
    ...
    sifr_stdlib_manifest/   (target compiler-host stdlib source inventory, private declaration inventory, feature planning, sysroot validation, and import suggestions; retained-signature and IPC responsibilities are being narrowed into dedicated boundaries)
    sifr_ipc/               (target shared IPC protocol/frame/schema/request-tracking crate extracted from manifest-hosted IPC helpers)
