...
- Schema-first SQL architecture is defined in [`sql_architecture.md`](./sql_architecture.md). It uses template strings, named schema profiles, immutable structural records, and deterministic compiler components for checked embedded SQL. Dialect packages own exact PostgreSQL, MySQL, and SQLite semantics. Query templates separate owned binding from preparation, callers select explicit fetch methods, and runtime execution requires a verified schema contract. Host-only tools execute through `sifr x`. The canonical schema remains compile authority, while a typed migration graph proves every supported path to that schema.
