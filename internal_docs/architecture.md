...
- `sifr_analysis::AnalysisSnapshot`: analysis-facing handle to a captured `WorkspaceSnapshot`. Semantic LSP request handling routes editor queries through snapshot methods while staying serialized; syntax-only formatting, folding and selection use captured text without constructing a semantic host. Snapshots pin their host identity and compiler metadata generation, and incremental updates commit overlay bytes for later reload. `internal_docs/typescript_go_architecture_transfer_analysis_snapshot.md` records query metadata snapshot ids, stale-result identity, and the conservative dirty-scope report slot.
- `sifr_lsp::Session`: persistent analysis handles that wrap `WorkspaceSession`; `DocumentStore` keeps protocol document state. An ingress generation lock orders source/configuration events against publication independently of cancellation. Explicit configuration changes re-resolve toolchains; closing the final document releases idle metadata ownership. `internal_docs/typescript_go_architecture_transfer_lsp_persistent_session.md` records overlay-fed open/change/save handling and document-version stale-result checks.
...
- Frontend project compilation: `sifr_frontend` owns deterministic dependency ordering, cyclic strongly connected component selection, and source-backed import-cycle diagnostics with related import spans. The shared `FrontendProduct` compiles one pinned parsed/source snapshot for single-file, project, package and test flows; it collects exports from the complete `LoweringResult` before retaining HIR, flow graphs, and rendered diagnostic metadata for driver code generation. `FrontendContext` applies the same ordering and cycle diagnostic to analysis queries, including source edits.
