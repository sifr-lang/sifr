...
dynamic attributes, optional positional-only parameters whose omission cannot
be represented by the call plan, or unsupported conversion types. It must never
silently replace an unsupported type with `py.Object`.
...
An environment-identity change cannot relabel retained bindings: the authoring
command re-probes every retained module before adopting the new digest. The
complete updated artifact, output uniqueness, and in-memory generated bytes are
validated before any output write. Existing user-owned files and symlinked
outputs or typing sources are rejected rather than overwritten or followed.

## Editor Declaration Authoring

The analysis host exposes the same lowered `PythonInteropPlan` used by code
generation. The language server enriches completion and hover from that plan,
uses ordinary definition navigation for the Sifr declaration, and runs the
driver's exact target and call-shape probe against the package-selected
interpreter. Editor status is therefore `verified`, `runtime-checked`, or
`deferred` under the same meanings as check/build; it is not inferred from
Python hints or completion text.

Target and authoring diagnostics are attached only after compiler checking.
If the source has an error, an unsupported or untyped declaration cannot gain
a verified status. Protocol-specific completion help reports the compiler's
