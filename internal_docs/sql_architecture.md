...
`component-model`, `cranelift`, `gc-null`, `runtime`, and `std`. The `gc-null`
feature enables the standard exception encoding used by WASI SDK `34` output.
The host enables exceptions and multiple memories because the virtualized
PostgreSQL components require them. It applies explicit instance, memory, and
table limits. The component linker is empty. The Wasmtime `threads` feature is
absent, so shared memories cannot compile. No WASI interface or other host
interface is linked.
...
| Compiler component guest ABI | [`wit-bindgen`](https://docs.rs/wit-bindgen/) | Provider guests export only the checked `analyze` world. The build uses WASI SDK for provider-owned C sources. |
| Component WASI encapsulation | [`WASI-Virt`](https://github.com/bytecodealliance/WASI-Virt) | PostgreSQL build tooling composes deny-by-default WASI implementations into the guest. The final artifact has no host imports. |
...
| PostgreSQL parser build adapter | [`cc`](https://docs.rs/cc/) | The component build compiles the selected checked-in `libpg_query` source with documented WASI-only server-unit exclusions. This is build tooling, not a Sifr standard-library API. |
...
| MySQL runtime protocol | [`mysql_async`](https://docs.rs/mysql_async/) | The provider uses the driver's exact Row/Value re-exports, raw connections, and Sifr-owned pooling. |
...
The following baseline was verified on 2026-08-30, with the current CC build
adapter and Rustls/Tokio-Rustls selections refreshed on 2026-09-08. The PostgreSQL
and SQLite components were regenerated with WASI SDK `34` on 2026-09-09; the PostgreSQL
components used `cc` `1.4.5`, as recorded by their current qualification and
producer manifests. A stable release has no prerelease suffix and is not yanked.
The historical MySQL live
qualification likewise retains its actual Rustls `0.23.43` producer; the
current TLS selection is not new SQL runtime qualification evidence.

| Tool | Baseline | Release authority |
| --- | --- | --- |
| Wasmtime | `48.0.1` | [`wasmtime` releases](https://crates.io/crates/wasmtime/48.0.1) |
| WIT Bindgen | `0.61.1` | [`wit-bindgen` releases](https://crates.io/crates/wit-bindgen/0.61.1) |
| WASI SDK | `34` | [`wasi-sdk` releases](https://github.com/WebAssembly/wasi-sdk/releases/tag/wasi-sdk-34) |
| WASI-Virt | `0.2.0` at `448f6df8f688cee5d6995e96b1ffc31f9bf00742` | [`WASI-Virt` source](https://github.com/bytecodealliance/WASI-Virt/tree/448f6df8f688cee5d6995e96b1ffc31f9bf00742) |
| Tokio | `1.53.1` | [`tokio` releases](https://crates.io/crates/tokio/1.53.1) |
| Rustls | `0.23.44` | [`rustls` releases](https://crates.io/crates/rustls/0.23.44) |
| Tokio Rustls | `0.26.5` | [`tokio-rustls` releases](https://crates.io/crates/tokio-rustls/0.26.5) |
...
| CC build adapter | `1.4.5` | [`cc` releases](https://crates.io/crates/cc/1.4.5) |
...
The MySQL driver was rechecked on 2026-09-09. `mysql_async` `0.37.1` requires
`mysql_common ^0.37.1` with default features disabled, resolving to `0.37.3`.
Common's latest stable `0.38.2` is outside that upstream requirement. Sifr
owns no direct common dependency or derive anchor; its codecs use the driver's
exact Row/Value re-exports. An upstream driver accepting the newer common family
is required to move that transitive edge. Historical MySQL live producer
records remain evidence for their recorded toolchain, not this new selection.

`rustls` `0.24.0-dev.1` is a prerelease. It does not qualify as a stable
baseline. The current workspace selects `0.23.44` until Rustls publishes and qualifies a
stable `0.24` release.
...
sandboxed WebAssembly artifact. It uses WASI SDK `34.0` only as a build
toolchain. Pinned WASI-Virt `0.2.0` encapsulates the guest's WASI imports with
deny-by-default implementations. The finished component imports no host
capability. The artifact manifest binds each binary to its parser source and a
deterministic digest of every Rust, WIT, Cargo, and compatibility-layer input.
A small Rust adapter converts its raw parse tree into provider-owned nodes. A provider
supports a PostgreSQL major version only after the matching parser source,
component artifact, component-host execution, and server qualification pass.
...
The implemented PostgreSQL schema and core query compiler is detailed in
[`sql_postgresql_compiler.md`](./sql_postgresql_compiler.md). It defines parser
source selection, provider-owned syntax, schema normalization, semantic facts,
diagnostics, and differential server evidence.
