...
| CLI check/package command reads | `crates/sifr/src/check_and_package_commands.rs:409`, `crates/sifr/src/check_and_package_commands.rs:415`, `crates/sifr/src/check_and_package_commands.rs:427`, `crates/sifr/src/check_and_package_commands.rs:551`, `crates/sifr/src/check_and_package_commands.rs:554`, `crates/sifr/src/check_and_package_commands.rs:587`, `crates/sifr/src/check_and_package_commands.rs:583`, `crates/sifr/src/check_and_package_commands.rs:590`, `crates/sifr/src/check_and_package_commands.rs:601`, `crates/sifr/src/check_and_package_commands.rs:658` | CLI package/check command surfaces probe targets and cache paths and read package sources for command output. | Provider-backed for semantic source reads; command-output/cache probes remain documented exceptions where non-semantic. |
| Package Python environment probe | `crates/sifr_package/src/cargo/python_probe.rs:11`, `crates/sifr_package/src/python/environment.rs:246` | Python interop checks the selected interpreter and configured `pyproject.toml`/`uv.lock` files before running the CPython probe. | Package/environment metadata probes are build identity inputs and should move behind a package-aware snapshot boundary when Python runtime metadata is integrated into broader compiler-service state. |
