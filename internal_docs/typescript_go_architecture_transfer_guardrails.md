...
| Formatter cache metadata | `crates/sifr/src/formatter_cache.rs` | The formatter reads its own versioned cache marker and checks whether the marker exists. | Non-semantic command cache metadata. Keep this operation outside source snapshots. |
...
| Package graph source digest | `crates/sifr_package/src/graph/digest_source_map.rs` | Package graph identity hashes the resolved module source bytes. | Package semantic identity input; keep inventoried until the package source map supplies provider-captured bytes. |
...
| Package Python binding payload reads | `crates/sifr_package/src/python/binding_authoring.rs` | Binding authoring reads package-owned artifact bytes to hash, validate, and preserve the declared payload. | Package authoring identity input; keep inventoried until a package-aware metadata snapshot owns binding payload state. |
...
| Prepared Cargo lock cache metadata | `crates/sifr_driver/src/build/cargo_resolution_cache.rs` | The driver reads a generated lock and its full-key marker before it accepts a prepared-lock cache hit. | Non-semantic build cache metadata. Keep this operation outside source snapshots. |
| Generated-artifact cache lifecycle | `crates/sifr_driver/src/build/cache_lifecycle.rs` | Cache status and cleanup enumerate the fixed Sifr-owned cache root, measure entries, and remove selected cache paths. | Operational cache management. It must not read compiler source or accept an arbitrary root. |
...
| LSP Python input identity | `crates/sifr_lsp/src/python_input_fingerprint.rs` | The LSP hashes package metadata, Python bridge files, app targets, and the selected interpreter state for in-memory invalidation. | Package and tooling identity input. Keep it inventoried until a package-aware provider captures these external inputs. |
