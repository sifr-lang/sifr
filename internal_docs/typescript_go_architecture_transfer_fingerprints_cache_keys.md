...
cache-key identity defines typed keys for:
...
Default constructors use the current cache-key identity policy constants. `ParseCacheKey` also
exposes `with_parser_options`, and `FormatCacheKey` carries a separate
`formatter_options` fingerprint so cache-key identity can key user-configured parser and
