...
- `verification/leetcode/phase31_corpus_inventory.json`
  - raw inventory of every `audits/leetcode/src/*.sifr` fixture in the repository
...
- The raw `audits/leetcode/src` fixture directory currently contains `411` Sifr fixtures.
- Oracle distribution in the raw inventory:
  - `embedded_asserts`: `208`
  - `no_oracle`: `203`
- The historical phase-31 artifacts in `audits/leetcode/src/*.md` are informative but not authoritative because they disagree on corpus size and pass-rate totals.
