# TypeScript-Go Architecture Transfer M5 LSP Persistent Session

status: M5 implementation review

M5 moves LSP analysis ownership out of `DocumentStore` and into the serialized
language-server session. This keeps execution single-threaded while making the
compiler-service owner explicit before M6 dirty scopes and M11/M13 scheduling.
...
M5 does not introduce async scheduling, cancellation, debounce, or precise dirty
scopes. Those remain M6, M11, and M13 responsibilities.
...
- `scripts/run_all_tests.sh --profile quick` -> PASS, report
  `target/validation_lane_reports/quick.latest.json`, wall time 227.66s
