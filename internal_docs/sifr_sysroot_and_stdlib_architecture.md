...
| Encoding, Unicode, and i18n helpers | `_sifr.encoding` and `_sifr.unicode` private Rust interop declarations backed by `sifr_stdlib`; remaining active lowerers in `registry/i18n.rs` | `sifr_stdlib` text/data implementations with shared primitives retained only when justified |
