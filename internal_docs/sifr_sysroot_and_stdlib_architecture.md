...
| Encoding, Unicode, and i18n helpers | `_sifr.encoding` private Rust interop declarations backed by `sifr_stdlib::encoding`; remaining active lowerers in `registry/unicode.rs` and `registry/i18n.rs` | `sifr_stdlib` text/data implementations with shared primitives retained only when justified |
