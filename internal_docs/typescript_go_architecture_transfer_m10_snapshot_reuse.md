...
indexes. Cache entries are addressed by the M9 typed key families and are held
...
- source-file view cache keys include document version because the view carries
  editor metadata, even though M9 content keys intentionally omit document
  identity inputs
...
- package graph, lint, format, and flow graph reuse remain future work even
  though M9 defines their key identities
