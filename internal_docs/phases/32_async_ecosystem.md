...
- PR [#2013](https://github.com/sifr-lang/sifr/pull/2013) milestone_async_3 closure slice: marked structured concurrency and cancellation semantics completed after verifying all positive, negative, runtime-failure, and demo artifacts, running the structured concurrency demo, passing the quick validation lane, and receiving satisfied Opus milestone review.
...
- PR [#2044](https://github.com/sifr-lang/sifr/pull/2044) explicit async-iterator `anext()` slice: `await anext(value)` now lowers through the async iteration protocol for `AsyncIterator[T, E]`, `AsyncGenerator[T, E]`, and structural async iterators, returning `Result[Option[T], E]`; repeated generator advances are marked mutable for Rust lowering, with `async_generator_anext_result_option.sifr` in the quick lane and non-async iterable rejection covered by `anext_non_async_iterator_rejected.sifr`.
