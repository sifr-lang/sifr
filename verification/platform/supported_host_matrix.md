...
| Async subprocess run/output/timeout loopback | supported | supported | host-limited | concurrency/runtime | `process_async_run_output`, `process_async_run_timeout`, and `process_async_output_timeout` validate Tokio-backed async argv run/output and timeout status evidence on Unix shell fixtures. Windows process semantics are not rejected, but require a Windows-specific deterministic fixture before marking supported. |
