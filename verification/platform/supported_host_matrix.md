...
| Sync subprocess stdin pipe writers | supported | supported | host-limited | concurrency/runtime | `process_spawn_pipe_writer` validates sync child stdin `PIPE` extraction, repeated byte writes, explicit close, and typed double-use errors on Unix shell fixtures. Windows requires a deterministic fixture before marking supported. |
