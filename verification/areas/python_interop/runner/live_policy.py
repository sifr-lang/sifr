...
def run_live_policy_self_tests(paths: RunnerPaths) -> None:
    ...
    poisoned_offline = _load_json(
        paths.repo_root / "verification" / "profiles" / "create-pr.json"
    )
...
def _validate_policy_shape(policy: dict[str, Any]) -> None:
    ...
    if missing:
        raise SystemExit(
            f"python interop live policy missing keys: {', '.join(missing)}"
        )
    ...
    if {"container-runtime", "network", "platform-specific"}.difference(
        required_classes
    ):
        raise SystemExit(
            "python interop live policy must require container-runtime, network, and platform-specific"
        )
    statuses = _string_set(policy, "result_statuses")
    if statuses != EXPECTED_STATUSES:
        raise SystemExit(
            f"python interop live policy result statuses drifted: {sorted(statuses)}"
        )
    container = policy["container_runtime"]
    if not isinstance(container, dict):
        raise SystemExit(
            "python interop live policy container_runtime must be an object"
        )
    ...
    if (
        container["docker_required"] is not True
        or container["cleanup_required"] is not True
    ):
        raise SystemExit("python interop live policy must require Docker and cleanup")
    if (
        not isinstance(container["timeout_seconds"], int)
        or container["timeout_seconds"] <= 0
    ):
        raise SystemExit(
            "python interop live policy timeout_seconds must be a positive integer"
        )
    live_suites = _string_set(policy, "live_suites")
    if live_suites != EXPECTED_LIVE_SUITES:
        raise SystemExit(
            f"python interop live policy suite drift: {sorted(live_suites)}"
        )
    ...
    if services != EXPECTED_LIVE_SERVICES:
        raise SystemExit(
            f"python interop live examples service drift: {sorted(services)}"
        )
    ...
    if live_examples["compiled_sifr_binaries_required"] is not True:
        raise SystemExit(
            "python interop live examples must require compiled Sifr binaries"
        )
    if live_examples["service_client_owner"] != "compiled-sifr-bridge":
        raise SystemExit(
            "python interop live service clients must be owned by compiled Sifr bridges"
        )
    if (
    ...
    if live_examples["structured_skip_when_docker_unavailable"] is not True:
        raise SystemExit(
            "python interop live examples must declare structured Docker skip semantics"
        )
...
def _validate_service_runner_source(source: str) -> None:
    ...
    if "execute_live_binary" not in source:
        raise SystemExit(
            "python interop live runner does not execute compiled Sifr binaries"
        )
...
def _validate_live_profile(profile: dict[str, Any], policy: dict[str, Any]) -> None:
    if profile.get("toolchain_steps") != [] or profile.get("guardrail_steps") != []:
        raise SystemExit(
            "python-interop-live profile must select no toolchain or guardrail steps"
        )
    resource_policy = profile.get("resource_policy", {})
    classes = (
        set(resource_policy.get("classes", []))
    ...
    if missing:
        raise SystemExit(
            f"python-interop-live profile is missing resource classes: {', '.join(missing)}"
        )
    ...
    if network_policy.get("live_network_allowed") is not True:
        raise SystemExit(
            "python-interop-live profile must explicitly allow live network"
        )
    ...
    if not isinstance(selections, list) or len(selections) != 1:
        raise SystemExit(
            "python-interop-live profile must select only the python_interop area"
        )
...
def _validate_live_manifest(manifest: dict[str, Any], policy: dict[str, Any]) -> None:
    ...
    for suite_name in _string_list(policy, "live_suites"):
        ...
        if live_suite.get("network_mode") != "live":
            raise SystemExit(
                f"python interop live suite {suite_name} must declare live network mode"
            )
        ...
        if (
            not isinstance(live_suite.get("timeout_seconds"), int)
            or live_suite["timeout_seconds"] <= 0
        ):
            raise SystemExit(
        ...
        if not isinstance(cases, list) or len(cases) != 1:
            raise SystemExit(
                f"python interop live suite {suite_name} must contain exactly one case"
            )
        case = cases[0]
...
def _validate_offline_profiles(repo_root: Path, policy: dict[str, Any]) -> None:
    ...
    for profile_name in _string_list(policy, "offline_profiles"):
        profile = _load_json(
            repo_root / "verification" / "profiles" / f"{profile_name}.json"
        )
...
def _validate_offline_profile_payload(
    ...
    if not isinstance(network_policy, dict) or network_policy.get("mode") != "offline":
        raise SystemExit(
            f"offline profile {profile_name} must keep network mode offline"
        )
    ...
    if "container-runtime" in policy_classes:
        raise SystemExit(
            f"offline profile {profile_name} must not declare container-runtime"
        )
    for selection in profile.get("selected_areas", []):
        ...
        if "container-runtime" in selection_classes:
            raise SystemExit(
                f"offline profile {profile_name} must not select container-runtime"
            )
...
def _expect_policy_failure(callback: Callable[[], None], expected: str) -> None:
    try:
        ...
        raise SystemExit(
            f"negative live-policy self-test failed: expected {expected!r}"
        )
