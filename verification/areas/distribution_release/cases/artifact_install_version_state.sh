...
seed_generation() {
  local install_dir="$1" seed_version="$2"
  local seed_artifacts="$tmp_dir/seed-$seed_version"
  make_target_specific_artifacts "$seed_version" "$seed_artifacts"
  "$REPO_ROOT/scripts/distribution/generate_version_installer.sh" --version "$seed_version" \
    --artifact-dir "$seed_artifacts" --out "$seed_artifacts/install.sh" \
    --artifact-base-url "file://$seed_artifacts" >/dev/null
  SIFR_TARGET="$target" SIFR_INSTALL_DIR="$install_dir" SIFR_NO_MODIFY_PATH=1 \
    sh "$seed_artifacts/install.sh" --no-modify-path >/dev/null
}
known_old_dir="$tmp_dir/known-old/bin"
seed_generation "$known_old_dir" "0.1.0-beta.2"
...
require_failure_contains \
  "select an empty install root for immutable toolchain generations" \
  run_installer "$unknown_old_dir"

newer_dir="${tmp_dir}/newer/bin"
seed_generation "$newer_dir" "0.1.0-beta.4"
require_failure_contains \
