...
CALLABLE_ROOTS = PARSERS + ("sifr_lowering", "sifr_frontend")
...
def import_bindings(code: str, test_ranges: list[tuple[int, int]]) -> tuple[set[str], dict[str, str], bool]:
    """Resolve first-order Rust `use` aliases for every guarded callable/root."""
    roots = set(CALLABLE_ROOTS)
    calls: dict[str, str] = {}
    wildcard = False
    callable_names = set(ENTRYPOINTS + OTHER_FORBIDDEN)
    root_names = "|".join(CALLABLE_ROOTS)
    for match in re.finditer(r"\buse\s+([^;]+);", code, re.DOTALL):
        if within(match.start(), test_ranges):
            continue
        statement = match.group(1)
        root = re.match(rf"\s*({root_names})\b", statement)
        if root is None:
            continue
        rest = statement[root.end():]
        roots.update(re.findall(r"\bself\s+as\s+(\w+)", rest))
        alias = re.match(r"\s+as\s+(\w+)\s*$", rest)
        if alias:
            roots.add(alias.group(1))
        if re.search(r"::\s*\*", rest):
            wildcard = True
        for entry, renamed in re.findall(r"\b(\w+)\b(?:\s+as\s+(\w+))?", rest):
            if entry in callable_names:
                calls[renamed or entry] = entry
    return roots, calls, wildcard
...
def violations_text(path: Path, text: str, *, test_only: bool = False) -> list[str]:
    ...
    roots, imported, wildcard = import_bindings(code, ranges)
    allowed = ALLOWED_SITES.get(rel, {})
    remaining = Counter({line: ALLOWED_COUNTS.get(rel, {}).get(line, 1) for line in allowed})
    failures = []
    names = "|".join(re.escape(name) for name in sorted(set(ENTRYPOINTS + OTHER_FORBIDDEN) | set(imported), key=len, reverse=True))
    candidates = re.finditer(rf"\b(?:(\w+)\s*::\s*)?({names})\s*\(", code)
    for match in candidates:
        if within(match.start(), ranges):
            continue
...
def run_self_test() -> None:
    with tempfile.TemporaryDirectory(prefix="sifr-v03b-f25-") as tmp:
        ...
        for name in ENTRYPOINTS:
            for snippet in (
                f"fn prod() {{ sifr_syntax::{name}(src, None); }}",
        ...
        for owner_crate, entry in (
            ...
            for snippet in (
                f"fn prod() {{ {owner_crate}::{entry}(src); }}",
                f"use {owner_crate}::{entry}; fn prod() {{ {entry}(src); }}",
                f"use {owner_crate}::{{{entry} as alias}}; fn prod() {{ alias(src); }}",
                f"use {owner_crate}::{entry} as alias; fn prod() {{ alias(src); }}",
                f"use {owner_crate} as owner; fn prod() {{ owner::{entry}(src); }}",
                f"use {owner_crate}::{{self as owner}}; fn prod() {{ owner::{entry}(src); }}",
