"""Package management verification area adapter."""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Path for machine-readable package management result summary.",
    )
    parser.add_argument(
        "--hardening-summary",
        action="store_true",
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.bless:
        raise SystemExit("package_management area does not support --bless")
    manifest = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))
    selected = select_suites(manifest, set(args.suite))

    print("Running package management verification area", flush=True)
    ...
    if total_failures:
        ...
        return 1
    prefix = "verification ok" if args.hardening_summary else "package management verification ok"
    print(
...
def select_suites(manifest: dict[str, Any], requested: set[str]) -> list[dict[str, Any]]:
    ...
    if requested:
        ...
        if missing:
            raise SystemExit(f"unknown package_management suite filter(s): {', '.join(missing)}")
    if not selected:
        raise SystemExit("no package_management suites selected")
    return selected
...
def print_case_timing(label: str, elapsed_ms: float, status: str) -> None:
    print(
        f"[sifr-case-timing] bucket=package_management case={timing_token(label)} "
