...
| `test_random` module-level delegation parity | module-level `seed`, `getstate`, `setstate`, `randrange`, `randint`, `random`, `choice`, `choices`, `sample`, `shuffle`, `gauss`, `uniform`, and `randbytes` delegate through one shared module RNG state rules | `adapted` (shipped) | `lib/sifr/random.sifr`, `demos/random_state/main.sifr` |
