# `stdlib_parity_a1` CPython Traceability Matrix
...
| `tuple(dynamic_iterable)` on list variables | `test_tuple.py:34-38` | `waived` | `crates/sifr/tests/e2e/fail/tuple_dynamic_list_shape.sifr` | Sifr tuples are fixed-length typed values, so dynamic iterable-to-tuple conversion remains an explicit intentional difference for this implementation pass. |
| `dict()` / `dict(iterable_of_pairs)` / `dict(mapping, **keywords)` style entry | `test_dict.py:37`, `test_dict.py:382-389`, `test_dict.py:1118-1125`, `test_dict.py:1527-1540` | `adapted` | `crates/sifr/tests/e2e/pass/builtin_callables_and_constructors.sifr` | Iterable-of-pairs and keyword merges work; unpacked `**kwargs` remain out of scope for this implementation pass. |
...
| `zip(..., strict=True)` | `test_builtin.py:2181-2286` | `waived` | `verification/areas/stdlib_parity/reports/stdlib_parity_a1_cpython_traceability.md` | `strict` is intentionally deferred; this implementation pass closes variadic arity and base iterable lowering first. |
...
| `map(..., strict=True)` | `test_builtin.py:1395-1504` | `waived` | `verification/areas/stdlib_parity/reports/stdlib_parity_a1_cpython_traceability.md` | `strict` is deferred with the rest of the iterator-family parity work. |
...
`stdlib_parity_a1` closes the root-cause gap where builtin-special lowering either missed Python-shaped constructor/call forms entirely or silently dropped builtin keywords. Remaining gaps in this slice are explicit waivers, not accidental fallthrough behavior.
