...
| --- | --- | --- |
| `sifr.url.Url` and `UrlQuery` | `production-public`, stable-public-api over ASCII and already-valid Sifr text. | `lib/sifr/url.sifr`; `_sifr.url` signatures in `crates/sifr_stdlib_manifest/src/url.rs`; generated helpers in `crates/sifr_codegen/src/preamble/url_http_runtime.rs`; fixture `network_http_url_query_percent.sifr`. |
