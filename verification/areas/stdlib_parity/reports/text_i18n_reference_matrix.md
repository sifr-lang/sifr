...
| --- | --- | --- |
| `Lib/codecs.py` | 12 public functions, 10 public classes, 42 public methods, 11 BOM constants | Static registry, encode/decode, incremental codecs, BOM fixtures mined for text encoding capability; stream wrappers and registry mutation deferred or unsupported. |
...
| `Modules/_codecsmodule.c`, `Modules/cjkcodecs/*` | Native codec implementation details | Evidence only; no from-scratch CJK codec work in this capability. |
| `Doc/library/unicodedata.rst` | Unicode property API names | Unicode core capability API names and typed error/option behavior adapted for `sifr.unicode`. |
| `Lib/test/test_unicodedata.py` | 7 classes, 42 test methods | Normalization, properties, names/lookup, numeric values, and grapheme tests mined for Unicode core capability/Unicode segmentation capability. |
...
| `Lib/locale.py` | 12 public functions | `setlocale`, `localeconv`, `strcoll`, `strxfrm`, implicit encodings unsupported; locale ID and formatting behavior adapted to object-scoped locale-formatting capability APIs. |
...
| `Lib/test/test_gettext.py` | 20 classes, 67 test methods, `.mo` binary fixtures | `.mo` parser, plural expressions, contexts, fallbacks, malformed catalog fixtures mined for translation-catalog capability. |
...
Accepted or mined aliases include canonical labels and WHATWG-compatible names for `utf-8`, `utf-8-sig`, `ascii`, `latin-1`, `utf-16-le`, `utf-16-be`, and Windows-125x labels covered by `encoding_rs`. CPython-only aliases such as broad codepage, CJK, text-to-text, and bytes-to-bytes module families are not accepted in this capability.

## Fixture Plan

| Planned fixture | Owner capability | Source evidence |
| --- | --- | --- |
...
| `text_i18n_unicode_segmentation.sifr` | Unicode segmentation capability | Unicode UAX #29 grapheme/word examples; CPython has no dedicated stdlib segmentation API. |
...
| `demos/text_i18n/main.sifr` | handoff | End-to-end public demo spanning text/i18n implementation capabilities production APIs. |
