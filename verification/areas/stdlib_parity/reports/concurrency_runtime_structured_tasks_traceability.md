...
| `task.race(...)` | `task_race_cancels_losers`; `task_runtime_contract_tests.rs::test_task_race_consumes_handle_collection_binding` | Homogeneous collection, first completion, loser cancellation, input handle consumption. |
| `task.select(first=..., second=...)` | `task_select_first_completion`; `cancelled_task_use_rejected`; `task_runtime_contract_tests.rs::test_task_select_rejects_positional_branches`; `task_runtime_contract_tests.rs::test_task_select_rejects_single_named_branch` | Named branch call form replaces positional select. Current binary result container maps branch order to `Select2.First` / `Select2.Second`. Duplicate branch names are rejected by the parser before lowering. |
...
| CPython family | Sifr disposition | Representative M1 fixtures |
| --- | --- | --- |
...
| `Lib/test/test_asyncio/test_runners.py` | `waived-with-rationale` | Raw event loop and runner policy APIs are rejected by M0/M0a namespace diagnostics; Sifr code uses compiler-owned async entrypoint lowering. |
...
M1 reserves task context shape only. `sifr.task.Context` propagation, explicit context keys, and request handoff semantics remain owned by M5. Raw event-loop policy, callbacks/transports, and `contextvars` parity remain rejected or deferred under M0/M0a decisions.
