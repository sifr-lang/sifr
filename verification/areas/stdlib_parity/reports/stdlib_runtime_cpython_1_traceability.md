...
Capability: `stdlib_parity_runtime_1`
...
| `test_io` binary file-handle entry paths | keep binary payload rules on first-class `bytes`; add `open_binary(...)` typed entry | `adapted` | `demos/in_memory_streams/main.sifr` |
| full `_pyio` inheritance graph and advanced buffered classes | keep explicitly deferred from baseline capability lock | `unsupported` | `crates/sifr/tests/e2e/fail/pyio_inheritance_unsupported.sifr` |

## Explicit Waivers / Boundaries (Capability 1)

- Advanced `_pyio` hierarchy (`BufferedReader`/`BufferedWriter`/`BufferedRWPair`/`BufferedRandom`) remains `unsupported`.
- File-handle `seek`/`tell` remain explicitly unsupported in this implementation pass until dedicated file-position intrinsic support is introduced.
