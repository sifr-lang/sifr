...
| `sifr.unicode.graphemes` / `words` and index/boundary variants | Unicode segmentation capability | `production-public` | `stable-public-api` | Owned iterators; streaming segmentation deferred. |
...
| `sifr.codecs`, `sifr.encodings`, `sifr.unicodedata`, `sifr.locale`, `sifr.gettext` | adapter capability | `deferred-to-adapter-capability` | `compatibility-adapter` | Not production API centers in this capability. |
| bare `codecs`, `encodings`, `unicodedata`, `locale`, `gettext` imports | text/i18n baseline capability | `unsupported-with-diagnostic` | `compiler-known-intrinsic` | `SIFR-IMPORT-0008` suggests native `sifr.*` APIs after normal resolution fails. |

## Unsupported And Deferred Python-Shaped Surfaces

| Surface | Owner capability | Terminal state | Stability | CPython evidence | Regression fixture / revisit |
| --- | --- | --- | --- | --- | --- |
| `codecs.register` | TCP | `unsupported-with-diagnostic` | `compiler-known-intrinsic` | `Lib/codecs.py`, `Lib/test/test_codecs.py::CodecsModuleTest.test_register` | text encoding capability unsupported registry-mutation fixture; bare root covered by `bare_cpython_text_i18n_imports.sifr`. |
| `codecs.unregister` | TCP | `unsupported-with-diagnostic` | `compiler-known-intrinsic` | `Lib/codecs.py`, `Lib/test/test_codecs.py::CodecsModuleTest.test_unregister` | text encoding capability unsupported registry-mutation fixture. |
| `codecs.register_error` | TCP | `unsupported-with-diagnostic` | `compiler-known-intrinsic` | `Lib/codecs.py`, `Lib/test/test_codecs.py` error-handler families | text encoding capability dynamic error-handler fixture. |
| `codecs.open`, `EncodedFile`, `StreamReader`, `StreamWriter`, `StreamReaderWriter`, `StreamRecoder` | TCP | `deferred-to-adapter-capability` | `compatibility-adapter` | `Lib/codecs.py`, `Lib/test/test_codecs.py` stream classes | Revisit only in adapter capability over `sifr.io.open_text`; bare root covered by `bare_cpython_text_i18n_imports.sifr`. |
| public `encodings.*` modules | TCP | `deferred-to-adapter-capability` | `compatibility-adapter` | `Lib/encodings/*.py`, `Lib/encodings/aliases.py` | `bare_cpython_encodings_import.sifr`; dotted root covered by `bare_cpython_dotted_codecs_import.sifr` pattern and future `encodings.*` fixture. |
| CPython codec C APIs | TCP | `rejected` | `test-only-harness` | `Lib/test/test_capi/test_codecs.py`, `Modules/_codecsmodule.c` | No Sifr external CPython C-extension ABI in this capability. |
| `locale.setlocale` | URL/HTTP primitives | `unsupported-with-diagnostic` | `compiler-known-intrinsic` | `Lib/locale.py`, `Lib/test/test_locale.py::TestRealLocales.test_getsetlocale_issue1813` | `bare_cpython_locale_import.sifr`; locale-formatting capability direct API negative fixture if an adapter root is added. |
| `localeconv` | URL/HTTP primitives | `unsupported-with-diagnostic` | `compiler-known-intrinsic` | `Lib/locale.py`, `Lib/test/test_locale.py` formatting tests | Use object-scoped `NumberFormatter`; locale-formatting capability negative fixture. |
| `strcoll` / `strxfrm` | URL/HTTP primitives | `deferred-to-adapter-capability` | `compatibility-adapter` | `Lib/locale.py`, `Lib/test/test_locale.py::TestCollation` | Native `Collator` is production API; adapter revisit requires no process-global locale. |
| implicit preferred text encoding | text encoding capability/locale-formatting capability | `rejected` | `compiler-known-intrinsic` | `Lib/locale.py`, `Lib/test/test_locale.py::TestMiscellaneous.test_getpreferredencoding` | text encoding capability missing-encoding `open(...)` diagnostics. |
| `gettext.install` and global `_` | HTTP transport | `unsupported-with-diagnostic` | `compiler-known-intrinsic` | `Lib/gettext.py`, `Lib/test/test_gettext.py::MiscTestCase` | `bare_cpython_gettext_import.sifr`; translation-catalog capability direct negative fixture if adapter root is added. |
| `gettext.textdomain` / `bindtextdomain` and module-global domain functions | HTTP transport | `deferred-to-adapter-capability` | `compatibility-adapter` | `Lib/gettext.py`, `Lib/test/test_gettext.py::GettextTestCase2` | Native `Bundle`/`Translator` fallback chains are production API. |

## Encoding Tier Decisions

| Tier | State | Encodings |
| --- | --- | --- |
| Tier 0 | required text encoding capability | `utf-8`, `utf-8-sig`, `ascii`, exact `latin-1`, `utf-16-le`, `utf-16-be` |
| Tier 1 | required text encoding capability where covered by `encoding_rs` | `windows-1252` and selected WHATWG `windows-125x` labels |
| Tier 2 | `deferred-to-encoding-expansion-capability` | `utf-32-le`, `utf-32-be`, `Shift_JIS`, `EUC-JP`, `GBK`, `GB18030`, `Big5`, `EUC-KR` |
| Tier 3 | `rejected` / `deferred-to-adapter-capability` | CPython-only aliases, text-to-text codecs, bytes-to-bytes pseudo-codecs, public `encodings.*` parity |
...
### Exact text encoding capability Alias Table
...
text/i18n implementation capabilities must hide crate types behind Sifr-owned runtime/compiler API shapes, map all crate errors into typed Sifr errors, and scan emitted/generated runtime code for data-dependent `unwrap`, `expect`, or `panic`.
...
Text/i18n follows the shared platform no-global-state policy in [platform_rules.md](../platform/platform_rules.md). The static codec registry is immutable, locale-sensitive APIs are object-scoped, host locale discovery is read-only, `gettext.install`/global `_` mutation is unsupported, and no locale query can legalize text I/O without an explicit `encoding=`.
...
baseline smoke commands passed:

- `cargo run -q -p sifr -- run demos/binary_files/main.sifr`
- `cargo run -q -p sifr -- run demos/bytes_file_io/main.sifr`

Existing coverage also records in-memory `StringIO`/`BytesIO` seek/tell and use-after-close behavior in `crates/sifr/tests/e2e/pass/in_memory_streams.sifr`. File-handle seek/tell remains explicitly unsupported where not implemented; text-mode work must not duplicate or work around file-position behavior.

## Pending Capability Queue

| Capability | Concrete pending work |
| --- | --- |
...
| Internal architecture | `internal_docs/architecture.md` records the architecture rules, and `internal_docs/text_i18n_architecture.md` carries the focused substrate readiness note. |
...
| Dependency snapshots | `verification/areas/stdlib_parity/data/text_i18n_dependency_snapshots.json` records generated Cargo dependency snapshots for each text/i18n module and every pairwise/full module combination; `crates/sifr_stdlib_model/src/features.rs::text_i18n_feature_dependency_snapshots_cover_feature_combinations` locks the same combinations in unit tests. |
