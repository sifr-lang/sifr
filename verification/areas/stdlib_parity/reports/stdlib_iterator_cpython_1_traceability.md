...
| --- | --- | --- | --- |
| `test_iter` reverse-capability behavior | enforce explicit reversible capability at type-check time (`reversed(...)` rejects non-double-ended iterators) | `adapted` (closed in wave 1 typing layer) | `crates/sifr/tests/e2e/fail/reversed_iterator_not_reversible.sifr` |
| `test_iter` iterable protocol annotations | add `Reversible[T]` protocol alias and capability-aware assignability for builtin containers | `adapted` (closed in wave 1 typing layer) | `crates/sifr/tests/e2e/fail/reversible_annotation_rejects_set.sifr`, `crates/sifr/tests/e2e/pass/reversible_iteration.sifr` |
| `test_tuple` homogeneous vs heterogeneous iteration | allow homogeneous tuple iteration and reject heterogeneous tuple iteration with explicit diagnostics | `adapted` (closed in wave 1 typing layer) | `crates/sifr/tests/e2e/pass/reversible_iteration.sifr`, `crates/sifr/tests/e2e/fail/iter_heterogeneous_tuple_unsupported.sifr` |
