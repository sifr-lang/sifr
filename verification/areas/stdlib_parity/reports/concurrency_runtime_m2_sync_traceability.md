...
| `Barrier` / public `Once` | This traceability artifact; phase decision register | Deferred/internal-only for M2. Channels, locks, semaphores, and notify cover the near-term production coordination model. |

## CPython Family Mapping

| CPython family | Sifr disposition | Representative M2 fixtures |
| --- | --- | --- |
...
| `sifr.asyncio.Queue` and CPython-shaped queue modules | `unsupported-with-diagnostic` / `rejected` | M0a removes legacy public adapters; production APIs use `sifr.sync`. |
...
M2 does not add `sync.AsyncMutex[T]`, `sync.AsyncRwLock[T]`, public `Barrier`, public `Once`, Python-shaped queue accounting, or raw Tokio sync types. Those require a later phase amendment or milestone with explicit await-safe guard and lifecycle semantics.
