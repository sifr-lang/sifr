...
Capability: `stdlib_parity_iter_fix_3`
...
| --- | --- | --- | --- |
| `test_iter` iterator-consumer codegen | lower iterator consumers through canonical iterable-to-owned-iterator conversion path | `adapted` (closed in locale-formatting capability codegen layer) | `crates/sifr/tests/e2e/pass/iterator_consumers.sifr` |
| `test_filter` iterator-input readiness | `filter(pred, iter(xs))` must compile through concrete iterator chain instead of unresolved builtin symbol fallback | `adapted` (closed in locale-formatting capability codegen layer) | `crates/sifr/tests/e2e/pass/iterator_consumers.sifr` |
| `test_sort` iterable-input readiness | `sorted(iter(xs))` must materialize only at explicit eager boundary and accept iterator-typed sources | `adapted` (closed in locale-formatting capability codegen layer) | `crates/sifr/tests/e2e/pass/iterator_consumers.sifr` |
