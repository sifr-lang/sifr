...
def main() -> int:
    ...
    if not virt_manifest.is_file():
        raise SystemExit("initialize the pinned third_party/wasi-virt submodule")
    virt_package = tomllib.loads(virt_manifest.read_text(encoding="utf-8"))["package"]
    if virt_package.get("version") != "0.2.0":
        raise SystemExit("PostgreSQL components require WASI-Virt 0.2.0")
    virt_commit = subprocess.check_output(
        ["git", "-C", str(WASI_VIRT), "rev-parse", "HEAD"], text=True
    ).strip()
    if virt_commit != WASI_VIRT_COMMIT:
        raise SystemExit("third_party/wasi-virt commit has drifted")
    if tracked_content_sha256(WASI_VIRT) != WASI_VIRT_SOURCE_SHA256:
