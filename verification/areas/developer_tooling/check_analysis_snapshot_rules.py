#!/usr/bin/env python3
"""Validate the editor tooling AnalysisHost snapshot/session rules."""
...
def run_self_test() -> None:
    ...
    if result.returncode != 0 or "stale_snapshot_is_rejected_after_update ... ok" not in result.stdout:
        ...
        raise SystemExit("analysis snapshot rules self-test failed: stale snapshot test did not pass")
    print("analysis snapshot rules self-test: PASS")
...
def main() -> int:
    ...
    if missing:
        print("analysis snapshot rules: FAIL", file=sys.stderr)
        for test in missing:
            print(f"  - missing required test evidence: {test}", file=sys.stderr)
        return 1
    print("analysis snapshot rules: PASS")
    return 0
