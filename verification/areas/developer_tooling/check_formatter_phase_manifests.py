...
def phase_doc_path(name: str) -> Path:
    ...
    if active_path.exists():
        return active_path
    archived_path = REPO_ROOT / "plans" / "issues" / "archive" / name
...
def check_capability_manifest(failures: list[str], docs_text: str) -> None:
...
def check_cli_manifest(failures: list[str], docs_text: str) -> None:
...
def check_ast_manifest(failures: list[str], docs_text: str) -> None:
    ...
    for required_phrase in [
        ...
        "own mut",
        "mut own",
        "Sifr type annotation extensions",
        "generics",
        "match/case",
        "ownership-aware collection",
        "docstring code",
    ]:
        require(
            required_phrase in docs_text,
            f"reviewed docs no longer mention AST coverage phrase: {required_phrase}",
            failures,
        )
...
def run_positive() -> None:
    ...
    failures: list[str] = []
    check_capability_manifest(failures, docs_text)
    check_cli_manifest(failures, docs_text)
