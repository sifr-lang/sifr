...
WRITE_OPERATIONS = set(WRITE_FUNCTIONS) | {"create", "create_new", "open-write"}
...
def patterns(code: str) -> list[tuple[str, re.Pattern[str]]]:
    ...
    for match in re.finditer(r"\buse\s+std::fs::(?:\{([^}]*)\}|(File|OpenOptions)(?:\s+as\s+(\w+))?)\s*;", code, re.S):
        items = match[1] or (match[2] + (" as " + match[3] if match[3] else ""))
        for item in items.split(","):
            part = re.fullmatch(r"\s*(File|OpenOptions)(?:\s+as\s+(\w+))?\s*", item)
            if part:
                (file_aliases if part[1] == "File" else options_aliases).add(part[2] or part[1])
...
def sites(root: Path) -> list[dict[str, str]]:
    ...
    for path in source_paths(root):
        ...
        if not any(token in raw for token in ("std::fs", "fs::", "File::", "OpenOptions", ".is_file(", ".is_dir(", ".exists(", ".try_exists(", ".metadata(", ".canonicalize(", ".read_link(")):
            continue
        code = rust_code(raw)
...
def compare(found: list[dict[str, str]], expected: list[dict[str, str]]) -> list[str]:
    ...
    failures.extend(f"write effect misclassified: {item.get('path')}::{item.get('symbol')}"
