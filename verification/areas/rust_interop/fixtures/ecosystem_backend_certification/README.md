...
- Negative evidence: `sqlx_without_offline_artifacts` independently removes
  and stale-mutates the query metadata. Sifr forces `SQLX_OFFLINE=true`,
  removes `DATABASE_URL`, reports `SIFR-RUST-CARGO-0001`, and never connects
  to the armed database sentinel.
