...
Intentional post-M3 follow-up boundaries:
...
- Process-global panic-hook mutation is no longer unsynchronized across generated M3 CPU/Rayon surfaces: `sifr.parallel`, configured `Pool` work, `TaskScope`/`TaskGroup` scoped CPU offload, `task.spawn_cpu`, and `JoinSet.spawn_cpu` share one generated mutex around the temporary hook swap. Stable Rust exposes panic hooks as process-global state, so a fully per-thread hook is not available without a future runtime/crate decision.
