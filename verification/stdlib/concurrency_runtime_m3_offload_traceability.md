...
Status: first implementation wave active.
...
| Generated dependency gating | `crates/sifr_stdlib/src/features.rs`; `crates/sifr_codegen/src/lib_modules_and_codegen.rs` | Importing `sifr.parallel` adds `rayon = "1.12.0"` to generated projects only when the module is used. |
| Worker panic boundary | `parallel_map_worker_panic_typed` | Rayon worker calls are wrapped in `catch_unwind` and converted into `WorkerRuntimeError`/`WorkerError` instead of propagating a process panic. |

## CPython Family Mapping

| CPython family | Sifr disposition | Representative M3 fixtures |
| --- | --- | --- |
| `Lib/test/test_concurrent_futures/` map ordering and worker failure behavior | `adapted-for-sifr-api` | `parallel_map_basic`, `parallel_try_map_basic`, `parallel_map_worker_panic_typed`, `parallel_try_map_user_error_typed` |
...
| --- | --- |
| Create PR | `parallel_map_basic`, `parallel_try_map_basic`, `parallel_pool_map_basic`, `parallel_map_worker_panic_typed`, `parallel_try_map_user_error_typed` |
| Merge | `parallel_map_basic`, `parallel_try_map_basic`, `parallel_pool_map_basic`, `parallel_map_worker_panic_typed`, `parallel_try_map_user_error_typed` |
| Fail suite | `parallel_map_async_direct_rejected`, `parallel_map_non_send_item_rejected` |

## Open Follow-up Boundaries

This first wave does not close all of M3. Remaining M3 work before milestone closure:

- `sifr.runtime.spawn_cpu` and scoped CPU offload methods.
