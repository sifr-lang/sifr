...
Status: In progress. The design gate is approved, dependency metadata is wired, `ipc_value_model_basic` validates the host-independent schema/frame/backpressure value model, and internal `sifr_stdlib` helpers now encode/decode/read/write length-prefixed Postcard envelopes, track request IDs and bounded in-flight windows, validate bootstrap/established-frame connection state, validate host-independent payload eligibility with unsupported-payload evidence, extract generated schema descriptors from concrete Sifr type graphs, enforce representative compile-time payload diagnostics for concrete `require_serializable(...)` marker calls, and exercise Unix child-process stdin/stdout pipes for bootstrap, completion, cancellation, shutdown close, backpressure, malformed-frame, and unsupported-payload evidence. Generated worker integration remains M6 implementation work; Windows child-process fixture evidence remains host-limited follow-up work.
...
| Internal schema descriptor and hash v1 | `cargo test -p sifr_stdlib ipc_schema -- --nocapture` | `sifr_stdlib::ipc_schema` renders canonical schema descriptors and stable FNV-1a-128 schema hashes without adding a new hash dependency. Generated worker integration remains follow-up work. |
| Compiler-side schema extraction from Sifr types | `cargo test -p sifr_lowering ipc_schema_extraction -- --nocapture` | `sifr_lowering::lower::ipc_schema_extraction` converts concrete Sifr `Type` graphs for records, enums, newtypes, options, results, tuples, lists, and `dict[str, T]` into canonical `IpcSchemaDescriptor` values, validates them through the internal payload-eligibility helper, and rejects schema-less/dynamic/process-local shapes before generated peers are considered wire-compatible. |
...
7. Extend process-pipe fixture coverage to Windows and generated worker boundaries when those are accepted.
