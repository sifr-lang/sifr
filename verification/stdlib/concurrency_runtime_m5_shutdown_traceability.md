...
Status: In progress; signal value-model foundation covers portable `sifr.signal.Signal`, `sigint()`, `sigterm()`, `SIGINT`, `SIGTERM`, and `strsignal(signal)` evidence, structured signal stream shape/lowering now covers Ctrl-C and Unix SIGTERM shutdown waits, deterministic Unix signal-delivery evidence covers child-sent Ctrl-C, SIGTERM, and shutdown-stream waits, deterministic cleanup helper coverage includes no-value and value-carrying generic `sifr.resource.nullcontext(...)`, and task context value types are importable through `sifr.task.Context` / `ContextKey[T]`. Cleanup stacks, explicit task context propagation, and structured runtime diagnostics remain M5 work.
...
| --- | --- |
| Create PR | `signal_value_model_basic`; `signal_strsignal_basic`; `signal_constants_basic`; `signal_stream_shape_strsignal`; `signal_stream_delivery_unix`; `task_context_value_model_basic`; `resource_nullcontext_basic` |
| Merge | `signal_value_model_basic`; `signal_strsignal_basic`; `signal_constants_basic`; `signal_stream_shape_strsignal`; `signal_stream_delivery_unix`; `task_context_value_model_basic`; `resource_nullcontext_basic` |
...
- `nullcontext()` is supported as a no-op cleanup helper and `nullcontext(value)` preserves the entered value type; cleanup stacks and owned closing helpers are closed as unsupported diagnostics in this phase until an owned-close protocol can preserve cleanup errors honestly. Task context propagation and structured diagnostics/tracing remain separate M5 waves.
