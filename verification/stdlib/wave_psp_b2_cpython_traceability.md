...
| Residual `random` waiver family (`choices(weights=...)`, `SystemRandom.getstate/setstate`) | `unsupported` | Follow-up `wave_psp_rng_1` now ships deterministic mutable-state parity (`RandomState`, `Random`, module-level delegation, `randbytes`). Remaining unsupported pieces are weighted-distribution `choices(weights=...)` and host-backed `SystemRandom` state export/import (guarded by `phase_psp_b2_random_choices_weights_unsupported.sifr` and `phase_psp_rng_1_system_random_state_unsupported.sifr`). |
