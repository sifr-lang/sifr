...
| Generated Cargo dependency snapshots | pending-pr | `verification/stdlib/concurrency_runtime_dependency_snapshots.json` records resolver-backed snapshots for Tokio task/sync/process/signal/offload paths, Rayon parallel map, runtime diagnostics metrics/tracing, and IPC Postcard/Serde serialization dependencies; `crates/sifr_stdlib/tests/concurrency_runtime_dependency_snapshots.rs` compares the snapshot to `sifr_stdlib::generated_cargo_dependencies(...)`. |
