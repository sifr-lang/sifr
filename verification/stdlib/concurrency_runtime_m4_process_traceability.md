...
| `sifr.process.PipeReader` | `process_spawn_pipe_readers` | `Child.stdout()` and `Child.stderr()` transfer owned stdout/stderr pipes into one-shot `PipeReader.read_all()` handles. Double reads, missing pipes, and repeated child pipe extraction return typed `ProcessError`; stdin `PipeWriter`, streaming reads, async pipes, and sendability/shareability checks remain later M4 work. |
...
| Sync `spawn`, `wait`, `Child.wait` | `process_spawn_wait_status`; `process_spawn_pipe_readers`; `process_wait_direct_async_rejected`; `process_child_wait_method_direct_async_rejected` | Sync child lifecycle stores `std::process::Child` behind a private generated handle table. `wait(child)` and `Child.wait()` are one-shot observation paths; top-level `wait(child)` and method-form `Child.wait()` are `@blocking_io` and direct async calls are rejected. This wave adds sync stdout/stderr pipe extraction; stdin pipe writing, async wait, termination, timeout, and scoped supervision remain later M4 work. |
...
| --- | --- | --- |
| `Lib/test/test_subprocess.py` sync argv, env, cwd, stdin/stdout/stderr, return-code behavior | `adapted-for-sifr-api` | `process_sync_output_text`, `process_sync_bytes_env_cwd_stdin`, `process_shell_exec_output`, `process_spawn_pipe_readers` |
...
| `Lib/test/test_asyncio/test_subprocess.py` async process lifecycle | `adapted-for-sifr-api` / `planned-for-M4-follow-up` | `process_async_run_output` covers initial async argv run/output loopback. Async spawn/wait/communicate, async stdin/pipes, cancellation, and scoped supervision fixtures remain open. |

## Validation Coverage

| Lane | Representative entries |
| --- | --- |
| Create PR | `process_sync_output_text`, `process_sync_bytes_env_cwd_stdin`, `process_shell_exec_output`, `process_spawn_wait_status`, `process_spawn_pipe_readers`, `process_timeout_status`, `process_signal_status`, `process_async_run_output`, `process_child_kill_wait` |
| Merge | `process_sync_output_text`, `process_sync_bytes_env_cwd_stdin`, `process_shell_exec_output`, `process_spawn_wait_status`, `process_spawn_pipe_readers`, `process_timeout_status`, `process_signal_status`, `process_async_run_output`, `process_child_kill_wait` |
...
- `PipeWriter`, owned stdin pipe lifecycle, streaming `PipeReader` reads beyond one-shot `read_all`, and handle sendability/shareability checks beyond the one-shot sync `Child.wait()` state.
