...
| `sifr.process.Status` | `process_sync_output_text`; `process_shell_exec_output`; `process_spawn_wait_status`; `process_timeout_status`; `process_child_kill_wait` | Sync status evidence distinguishes normal success from nonzero exit through `success`, `code`, and `kind`. Child wait reuses the same status evidence for one-shot sync observation. Timeout status evidence sets `kind == "timeout"` and `timed_out == True`; Unix child signal exits set `kind == "signal"` and `signal is not None`. Cancellation fields remain open for later lifecycle waves. |
...
- Graceful `terminate`, termination escalation, parent cancellation evidence, and supported-host matrix updates for process termination behavior beyond the current Unix signal / non-Unix sentinel wait status.
