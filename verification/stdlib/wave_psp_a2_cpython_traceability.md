...
| `Lib/test/test_dict.py` | `dict.update()`/iterable-of-pairs/`**kwargs` and `dict.pop(key, default)` | `crates/sifr/tests/e2e/pass/phase_psp_a2_core_object_model_surface.sifr` | adapted | Default values remain statically typed to the dict value type, and Sifr accepts `default=` keyword binding for `dict.pop(...)` as an intentional convenience adaptation even though CPython keeps that parameter positional-only. |
