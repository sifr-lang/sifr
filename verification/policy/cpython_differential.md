...
Profiles selecting either generated suite prepare its release compiler during the explicit setup step, using the same configured target and locked offline build. Cold compilation is recorded as setup work. Preparation also runs each selected deterministic generated program once through the normal CLI run cache, using the existing 300-second build deadline to allow native compilation. Its output, exit status and duration are retained in `preparation.json`; an unexpected exit or timeout fails preparation. Preparation and comparisons share the same generated source paths. The oracle then reruns every selected program through the same compiler/cache path with the unchanged 20-second per-program limit and performs all normal CPython comparisons. The runner still verifies the release compiler build within its unchanged timeout before those comparisons.

Generated oracle suites use `verification/areas/cpython_differential/data/generated_seed_manifest.json` as the seed and coverage ledger. The runner builds the Sifr CLI once with `cargo build --release -p sifr`, honors `CARGO_TARGET_DIR` (relative to the checkout when not absolute), records the selected release binary path and SHA-256 plus a source digest over the declared inputs, and then runs generated Sifr programs with that exact recorded binary and an explicit checkout sysroot argument. With no target override, the default remains `target/release/sifr`. A missing configured artifact or failed build is an error; the runner never substitutes a binary from another target directory. Each generated program has a per-program timeout and each generated suite has an overall timeout recorded in the manifest. Generated comparison checks stdout byte equality after the documented CRLF-to-LF normalization, exit-code buckets (`0` or `non-zero`), and error presence (`no-error`, `compile-error`, or `runtime-error`). Error message text is never compared. Generated dict behavior is currently tested through existing-key lookups that serialize canonical lookup facts; direct dict serialization is excluded from generated programs until Sifr exposes canonical key ordering for that comparison boundary. Generated failures must write the original seed and a minimized candidate under `target/verification/actual/cpython_differential/generated/**`; checked-in minimized regressions are tracked through `verification/areas/cpython_differential/data/minimized_failures.json` before promotion to `regression:fixedbugs`.

Generated suites remain nightly/release-only until the generated seed ledger records 20 consecutive green nightly runs with zero quarantine entries and zero flaky retries. Only then may the deterministic `generated_minimized_seeds` subset be promoted to the merge profile.
...
| 10 | `D010_FLOAT_PRECISION` | Float formatting, NaN, infinities, signed zero, and platform math precision are not in the first oracle subset. | Do not generate floats unless a later policy row gives an operation-specific exactness rules. |
