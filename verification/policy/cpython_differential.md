...
Generated oracle suites use `verification/areas/cpython_differential/data/generated_seed_manifest.json` as the seed and coverage ledger. The runner builds the Sifr CLI once with `cargo build --release -p sifr`, records the `target/release/sifr` SHA-256 plus a source digest over the declared inputs, and then runs generated Sifr programs with that release binary and an explicit checkout sysroot argument. Each generated program has a per-program timeout and each generated suite has an overall timeout recorded in the manifest. Generated comparison checks stdout byte equality after the documented CRLF-to-LF normalization, exit-code buckets (`0` or `non-zero`), and error presence (`no-error`, `compile-error`, or `runtime-error`). Error message text is never compared. Generated dict behavior is currently tested through existing-key lookups that serialize canonical lookup facts; direct dict serialization is excluded from generated programs until Sifr exposes canonical key ordering for that comparison boundary. Generated failures must write the original seed and a minimized candidate under `target/verification/actual/cpython_differential/generated/**`; checked-in minimized regressions are tracked through `verification/areas/cpython_differential/data/minimized_failures.json` before promotion to `regression:fixedbugs`.
