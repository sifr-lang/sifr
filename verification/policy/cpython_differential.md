...
Profiles selecting either generated suite prepare its release compiler during the explicit setup step, using the same configured target and locked offline build. Cold compilation is recorded as setup work; the generated runner still verifies the build within its unchanged timeout before comparing programs.
