#!/usr/bin/env python3
"""Validate the diagnostic presentation phase contract."""
...
def check_run_all_wiring(root: Path) -> None:
    ...
    script = "verification/tooling/check_diagnostic_presentation_contract.py"
    require(script in text, "run_all_tests.sh missing diagnostic presentation contract check")
    require(
        f"{script}\" --self-test" in text or f"{script} --self-test" in text,
        "run_all_tests.sh missing diagnostic presentation contract self-test",
    )


def check_schema_lock(root: Path) -> None:
    lock = read_json(root, "verification/tooling/diagnostic_presentation_schema_lock.json")
...
def check_human_baselines(root: Path) -> None:
    ...
    presentation = baseline_path(
        root, "presentation_contract_cases", "check-human.stderr.txt"
    ).read_text(encoding="utf-8")
    require("::: " in presentation, "human contract baseline missing related span")
    require("= related span" in presentation, "human contract baseline missing related label")
    require("= location: <unavailable>" in presentation, "human contract baseline missing spanless fallback")
    require("= note: child note rendered" in presentation, "human contract baseline missing child note")
    require("= help: child help rendered" in presentation, "human contract baseline missing help")
    require("= suggestion: replace value" in presentation, "human contract baseline missing suggestion")
    require("\r" not in presentation, "human contract baseline must not print CR characters")
...
def check_json_baselines(root: Path) -> None:
    ...
    require(
        ...
        "json contract baseline must preserve CR source text semantics",
    )
    require(presentation[0]["suggestions"], "json contract baseline missing suggestions")
...
def seed_minimal_repo(root: Path) -> None:
    ...
    write(
        root / "verification/tooling/diagnostic_presentation_schema_lock.json",
...
def run_self_tests() -> None:
    ...
    expect_self_test_failure(
        ...
        "missing diagnostic presentation contract self-test",
        lambda root: write(
            root / "scripts/run_all_tests.sh",
            "python3 verification/tooling/check_diagnostic_presentation_contract.py\n",
        ),
...
def main() -> int:
    ...
    try:
        if args.self_test:
            ...
            print("diagnostic presentation contract: PASS")
    except AssertionError as error:
        print(f"diagnostic presentation contract: FAIL: {error}", flush=True)
        return 1
    return 0
