...
def check_run_all_wiring(root: Path) -> None:
    ...
    script = "verification/tooling/check_diagnostic_source_canonicalization_contract.py"
    require(script in text, "run_all_tests.sh missing source canonicalization contract check")
    require(
        f"{script}\" --self-test" in text or f"{script} --self-test" in text,
        "run_all_tests.sh missing source canonicalization contract self-test",
    )
