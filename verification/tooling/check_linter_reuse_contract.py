#!/usr/bin/env python3
"""Validate the production-grade Sifr linter reuse contract."""
...
MANIFEST_DIR = REPO_ROOT / "verification" / "tooling" / "linter_manifests"
...
def load_json(path: Path) -> Any:
    try:
        ...
        raise ContractError(f"missing manifest: {path.relative_to(REPO_ROOT)}") from exc
    except json.JSONDecodeError as exc:
        raise ContractError(f"invalid JSON in {path.relative_to(REPO_ROOT)}: {exc}") from exc
...
def validate_cli_manifest(manifest: dict[str, Any]) -> None:
    ...
    for output_format in output_formats:
        ...
        require(output_format.get("schema_contract"), f"missing output-format schema contract: {output_format}")
...
def expect_failure(func: Any, label: str) -> None:
    try:
        ...
        return
    raise ContractError(f"self-test failed: {label} did not fail")
...
def main() -> int:
    ...
    try:
        if args.self_test:
            run_self_test()
            print("linter reuse contract self-test: PASS")
        else:
            validate_all()
            print("linter reuse contract: PASS")
    except ContractError as error:
        print(f"linter reuse contract: FAIL: {error}", file=sys.stderr)
        return 1
    return 0
