...
MATRIX_PATH = REPO_ROOT / "verification" / "tooling" / "lsp_protocol_matrix.json"
VSCODE_CONTRACT_PATH = REPO_ROOT / "verification" / "tooling" / "vscode_extension_contract.json"
...
def validate_vscode_contract(contract: dict[str, Any]) -> list[str]:
    failures: list[str] = []
    ...
    if contract.get("launch", {}).get("default_command") != "sifr":
    ...
    forbidden = set(contract.get("forbidden_extension_behavior", []))
...
def validate_paths() -> list[str]:
    ...
    if not VSCODE_CONTRACT_PATH.exists():
        failures.append(f"required VS Code contract missing: {VSCODE_CONTRACT_PATH.relative_to(REPO_ROOT)}")
    return failures
...
def validate() -> list[str]:
    ...
    if not failures:
        ...
        failures.extend(validate_vscode_contract(read_json(VSCODE_CONTRACT_PATH)))
    return failures
...
def run_self_test() -> None:
    ...
    if not any("textDocument/completion" in failure for failure in failures):
        raise SystemExit("contract lock self-test failed: missing completion method passed")
    print("tooling contract lock self-test: PASS")
...
def main() -> int:
    ...
    failures = validate()
    if failures:
        print("tooling contract lock: FAIL")
        for failure in failures:
            print(f"  - {failure}")
        return 1
    print("tooling contract lock: PASS")
    return 0
