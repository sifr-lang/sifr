...
fn collect_logger_actual(base: &String) -> Vec<bool> {
    ...
    let __sifr_try_res: Result<(), IOError> = (|| {
    let _w: () = std::fs::write(&app_log, "".to_string().as_bytes()).map(|_| ()).map_err(__io_err)?;
...
fn collect_root_and_handler_actual(base: &String) -> Vec<bool> {
    ...
    let __sifr_try_res: Result<(), IOError> = (|| {
    let _w1: () = std::fs::write(&root_log, "".to_string().as_bytes()).map(|_| ()).map_err(__io_err)?;
    let _w2: () = std::fs::write(&handler_log, "".to_string().as_bytes()).map(|_| ()).map_err(__io_err)?;
    ...
    let mut handler: FileHandler = FileHandler::new(format!("{}{}", handler_log, "".to_string()), 0 as i64);
