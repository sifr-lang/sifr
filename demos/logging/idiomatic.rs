...
fn mktemp_path(prefix: &String) -> String {
    ...
    if (root.chars().count() as i64) == (0 as i64) {
        ...
        if let Some(last) = last {
            if last == "/".to_string() {
                root = String::from_iter((root).chars().skip(0 as usize).take(
                    (((root.chars().count() as i64) - (1 as i64)).max(0) - 0).max(0) as usize,
                ));
            }
        }
    }
...
fn collect_logger_actual(base: &String) -> Vec<bool> {
    ...
    let app_log: String = format!("{}{}", base, "/app.log".to_string());
    let mut app_ok: bool = false;
    let __sifr_try_res: Result<(), IOError> = (|| {
        let _w: () = std::fs::write(&app_log, "".to_string().as_bytes())
            .map(|_| ())
            .map_err(__io_err)?;
        let mut app: Logger = getLogger(&"demo".to_string());
        app.set_file(&app_log);
        app.set_level(INFO);
        app.info(&"start".to_string());
        app.warning(&"warn".to_string());
        app.debug(&"hidden".to_string());
        let app_content: String = std::fs::read_to_string(&app_log).map_err(__io_err)?;
...
fn collect_root_and_handler_actual(base: &String) -> Vec<bool> {
    let mut actual: Vec<bool> = vec![];
    let root_log: String = format!("{}{}", base, "/root.log".to_string());
    let handler_log: String = format!("{}{}", base, "/handler.log".to_string());
    let mut root_ok: bool = false;
    let mut handler_ok: bool = false;
    let __sifr_try_res: Result<(), IOError> = (|| {
        let _w1: () = std::fs::write(&root_log, "".to_string().as_bytes())
            .map(|_| ())
            .map_err(__io_err)?;
        let _w2: () = std::fs::write(&handler_log, "".to_string().as_bytes())
            .map(|_| ())
            .map_err(__io_err)?;
        ...
        let root_content: String = std::fs::read_to_string(&root_log).map_err(__io_err)?;
        root_ok = root_content == "[ERROR] root: boom\n".to_string();
        let mut handler: FileHandler =
            FileHandler::new(format!("{}{}", handler_log, "".to_string()), 0 as i64);
        handler.set_formatter(&Formatter::new("%(levelname)s:%(message)s".to_string()));
        handler.emit(
            &"INFO".to_string(),
            &"demo".to_string(),
            &"formatted".to_string(),
        );
        let handler_content: String = std::fs::read_to_string(&handler_log).map_err(__io_err)?;
...
fn collect_safety_actual(base: &String) -> Vec<bool> {
    let mut actual: Vec<bool> = vec![];
    let missing_log: String = format!("{}{}", base, "/missing/blocked.log".to_string());
    let mut missing_safe: bool = false;
    let __sifr_try_res: Result<(), IOError> = (|| {
        let mut bad: Logger = getLogger(&"bad".to_string());
        bad.set_file(&missing_log);
        bad.error(&"should fail".to_string());
        missing_safe = !(std::path::Path::new(&missing_log).exists());
        return Ok(());
    })();
    if let Err(__sifr_try_err) = __sifr_try_res {
        let e = __sifr_try_err.clone();
    ...
    actual.push((INFO == (20 as i64)) && (WARNING == (30 as i64)));
    return actual;
}
