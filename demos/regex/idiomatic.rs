...
fn collect_primary_actual() -> Vec<bool> {
    ...
    let __sifr_try_res: Result<(), RegexError> = (|| {
        let m: bool = regex::Regex::new(&"[0-9]+".to_string())
            .map(|re| re.is_match(&"42 bottles".to_string()))
            .map_err(|e| RegexError {
                message: e.to_string(),
                detail: e.to_string(),
            })?;
        match_ok = m;
        let found_num: Option<String> = regex::Regex::new(&"[0-9]+".to_string())
            .map(|re| {
                re.find(&"id=9000".to_string())
        ...
        find_ok = (found_num).map_or("None".to_string().to_string(), |__v| format!("{}", __v))
            == "9000".to_string();
        let replaced: String = regex::Regex::new(&"\\s+".to_string())
            .map(|re| {
                re.replace_all(&"hello   world".to_string(), &*"-".to_string())
                    .to_string()
            })
            .map_err(|e| RegexError {
                message: e.to_string(),
        ...
        let all_alpha: Vec<String> = regex::Regex::new(&"[a-z]+".to_string())
            .map(|re| {
                re.find_iter(&"ab 12 cd".to_string())
                    .map(|m| m.as_str().to_string())
                    .collect::<Vec<String>>()
            })
            .map_err(|e| RegexError {
        ...
        let split_parts: Vec<String> = regex::Regex::new(&":+".to_string())
            .map(|re| {
        ...
        let case_fold: Option<String> =
            search_flags(&"hello".to_string(), &"HELLO".to_string(), IGNORECASE)?;
