...
fn main() {
    let contract_payload: Vec<u8> = encode_utf8("binary-contract");
    ...
    let __sifr_try_res: Result<(), ParseError> = (|| {
        let as_hex: String = bytes_to_hex(&contract_payload)?;
        let restored: Vec<u8> = bytes_from_hex(&as_hex)?;
        let text: String = decode_utf8(&restored)?;
