...
fn main() {
    ...
    println!("Current epoch: {}", t1);
    {
        ...
        if __secs.is_finite() && (__secs > 0.0) {
            std::thread::sleep(std::time::Duration::from_nanos(
                (__secs * 1000000000.0) as u64,
            ))
        } else {
            ()
        }
    ...
    let __sifr_try_res: Result<(), RegexError> = (|| {
        let matched: bool = regex::Regex::new(&"[0-9]+".to_string())
            .map(|re| re.is_match(&"hello 42".to_string()))
            .map_err(|e| RegexError {
                message: e.to_string(),
                detail: e.to_string(),
            })?;
        println!("Match digits in \'hello 42\': {}", matched);
        let found: Option<String> = regex::Regex::new(&"[0-9]+".to_string())
            .map(|re| {
                re.find(&"price is $42.99".to_string())
        ...
        let replaced: String = regex::Regex::new(&"[0-9]+".to_string())
            .map(|re| {
                re.replace_all(&"a1b2c3".to_string(), &*"N".to_string())
                    .to_string()
            })
            .map_err(|e| RegexError {
                message: e.to_string(),
    ...
    println!(
        ...
        format!(
            "{:x}",
            (<sha2::Sha256 as sha2::Digest>::digest)(("sifr".to_string()).as_bytes())
        )
    );
    println!(
        "MD5(\'sifr\'): {}",
        format!("{:x}", md5::compute(("sifr".to_string()).as_bytes()))
    );
    println!("=== sifr.base64 ===");
    let encoded: String = base64::Engine::encode(
        &base64::engine::general_purpose::STANDARD,
        &"Hello, Sifr!".to_string().as_bytes(),
    );
