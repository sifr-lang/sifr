...
fn main() {
    ...
    assert_eq!(
        format!("Tuple index: {a}, {b}").to_string(),
        "Tuple index: 10, 20"
    );
    ...
    assert_eq!(
        format!("Division 10/2: {result}").to_string(),
        "Division 10/2: 5"
    );
    ...
    assert_eq!(
        format!("bool([1,2,3]): {}", !nums.is_empty()).to_string(),
        "bool([1,2,3]): true"
    );
    println!("bool([]): {}", !empty.is_empty());
    assert_eq!(
        format!("bool([]): {}", !empty.is_empty()).to_string(),
        "bool([]): false"
    );
    let mut base: SifrInt = SifrInt::from_i64(2);
    base = base.pow_known_valid(0_u32);
    println!("2**0 = {base}");
    assert_eq!(format!("2**0 = {base}").to_string(), "2**0 = 1");
    let _ = SifrInt::from_i64(10);
    let f: f64 = 3.5_f64;
    let mixed: f64 = 10.0 + f;
    println!("10 + 3.5 = {mixed}");
    assert_eq!(format!("10 + 3.5 = {mixed}").to_string(), "10 + 3.5 = 13.5");
