...
fn seed(seed_value: Option<i64>) {
    let mut generator: Random = Random::new(seed_value);
...
fn setstate(state: &RandomState) -> Result<(), ValueError> {
    let mut probe: Random = Random::new(Some(0 as i64));
...
fn main() {
    let mut rng: Random = Random::new(Some(77 as i64));
    let __sifr_try_res: Result<(), ValueError> = (|| {
        let state_before: RandomState = rng.getstate();
        let next_one: i64 = rng.randint(0 as i64, 100000 as i64)?;
        let _rng_set_result: Result<(), ValueError> = rng.setstate(&state_before);
        let _: Result<(), ValueError> = _rng_set_result;
        let replay_one: i64 = rng.randint(0 as i64, 100000 as i64)?;
        assert!(next_one == replay_one);
        seed(Some(1234 as i64));
        let first_module_random: f64 = random();
        let second_module_int: i64 = randint(0 as i64, 100000 as i64)?;
        let module_state: RandomState = getstate();
        let after_state_int: i64 = randint(0 as i64, 100000 as i64)?;
        let _module_set_result: Result<(), ValueError> = setstate(&module_state);
        let _: Result<(), ValueError> = _module_set_result;
        let replay_after_state_int: i64 = randint(0 as i64, 100000 as i64)?;
        assert!(after_state_int == replay_after_state_int);
        seed(Some(1234 as i64));
        assert!(first_module_random == random());
        let replay_second_module_int: i64 = randint(0 as i64, 100000 as i64)?;
    ...
    if let Err(__sifr_try_err) = __sifr_try_res {
        ...
        assert!(
            format!(
                "{}",
                format!("{}{}", "unexpected random error: ".to_string(), e.message)
            ) == "ad_hoc_rng_wave1_stateful_object_model_demo: pass".to_string()
        );
    }
    assert!(
        format!(
