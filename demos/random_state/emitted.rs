...
fn main() {
    let mut rng: __SifrStdlib_sifr_x2erandom_x2eRandom = __SifrStdlib_sifr_x2erandom_x2eRandom::new(Some(77_i64));
    let __sifr_try_res: Result<(), ValueError> = (|| {
    ...
    if let Err(__sifr_try_err) = __sifr_try_res {
        let e = __sifr_try_err.clone();
        assert!((format!("{}", format!("{}{}", "unexpected random error: ", e.message)) == "rng_random_state_object_model_demo: pass"));
    }
    assert!((format!("{}", "rng_random_state_object_model_demo: pass") == "rng_random_state_object_model_demo: pass"));
}
