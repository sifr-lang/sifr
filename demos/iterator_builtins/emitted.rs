...
fn main() {
    ...
    let mut evens: Box<dyn Iterator<Item = i64>> = Box::new(nums.iter().copied().filter(|__filter_item| {
    ...
    let mut indexed: Box<dyn Iterator<Item = (i64, i64)>> = Box::new((nums).iter().copied().enumerate().map(|__pair| ((__pair.0 as i64) + (10 as i64), __pair.1)));
