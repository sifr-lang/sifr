...
fn finditer(
    ...
    let __sifr_try_res: Result<Result<Box<dyn Iterator<Item = Match>>, RegexError>, RegexError> =
        (|| {
...
fn main() {
    let mut nums: Box<dyn Iterator<Item = i64>> =
        Box::new((vec![3 as i64, 4 as i64, 5 as i64]).into_iter());
    let via_binding: Vec<i64> = nums.collect::<Vec<_>>();
    println!("{:?}", (via_binding).iter().copied().collect::<Vec<_>>());
    let via_return: Vec<i64> =
        (adapt_to_iterable(Box::new((vec![7 as i64, 8 as i64]).into_iter())))
            .into_iter()
            .collect::<Vec<_>>();
    println!("{:?}", (via_return).iter().copied().collect::<Vec<_>>());
    let payload: Vec<u8> = vec![(65 as i64) as u8, (90 as i64) as u8];
    let mut byte_iter: Box<dyn Iterator<Item = i64>> =
        Box::new((payload).iter().map(|__byte| *__byte as i64));
    ...
    let __sifr_try_res: Result<(), RegexError> = (|| {
        let mut matches: Box<dyn Iterator<Item = Match>> =
    ...
    let base: String = format!(
        "{}{}",
        "/tmp/sifr_wave_psp_iter_fix_8_demo_".to_string(),
        format!("{}", std::process::id() as i64)
    );
    let __sifr_try_res: Result<(), IOError> = (|| {
        let _mk: String = ({
            let __cmd = format!(
                "{}{}",
                format!("{}{}", "mkdir -p ".to_string(), base),
                "/nested".to_string()
            );
        ...
        let _w1: () = std::fs::write(
            &format!("{}{}", base, "/a.txt".to_string()),
            "a".to_string().as_bytes(),
        )
        .map(|_| ())
        .map_err(__io_err)?;
        let _w2: () = std::fs::write(
            &format!("{}{}", base, "/nested/b.txt".to_string()),
        ...
        println!(
            "{}",
            islice(&entries_it.collect::<Vec<_>>(), 2 as i64, None, 1 as i64)
                .collect::<Vec<_>>()
                .len() as i64
        );
        let mut recursive_it: Box<dyn Iterator<Item = String>> =
            root.rglob(&"*.txt".to_string())?;
