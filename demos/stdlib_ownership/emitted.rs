...
fn demo_itertools() {
    ...
    let result: Vec<SifrInt> = chain(&[a.clone(), b.clone()]).collect::<Vec<_>>();
    ...
    let combined: Vec<SifrInt> = chain(&[x, y]).collect::<Vec<_>>();
...
fn demo_counter() {
    ...
    let top: Vec<(String, SifrInt)> = c.most_common(&Some(SifrInt::from_i64(1)));
