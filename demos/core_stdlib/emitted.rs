...
fn main() {
    ...
    let _w: () = std::fs::write(&"/tmp/sifr_demo.txt".to_string(), "Hello from Sifr!\nLine 2".to_string().as_bytes()).map(|_| ()).map_err(__io_err)?;
    return Ok(());
})();
    ...
    let __sifr_try_res: Result<(), IOError> = (|| {
    let content: String = std::fs::read_to_string(&"/tmp/sifr_demo.txt".to_string()).map_err(__io_err)?;
    ...
    let __sifr_try_res: Result<(), IOError> = (|| {
    let lines: Vec<String> = std::fs::read_to_string(&"/tmp/sifr_demo.txt".to_string()).map(|s| s.lines().map(|l| l.to_string()).collect::<Vec<String>>()).map_err(__io_err)?;
