...
fn collect_set_and_counter_actual() -> Vec<bool> {
    ...
    let left: HashSet<i64> = (vec![1_i64, 2_i64, 3_i64]).into_iter().collect::<std::collections::HashSet<_>>();
    let right: HashSet<i64> = (vec![3_i64, 4_i64, 5_i64]).into_iter().collect::<std::collections::HashSet<_>>();
    actual.push((left.union(&right).cloned().collect::<std::collections::HashSet<_>>().len() as i64) == (5_i64));
    actual.push((left.intersection(&right).cloned().collect::<std::collections::HashSet<_>>().len() as i64) == (1_i64));
    let counts: __SifrStdlib_sifr_x2ecollections_x2eCounter<String> = from_list(&vec!["x".to_string(), "y".to_string(), "x".to_string(), "z".to_string(), "x".to_string(), "y".to_string()]);
    actual.push(counts.get(&"x".to_string(), 0_i64) == (3_i64));
    actual.push((format!("{:?}", counts.most_common(Some(2_i64)))).as_str() == ("[(\"x\", 3), (\"y\", 2)]".to_string()).as_str());
    actual
}

fn collect_deque_actual() -> Vec<bool> {
    let mut actual: Vec<bool> = vec![];
    let mut d: __SifrStdlib_sifr_x2ecollections_x2edeque<i64> = __SifrStdlib_sifr_x2ecollections_x2edeque::new(None, Some(2_i64));
    d.append(&(10_i64));
    d.append(&(20_i64));
    d.append(&(30_i64));
    actual.push(((d.len() as i64) == (2_i64)) && (({
