...
fn median_grouped(data: &[f64], interval: f64) -> Result<f64, StatisticsError> {
    ...
    return Ok(lower + (interval * ((((n as f64) / (2.0 as f64)) - (cf as f64)) / (f as f64))));
}
...
fn main() {
    ...
    let __sifr_try_res: Result<(), StatisticsError> = (|| {
        let grouped_value: f64 = median_grouped(
            &vec![1.0 as f64, 2.0 as f64, 2.0 as f64, 3.0 as f64, 4.0 as f64],
            1.0 as f64,
        )?;
    ...
    let mut sentence_wrapper: TextWrapper = TextWrapper::new(
        40 as i64,
        "".to_string(),
        "".to_string(),
        true,
