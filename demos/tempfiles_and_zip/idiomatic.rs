...
fn main() {
    ...
    let __sifr_try_res: Result<(), IOError> = (|| {
        ...
        tempfile_ok = ((std::path::Path::new(&temp_file).exists())
            && (std::path::Path::new(&temp_dir).exists()));
        let _w: () = std::fs::write(&temp_file, "payload".to_string().as_bytes())
            .map(|_| ())
            .map_err(__io_err)?;
        zip_path = format!("{}{}", temp_file, ".zip".to_string());
        let mut archive: ZipFile = ZipFile::new(
    ...
    let __sifr_try_res: Result<(), IOError> = (|| {
        if (((zip_path.chars().count() as i64) > (0 as i64))
        ...
        if (((temp_file.chars().count() as i64) > (0 as i64))
            && (std::path::Path::new(&temp_file).exists()))
        {
            let _rm_file: () = std::fs::remove_file(&temp_file)
                .map(|_| ())
                .map_err(__io_err)?;
        }
        if (((temp_dir.chars().count() as i64) > (0 as i64))
            && (std::path::Path::new(&temp_dir).exists()))
        {
            let _rm_dir: () = std::fs::remove_dir(&temp_dir)
                .map(|_| ())
                .map_err(__io_err)?;
        }
        cleanup_ok = (((((temp_file.chars().count() as i64) == (0 as i64))
            || (!(std::path::Path::new(&temp_file).exists())))
            && (((temp_dir.chars().count() as i64) == (0 as i64))
                || (!(std::path::Path::new(&temp_dir).exists()))))
            && (((zip_path.chars().count() as i64) == (0 as i64))
                || (!(std::path::Path::new(&zip_path).exists()))));
        return Ok(());
    })();
    if let Err(__sifr_try_err) = __sifr_try_res {
