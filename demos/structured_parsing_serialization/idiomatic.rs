...
fn _append_object_item(mut value: JsonValue, key: String, item_value: JsonValue) -> JsonValue {
    value.object_items.push((key, item_value));
    return value;
}
fn from_array(items: &Vec<JsonValue>) -> JsonValue {
    let mut value: JsonValue = JsonValue::new("array".to_string(), None, None, None, None);
...
fn from_object(items: &Vec<(String, JsonValue)>) -> JsonValue {
    let mut value: JsonValue = JsonValue::new("object".to_string(), None, None, None, None);
...
fn load(path: &String) -> Result<JsonValue, Error> {
    let content_result: Result<String, IOError> = std::fs::read_to_string(&path).map_err(__io_err);
...
fn main() {
    ...
    let __sifr_try_res: Result<(), IOError> = (|| {
        ...
        let mut json_value: JsonValue =
            (load(&json_path)).map_err(|__e| IOError::new(__e.to_string()))?;
    ...
    println!(
        "{}",
        dumps(&from_object(&vec![
            ("name".to_string(), from_str(&"sifr".to_string())),
            (
                "items".to_string(),
                from_array(&vec![from_int(1 as i64), from_bool(true)])
            )
        ]))
    );
    ...
    println!("{}", quoted);
    let mut dict_reader: DictReader = DictReader::new(
        "name,age\nalice,30\n".to_string(),
        None,
        "".to_string(),
        "".to_string(),
    ...
    let mut dict_writer: DictWriter = DictWriter::new(
