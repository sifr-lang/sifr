...
fn main() {
    let counts: __SifrStdlib_sifr_x2ecollections_x2eCounter<String> = from_list(&vec!["delta".to_string(), "alpha".to_string(), "delta".to_string(), "beta".to_string()]);
    println!("{:?}", counts.most_common(None));
    let mut queue: __SifrStdlib_sifr_x2ecollections_x2edeque<i64> = __SifrStdlib_sifr_x2ecollections_x2edeque::new(Some(vec![1_i64, 2_i64, 3_i64]), Some(4_i64));
    queue.rotate(1_i64);
    ...
    heapify(&mut heap);
    println!("{}", (heappushpop(&mut heap, &(2_i64))).map_or("None".to_string().to_string(), |__v| format!("{}", __v)));
    println!("{}", (heapreplace(&mut heap, 4_i64)).map_or("None".to_string().to_string(), |__v| format!("{}", __v)));
}
