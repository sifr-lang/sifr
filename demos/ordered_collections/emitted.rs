...
fn main() {
    let mut counts = from_list(&vec!["delta".to_string(), "alpha".to_string(), "delta".to_string(), "beta".to_string()]);
    ...
    heapify(&mut heap);
    println!("{}", (heappushpop(&mut heap, &(2 as i64))).map_or("None".to_string().to_string(), |__v| format!("{}", __v)));
    println!("{}", (heapreplace(&mut heap, 4 as i64)).map_or("None".to_string().to_string(), |__v| format!("{}", __v)));
}
