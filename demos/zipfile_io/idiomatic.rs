...
impl NamedTemporaryFile {
    fn new(mode: String, delete: bool, prefix: String) -> Self {
        ...
        let _created_result: Result<(), IOError> =
            std::fs::write(&candidate, "".to_string().as_bytes())
                .map(|_| ())
                .map_err(__io_err);
        return Self {
...
fn main() {
    ...
    let __sifr_try_res: Result<(), IOError> = (|| {
        let mut temp_file: NamedTemporaryFile = NamedTemporaryFile::new(
            "wb".to_string(),
            false,
            "sifr_runtime_wave2_demo_".to_string(),
        );
        ...
        if std::path::Path::new(&zip_path).exists() {
            let _rm_old: () = std::fs::remove_file(&zip_path)
                .map(|_| ())
                .map_err(__io_err)?;
        }
        let mut writer: ZipFile = ZipFile::new(
            format!("{}{}", zip_path, "".to_string()),
            "w".to_string(),
            ZIP_STORED,
        );
        let _create: () = writer.create()?;
        let _write_text: () =
            writer.write(&"note.txt".to_string(), &"runtime-wave2".to_string())?;
        let _write_bytes: () = writer.write_bytes(
            &"bin/raw.bin".to_string(),
            &vec![(0 as i64) as u8, (1 as i64) as u8, (2 as i64) as u8],
        )?;
        let mut reader: ZipFile = ZipFile::new(
            format!("{}{}", zip_path, "".to_string()),
            "r".to_string(),
            ZIP_STORED,
        );
        let payload: Vec<u8> = reader.read_bytes(&"bin/raw.bin".to_string())?;
        let mut handle: ZipReadHandle = ZipReadHandle::new(vec![
            (97 as i64) as u8,
            (98 as i64) as u8,
            (99 as i64) as u8,
        ]);
        let read_all: Vec<u8> = handle.read_bytes(Some(-(1 as i64)))?;
        let handle_negative_ok: bool =
            read_all == vec![(97 as i64) as u8, (98 as i64) as u8, (99 as i64) as u8];
        let _open_handle_result: Result<ZipReadHandle, IOError> =
        ...
        let mut bad_mode_writer: ZipFile = ZipFile::new(
            format!("{}{}", zip_path, "".to_string()),
            "rw".to_string(),
            ZIP_STORED,
        );
        let _bad_mode_write_result: Result<(), IOError> =
        ...
        demo_ok = ((((((tempfile_ok) && (is_zipfile(&zip_path)))
            && (payload == vec![(0 as i64) as u8, (1 as i64) as u8, (2 as i64) as u8]))
            && (handle_negative_ok))
            && (open_rejected))
            && (bad_mode_rejected));
        return Ok(());
    ...
    let __sifr_try_res: Result<(), IOError> = (|| {
        if std::path::Path::new(&zip_path).exists() {
            let _rm_zip: () = std::fs::remove_file(&zip_path)
                .map(|_| ())
                .map_err(__io_err)?;
        }
        return Ok(());
