...
fn main() {
    let mut chained: Box<dyn Iterator<Item = i64>> =
        chain(&vec![vec![1 as i64, 2 as i64], vec![3 as i64]]);
    ...
    let mut sliced: Box<dyn Iterator<Item = i64>> = islice(
    ...
    println!(
        "{}",
        (counter.next()).map_or("None".to_string().to_string(), |__v| format!("{}", __v))
    );
    println!(
        "{}",
        (counter.next()).map_or("None".to_string().to_string(), |__v| format!("{}", __v))
