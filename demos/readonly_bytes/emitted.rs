...
fn main() {
    let payload: Vec<u8> = vec![(102 as i64) as u8, (102 as i64) as u8, (105 as i64) as u8, (45 as i64) as u8, (114 as i64) as u8, (101 as i64) as u8, (97 as i64) as u8, (100 as i64) as u8, (121 as i64) as u8];
    let second: Option<i64> = payload.get((1 as i64) as usize).map(|__byte| *__byte as i64);
    let mut second_ok: bool = false;
    if let Some(second) = second {
        second_ok = second == (102 as i64);
    }
    let iter_ok: bool = sum_bytes(&payload) > (0 as i64);
    let contains_ok: bool = (({
    let __needle = 102 as i64;
    ...
    reader.close();
    io_ok = ((loaded == payload) && (format!("{:?}", loaded.iter().map(|__byte| *__byte as i64).collect::<Vec<i64>>()) == "[102, 102, 105, 45, 114, 101, 97, 100, 121]".to_string()));
    return Ok(());
})();
