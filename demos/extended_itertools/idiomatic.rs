...
fn _combinations_impl<T: Clone + std::fmt::Display + PartialOrd + 'static>(
    ...
    while i <= ((data.len() as i64) - r) {
        ...
        if let Some(current) = current {
            let tails: Vec<Vec<T>> = _combinations_impl(data, i + (1 as i64), r - (1 as i64));
            for entry in _prepend(current, &tails).into_iter() {
                result.push(entry);
            }
        }
...
fn _combinations_with_replacement_impl<T: Clone + std::fmt::Display + PartialOrd + 'static>(
    data: &Vec<T>,
    start: i64,
    r: i64,
) -> Vec<Vec<T>> {
    ...
    while i < (data.len() as i64) {
        let current: Option<T> = Some(data[i as usize].clone());
        if let Some(current) = current {
            let tails: Vec<Vec<T>> = _combinations_with_replacement_impl(data, i, r - (1 as i64));
            for entry in _prepend(current, &tails).into_iter() {
                result.push(entry);
            }
        }
        ...
        if !__sifr_generator_initialized {
            ...
            if repeat >= (0 as i64) {
                ...
                for iterable in iterables.iter().cloned() {
                    base_pools.push(_collect_iterable(
            ...
            if r >= (0 as i64) {
                if r == (0 as i64) {
                    ...
                    if (materialized.len() as i64) > (0 as i64) {
                        result = _combinations_with_replacement_impl(&materialized, 0 as i64, r);
                    }
                }
            }
            let mut i: i64 = 0 as i64;
...
fn main() {
    let mut acc_it: Box<dyn Iterator<Item = i64>> = accumulate(
        &(vec![1 as i64, 2 as i64, 3 as i64, 4 as i64])
            .into_iter()
            .collect::<Vec<_>>(),
        None,
    );
    ...
    assert!(
        format!(
            "{:?}",
            dropwhile(
                |__arg0| lt3((__arg0).clone()),
                &(vec![1 as i64, 2 as i64, 3 as i64, 1 as i64])
                    .into_iter()
                    .collect::<Vec<_>>()
            )
            .collect::<Vec<_>>()
    ...
    assert!(
        format!(
            "{:?}",
            zip_longest(
                &(vec![1 as i64, 2 as i64]).into_iter().collect::<Vec<_>>(),
                &(vec![9 as i64]).into_iter().collect::<Vec<_>>(),
                &(0 as i64)
            )
            .collect::<Vec<_>>()
        ) == "[[1, 9], [2, 0]]".to_string()
    );
    let mut cyc: Box<dyn Iterator<Item = i64>> = cycle(
    ...
    assert!(
        format!(
            "{:?}",
            starmap(
                |__arg0, __arg1| add2((__arg0).clone(), (__arg1).clone()),
                &(vec![(2 as i64, 3 as i64), (4 as i64, 5 as i64)])
                    .into_iter()
                    .collect::<Vec<_>>()
            )
            .collect::<Vec<_>>()
    ...
    assert!(
        format!(
            "{:?}",
            product(&vec![vec![1 as i64, 2 as i64]], 2 as i64).collect::<Vec<_>>()
        ) == "[[1, 1], [1, 2], [2, 1], [2, 2]]".to_string()
    );
    assert!(
        format!(
            "{:?}",
            product(&vec![vec![1 as i64, 2 as i64]], -(1 as i64)).collect::<Vec<_>>()
        ) == "[]".to_string()
    );
    assert!(
        format!(
            ...
            .collect::<Vec<_>>()
        ) == "[[1, 2], [1, 3], [2, 1], [2, 3], [3, 1], [3, 2]]".to_string()
    );
    assert!(
        format!(
            "{:?}",
