...
impl ArgumentParser {
    ...
    fn add_parser(&mut self, name: &String, parser: &ArgumentParser) {
        ...
        self._subparsers
            .push((format!("{}{}", name, "".to_string()), specs_copy));
    }
    fn _append_spec(
        &mut self,
        name: &String,
    ...
    fn add_argument(&mut self, name: &String, dest: &String, action: &String, default: &String) {
    ...
    fn add_argument_typed(
        ...
        if (((normalized_type != "int".to_string()) && (normalized_type != "float".to_string()))
    ...
    fn _coerce_token(&self, spec: &ArgumentSpec, token: &String) -> Option<String> {
        if spec.type_name == "int".to_string() {
            let __sifr_try_res: Result<Option<String>, ParseError> = (|| {
                let parsed_int: i64 = (token).parse::<i64>().map_err(|e| ParseError {
                    message: e.to_string(),
                })?;
    ...
    fn _collect_option_values(
        ...
        if spec.nargs == "?".to_string() {
            ...
            if ((!(force_positional)) && (_is_option_like_token(&self._specs.clone(), &token_one)))
            {
                return (values, i);
            }
        ...
        if _is_digit_string(&spec.nargs) {
            let __sifr_try_res: Result<(), ParseError> = (|| {
                let parsed_count: i64 = (spec.nargs).parse::<i64>().map_err(|e| ParseError {
                    message: e.to_string(),
                })?;
    ...
    fn parse_args(&mut self, args: &Vec<String>) -> Namespace {
            ...
            && ((args.len() as i64) > (0 as i64)))
        {
            ...
            if let Some(first_token) = first_token {
                ...
                let subparser_specs: Option<Vec<ArgumentSpec>> =
                ...
                if let Some(subparser_specs) = subparser_specs {
                    ...
                    let mut subparser: ArgumentParser = ArgumentParser::new(command_name);
                    subparser._specs = subparser_specs;
                    let child_ns: Namespace = subparser.parse_args(&Vec::from_iter(
                        (args)
                            .iter()
                            .skip((1 as i64).max(0) as usize)
                            .take(((args.len() as i64).max(0) - (1 as i64).max(0)).max(0) as usize)
                            .cloned(),
                    ));
                ...
                for option_spec in self._specs.clone().iter().cloned() {
                        ...
                        || (option_spec.nargs == "+".to_string()))
                    {
                        ...
                        for raw in values.iter().cloned() {
                            let coerced: Option<String> = self._coerce_token(&option_spec, &raw);
                            if coerced.is_none() {
                                continue;
                            }
                        ...
                        if (values.len() as i64) > (0 as i64) {
                            let first_value: Option<String> =
                                Some(values[(0 as i64) as usize].clone());
                            if let Some(first_value) = first_value {
                                let token_value: String = _copy_token(&Some(first_value));
                                let coerced_first: Option<String> =
                                    self._coerce_token(&option_spec, &token_value);
                                if let Some(coerced_first) = coerced_first {
                                    let coerced_value: String = _copy_token(&Some(coerced_first));
                ...
                if let Some(positional_spec) = positional_spec {
                        ...
                        || (positional_spec.nargs == "+".to_string()))
                    {
                        ...
                        for raw2 in values2.iter().cloned() {
                            let coerced2: Option<String> =
                                self._coerce_token(&positional_spec, &raw2);
                            if coerced2.is_none() {
                                continue;
                            }
                        ...
                        if (values2.len() as i64) > (0 as i64) {
                            let first_value2: Option<String> =
                                Some(values2[(0 as i64) as usize].clone());
                            if let Some(first_value2) = first_value2 {
                                let token_value2: String = _copy_token(&Some(first_value2));
                                let coerced_first2: Option<String> =
                                    self._coerce_token(&positional_spec, &token_value2);
                                if let Some(coerced_first2) = coerced_first2 {
                                    let coerced_value2: String = _copy_token(&Some(coerced_first2));
...
fn _normalize_nargs(nargs: &String) -> String {
    ...
    if _is_digit_string(nargs) {
        let __sifr_try_res: Result<Option<String>, ParseError> = (|| {
            let parsed: i64 = (nargs).parse::<i64>().map_err(|e| ParseError {
                message: e.to_string(),
            })?;
            if parsed > (0 as i64) {
...
fn _nargs_is_multi(nargs: &String) -> bool {
    ...
    if _is_digit_string(&normalized) {
        let __sifr_try_res: Result<bool, ParseError> = (|| {
            let parsed: i64 = (normalized).parse::<i64>().map_err(|e| ParseError {
                message: e.to_string(),
            })?;
                ...
                if _in_ipv4_range(value, 2851995648 as i64, 2852061183 as i64) {
                    ...
                    if _in_ipv4_range(value, 2886729728 as i64, 2887778303 as i64) {
                        ...
                        if _in_ipv4_range(value, 3221225472 as i64, 3221225727 as i64) {
                            ...
                            if _in_ipv4_range(value, 3221225642 as i64, 3221225643 as i64) {
                                private_hit = true;
                            } else {
                                if _in_ipv4_range(value, 3221225984 as i64, 3221226239 as i64) {
                                    private_hit = true;
                                } else {
                                    if _in_ipv4_range(value, 3232235520 as i64, 3232301055 as i64) {
                                        private_hit = true;
                                    } else {
                                        if _in_ipv4_range(
                                            value,
...
fn _canonical_uuid_text(input_text: &String) -> Result<String, ValueError> {
    ...
    while j < (hex_only.chars().count() as i64) {
        if (((j == (8 as i64)) || (j == (12 as i64))) || (j == (16 as i64))) || (j == (20 as i64)) {
...
fn main() {
    ...
    let mut parsed: Namespace = parser.parse_args(&vec![
        "--strict".to_string(),
        "--mode".to_string(),
        "parity".to_string(),
        "main.sifr".to_string(),
    ...
    println!(
        ...
        format!(
            "{}{}",
            "argparse.strict = ".to_string(),
            format!("{}", parsed.get_bool(&"strict".to_string(), false))
        )
    );
    println!(
        "{}",
        format!(
            "{}{}",
            "argparse.mode = ".to_string(),
            parsed.get(&"mode".to_string(), &"".to_string())
        )
    );
    println!(
        "{}",
        format!(
            "{}{}",
            "argparse.entry = ".to_string(),
            parsed.get(&"entry".to_string(), &"".to_string())
        )
    );
    let mut parsed_inline: Namespace = parser.parse_args(&vec![
    ...
    println!(
        ...
        format!(
            "{}{}",
            "argparse.inline = ".to_string(),
            parsed_inline.get(&"mode".to_string(), &"".to_string())
        )
    );
    println!(
        "{}",
        format!(
            "{}{}",
            "argparse.literal = ".to_string(),
            parsed_inline.get(&"entry".to_string(), &"".to_string())
        )
    );
    let mut parsed_missing: Namespace = parser.parse_args(&vec![
        "--mode".to_string(),
        "--strict".to_string(),
    ...
    println!(
        ...
        format!(
            "{}{}",
            "argparse.missing_mode = ".to_string(),
            parsed_missing.get(&"mode".to_string(), &"".to_string())
        )
    );
    println!(
        "{}",
        format!(
            "{}{}",
            "argparse.missing_strict = ".to_string(),
            format!("{}", parsed_missing.get_bool(&"strict".to_string(), false))
        )
    );
    let __sifr_try_res: Result<(), AddressValueError> = (|| {
        ...
        println!(
            ...
            format!(
                "{}{}{}{}",
                "ipaddress.link_local = ".to_string(),
                format!("{}", link_local.is_link_local()),
                " global=".to_string(),
                format!("{}", link_local.is_global())
            )
        );
        ...
        println!(
            ...
            format!(
                "{}{}",
                "ipaddress.multicast_global = ".to_string(),
                format!("{}", multicast.is_global())
            )
        );
        return Ok(());
    })();
    if let Err(__sifr_try_err) = __sifr_try_res {
        ...
        println!(
            "{}",
            format!("{}{}", "ipaddress.error = ".to_string(), e.message)
        );
    }
    ...
    if let Err(__sifr_try_err) = __sifr_try_res {
        ...
        println!(
            "{}",
            format!("{}{}", "uuid.error = ".to_string(), e.message)
        );
    }
    ...
    if let Err(__sifr_try_err) = __sifr_try_res {
        ...
        println!(
            "{}",
            format!("{}{}", "graphlib.error = ".to_string(), e.message)
        );
    }
}
