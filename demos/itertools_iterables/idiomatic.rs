...
fn main() {
    ...
    println!(
        "{:?}",
        islice(
            &Box::new((nums).iter().copied()).collect::<Vec<_>>(),
            1 as i64,
            Some(4 as i64),
            2 as i64
        )
    ...
    println!(
        "{:?}",
        accumulate(&Box::new((nums).iter().copied()).collect::<Vec<_>>(), None).collect::<Vec<_>>()
    );
    println!(
        "{:?}",
        compress(
            &Box::new((nums).iter().copied()).collect::<Vec<_>>(),
            &Box::new((vec![true, false, true, false]).into_iter()).collect::<Vec<_>>()
        )
        .collect::<Vec<_>>()
    );
    println!(
    ...
    let base: String = format!(
        "{}{}",
        "/tmp/sifr_wave_psp_iter_fix_6_demo_".to_string(),
    ...
    let __sifr_try_res: Result<(), IOError> = (|| {
        ...
        let _w: () = std::fs::write(
            ...
            "demo".to_string().as_bytes(),
        )
        .map(|_| ())
        .map_err(__io_err)?;
        let _w2: () = std::fs::write(
            &format!("{}{}", base, "/b.txt".to_string()),
            "demo".to_string().as_bytes(),
        )
        .map(|_| ())
        .map_err(__io_err)?;
        ...
        println!(
            "{}",
            islice(&entries_it.collect::<Vec<_>>(), 1 as i64, None, 1 as i64)
                .collect::<Vec<_>>()
                .len() as i64
        );
        return Ok(());
    ...
    if let Err(__sifr_try_err) = __sifr_try_res {
        ...
        println!(
            "{}",
            format!("{}{}", "ioerror: ".to_string(), format!("{}", e.message))
        );
    }
}
