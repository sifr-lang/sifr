...
fn main() {
    ...
    println!(
        ...
        ({
            let __items = vec![1 as i64, 2 as i64, 2 as i64, 3 as i64, 3 as i64];
    ...
    let mut fruit_counter = from_list(&fruits);
    println!(
        "apple count: {}",
        fruit_counter.get(&"apple".to_string(), 0 as i64)
    );
    println!(
        "banana count: {}",
        fruit_counter.get(&"banana".to_string(), 0 as i64)
    );
    println!(
        "cherry count: {}",
        fruit_counter.get(&"cherry".to_string(), 0 as i64)
    );
    println!("=== Bytes ===");
    println!(
        "\'hello\' encoded: {} bytes",
        ({
            let __s = "hello".to_string();
            __s.as_bytes().to_vec()
        })
        .len() as i64
    );
    let __sifr_try_res: Result<(), ParseError> = (|| {
        let roundtrip: String = String::from_utf8(
            ({
                let __s = "Sifr".to_string();
                __s.as_bytes().to_vec()
            })
            .iter()
            .copied()
    ...
    let __sifr_try_res: Result<(), ParseError> = (|| {
        ...
        let decoded: String = String::from_utf8(hex_bytes.iter().copied().collect::<Vec<u8>>())
            .map_err(|e| ParseError {
                message: e.to_string(),
            })?;
