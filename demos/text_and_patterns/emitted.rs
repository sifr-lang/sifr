    ...
    while i < (template.chars().count() as i64) {
        ...
        if next_value == "{".to_string() {
            ...
            if j >= (template.chars().count() as i64) {
                if safe {
                    result = format!(
                        "{}{}", result, String::from_iter((template).chars().skip((i)
                        .max(0) as usize))
                    );
                    return Ok(result);
                }
            ...
            if ((!(has_first)) || (!(_is_identifier_start(&first_value)))) {
                ...
                return Err(
                    ValueError::new(
                        format!(
                            "{}{}", "invalid template placeholder: ".to_string(), name
                        ),
                    ),
                );
            }
            ...
            if mapped_value.is_none() {
                ...
                return Err(
                    ValueError::new(
                        format!(
                            "{}{}", "missing template value for key: ".to_string(), name
                        ),
                    ),
                );
            } else {
        ...
        if mapped_value2.is_none() {
            ...
            return Err(
                ValueError::new(
                    format!(
                        "{}{}", "missing template value for key: ".to_string(), name2
                    ),
                ),
            );
        } else {
    ...
    while i < (format_string.chars().count() as i64) {
        ...
        if ch_value == "{".to_string() {
            ...
            while j < (format_string.chars().count() as i64) {
                let part: Option<String> = Some({
                    let Some(__indexed_char) = format_string.chars().nth(j as usize)
                    else {
                        unreachable!(
                            "compiler-verified string index should be in range"
                        );
...
fn main() {
    ...
    let mut wrapper: TextWrapper = TextWrapper::new(8 as i64, "> ".to_string(), ".. ".to_string(), true, 8 as i64, true, true, true, false, None, " [...]".to_string());
    ...
    let __sifr_try_res: Result<(), ValueError> = (|| {
    let month_label: String = TextCalendar::new(0 as i64).formatmonthname(2024 as i64, 2 as i64, 0 as i64)?;
