...
fn main() {
    ...
    if let Err(__sifr_try_err) = __sifr_try_res {
        ...
        println!(
            "{}",
            format!("{}{}", "unexpected ValueError: ".to_string(), e.message)
        );
    }
    assert!(size_ok);
    ...
    let __sifr_try_res: Result<(), ValueError> = (|| {
        ...
        assert!(
            from_list
                .get((0 as i64) as usize)
                .map(|__byte| *__byte as i64)
                == Some(83 as i64)
        );
        assert!(
            from_list
                .get((3 as i64) as usize)
                .map(|__byte| *__byte as i64)
                == Some(114 as i64)
        );
        from_ints_ok = true;
    ...
    let __sifr_try_res: Result<(), ParseError> = (|| {
        ...
        let from_hex_text: String =
            String::from_utf8(from_hex.iter().copied().collect::<Vec<u8>>()).map_err(|e| {
                ParseError {
                    message: e.to_string(),
                }
            })?;
    ...
    if let Err(__sifr_try_err) = __sifr_try_res {
        ...
        println!(
            "{}",
            format!("{}{}", "unexpected ParseError: ".to_string(), e.message)
        );
    }
    assert!(from_hex_ok);
        ...
        let decoded: String = ({
            ...
            if (__encoding_lower != "utf-8".to_string()) && (__encoding_lower != "utf8".to_string())
            {
                Err(ParseError {
                    message: format!(
                        "{} currently supports only UTF-8 encoding, got {}",
                        "bytes.decode()".to_string(),
                        __encoding
                    ),
                })
            } else {
                String::from_utf8(encoded.iter().copied().collect::<Vec<u8>>()).map_err(|e| {
                    ParseError {
                        message: e.to_string(),
                    }
                })
