...
fn main() {
    ...
    let constants_ok: bool = ((PIPE == -(1 as i64)) && (STDOUT == -(2 as i64))) && (DEVNULL == -(3 as i64));
    demo_ok = (((((cp.returncode == (0 as i64)) && (cp.stdout.trim().to_string() == "runtime_wave4_demo".to_string())) && (rc == (0 as i64))) && (out.trim().to_string() == "runtime_wave4_demo_output".to_string())) && (constants_ok));
    return Ok(());
})();
    if let Err(__sifr_try_err) = __sifr_try_res {
        let e = __sifr_try_err.clone();
