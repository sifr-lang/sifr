...
fn main() {
    ...
    assert!(isclose(1.0 as f64, 1.0000001 as f64, 0.001 as f64, 0.0 as f64));
    println!("math: OK");
    ...
    let match_result: bool = regex::Regex::new(&"hello".to_string()).map(|re| re.is_match(&"hello world".to_string())).map_err(|e| RegexError { message: e.to_string(), detail: e.to_string() })?;
    assert!(match_result);
    let no_match: bool = regex::Regex::new(&"xyz".to_string()).map(|re| re.is_match(&"hello".to_string())).map_err(|e| RegexError { message: e.to_string(), detail: e.to_string() })?;
    {
    let __cond = no_match;
    assert!(!__cond)
};
    ...
    assert_eq!(bisect_left(&sorted_list, &(5 as i64), 0 as i64, None), 2 as i64);
    assert_eq!(bisect_right(&sorted_list, &(5 as i64), 0 as i64, None), 3 as i64);
    println!("bisect: OK");
    ...
    assert_eq!(serde_json::to_string(&"hello".to_string()).unwrap_or_default(), "\"hello\"");
