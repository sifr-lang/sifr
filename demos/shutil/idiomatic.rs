...
fn collect_copy_move_tree_actual() -> Vec<bool> {
    ...
    let __sifr_try_res: Result<(), IOError> = (|| {
        let _mk: () = std::fs::create_dir_all(&base)
            .map(|_| ())
            .map_err(__io_err)?;
        let _w: () = std::fs::write(&src, "demo".to_string().as_bytes())
            .map(|_| ())
            .map_err(__io_err)?;
        ...
        copy_ok = (((std::path::Path::new(&src).exists())
            && (std::path::Path::new(&copied).exists()))
            && (copied_content_ok));
        let _mv: () = move_file(&copied, &moved)?;
        move_ok = ((std::path::Path::new(&moved).exists())
            && (!(std::path::Path::new(&copied).exists())));
        let _mk_tree: () = std::fs::create_dir_all(&tree)
            .map(|_| ())
            .map_err(__io_err)?;
        let _w_nested: () = std::fs::write(&nested, "nested".to_string().as_bytes())
            .map(|_| ())
            .map_err(__io_err)?;
...
fn collect_tooling_and_cleanup_actual() -> Vec<bool> {
    ...
    let tool: Option<String> = std::env::var("PATH".to_string()).ok().and_then(|__path| {
        __path
            .split(':')
            .map(|d| std::path::Path::new(d).join(&"sh".to_string()))
            .find(|p| p.is_file())
    ...
    let __sifr_try_res: Result<(), IOError> = (|| {
        let _bad: () = copy(
            &format!("{}{}", base, "/missing_src.txt".to_string()),
            &format!("{}{}", base, "/missing_dst.txt".to_string()),
        )?;
        return Ok(());
    })();
    if let Err(__sifr_try_err) = __sifr_try_res {
