...
fn write_text(path: &String, data: &String) -> Result<(), IOError> {
...
fn main() {
    ...
    let __sifr_try_res: Result<(), IOError> = (|| {
        let _: () = write_text(&path, &"m14 preamble".to_string())?;
        ...
        assert!(
            format!("{}", format!("{}{}", "file = ".to_string(), text))
                == "file = m14 preamble".to_string()
        );
        return Ok(());
    })();
    if let Err(__sifr_try_err) = __sifr_try_res {
        ...
        assert!(
            format!("{}", format!("{}{}", "ioerror = ".to_string(), e.message))
                == "preamble demo complete".to_string()
        );
    }
