...
fn main() {
    let mut matrix: Vec<Vec<i64>> = vec![
        vec![0 as i64, 0 as i64, 0 as i64],
        vec![0 as i64, 0 as i64, 0 as i64],
        vec![0 as i64, 0 as i64, 0 as i64],
    ];
    {
        let __oi_raw = 0 as i64;
        let __oi_norm = if __oi_raw < 0 {
            (matrix.len() as i64) + __oi_raw
        } else {
            __oi_raw
        };
        if __oi_norm >= 0 {
            if let Some(__row) = matrix.get_mut(__oi_norm as usize) {
                ...
                let __ii_norm = if __ii_raw < 0 {
                    (__row.len() as i64) + __ii_raw
                } else {
                    __ii_raw
                };
                if __ii_norm >= 0 {
