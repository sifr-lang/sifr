...
fn main() {
    ...
    let content: String = std::fs::read_to_string(&"nonexistent_file.txt".to_string()).map_err(__io_err)?;
    ...
    let matched: bool = regex::Regex::new(&"[invalid regex".to_string()).map(|re| re.is_match(&"test".to_string())).map_err(|e| RegexError { message: e.to_string(), detail: e.to_string() })?;
