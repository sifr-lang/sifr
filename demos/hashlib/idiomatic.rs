...
fn _build_hash(algorithm: &String, data: &Vec<u8>) -> HashObject {
    ...
    if alg == "md5".to_string() {
        return HashObject::new(alg, (data).clone(), "md5".to_string(), 16 as i64, 64 as i64);
    } else {
                ...
                if algorithm.clone() == "sha256".to_string() {
                    ...
                    if algorithm.clone() == "sha384".to_string() {
                        ...
                        if algorithm.clone() == "sha512".to_string() {
                            return (<sha2::Sha512 as sha2::Digest>::digest)((data)).to_vec();
                        } else {
                            if algorithm.clone() == "blake2b".to_string() {
                                return (<blake2::Blake2b512 as blake2::Digest>::digest)((data))
                                    .to_vec();
                            } else {
...
fn file_digest(path: &String, name: &String) -> Result<String, HashlibError> {
    let __sifr_try_res: Result<Result<String, HashlibError>, IOError> = (|| {
        ...
        let mut h: HashObject = (new(name, &data)).map_err(|__e| IOError::new(__e.to_string()))?;
...
fn collect_positive_actual(tmp_path: &String) -> Vec<String> {
    ...
    actual.push(format!(
        ...
        (h.hexdigest()).as_str()
            == (format!(
                "{:x}",
                (<sha2::Sha256 as sha2::Digest>::digest)(("abc".to_string()).as_bytes())
            ))
            .as_str()
    ));
    actual.push(format!(
        "{}",
        ((h.digest().len() as i64) == (32 as i64)) && (h.digest_bytes() == h.digest())
    ));
    ...
    actual.push(format!(
        ...
        (c.hexdigest()).as_str()
            == (format!(
                "{:x}",
                (<sha2::Sha256 as sha2::Digest>::digest)(("abcx".to_string()).as_bytes())
            ))
            .as_str()
    ));
    ...
    actual.push(format!(
        "{}",
        (m.hexdigest()).as_str()
            == (format!("{:x}", md5::compute(("hello".to_string()).as_bytes()))).as_str()
    ));
    actual.push(format!(
        "{}",
        contains(&algorithms_guaranteed(), &"sha256".to_string())
    ));
...
fn main() {
    ...
    let _: Result<(), IOError> = std::fs::write(&tmp_path, "file-data".to_string().as_bytes())
        .map(|_| ())
        .map_err(__io_err);
