...
fn b64encode_bytes(data: &Vec<u8>) -> Vec<u8> {
    return base64::Engine::encode(&base64::engine::general_purpose::STANDARD, &data).into_bytes();
}
fn b64decode_bytes(data: &Vec<u8>) -> Result<Vec<u8>, ParseError> {
    return base64::Engine::decode(&base64::engine::general_purpose::STANDARD, &data).map_err(
        |e| ParseError {
            message: e.to_string(),
        },
    );
...
fn main() {
    let __sifr_try_res: Result<(), ValueError> = (|| {
        ...
        let dec: Vec<u8> =
            (b64decode_bytes(&enc)).map_err(|__e| ValueError::new(__e.to_string()))?;
        assert!(dec == data);
        return Ok(());
    })();
    if let Err(__sifr_try_err) = __sifr_try_res {
        ...
        assert!(
            format!(
                "{}",
                format!("{}{}", "unexpected value error: ".to_string(), e.message)
            ) == "ad_hoc_rng_wave2_hashlib_base64_bytes_demo: pass".to_string()
        );
    }
    assert!(
        format!(
