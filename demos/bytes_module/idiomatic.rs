...
fn bytes_to_hex(payload: &[u8]) -> Result<String, ParseError> {
    Ok(payload
        .iter()
...
fn bytes_from_hex(s: &str) -> Result<Vec<u8>, ParseError> {
    return {
...
fn collect_primary_actual(payload: &Vec<u8>) -> Vec<String> {
    ...
    actual.push(format!("{}", starts_with(payload, &encode_utf8("bytes"))));
    actual.push(format!("{}", ends_with(payload, &encode_utf8("e30"))));
    return actual;
}

fn bytes_to_hex_or_empty(payload: &[u8]) -> String {
    let __sifr_try_res: Result<String, ParseError> = (|| {
        let hx: String = bytes_to_hex(payload)?;
        return Ok(hx);
...
fn bytes_from_hex_to_text_or_empty(payload: &str) -> String {
    let __sifr_try_res: Result<String, ParseError> = (|| {
        let parsed: Vec<u8> = bytes_from_hex(payload)?;
        let txt: String = decode_utf8(&parsed)?;
        return Ok(txt);
        unreachable!("sifr try/except return capture fell through");
    })();
    match __sifr_try_res {
...
fn collect_invalid_actual_ok() -> Vec<bool> {
    ...
    let __sifr_try_res: Result<(), ParseError> = (|| {
        let bad_utf8: String = String::from_utf8(
...
fn main() {
    let payload: Vec<u8> = encode_utf8("bytes-phase30");
    ...
    let hex_text: String = bytes_to_hex_or_empty(&encode_utf8("Hi"));
    ...
    assert!(
        format!("{}", (roundtrip_text.chars().count() as i64) > (0 as i64)) == "true".to_string()
    );
