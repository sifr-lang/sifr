...
fn main() {
    ...
    let d: Decimal =
        Decimal::from_str_exact(("-7.5".to_string()).as_str()).unwrap_or_else(|__e| unreachable!());
    let bd: BigDecimal = ("-7.5".to_string())
        .parse::<BigDecimal>()
        .unwrap_or_else(|__e| unreachable!());
    println!("{}", {
        let __l = d;
        let __r = Decimal::from_str_exact(("2".to_string()).as_str())
            .unwrap_or_else(|__e| unreachable!());
    ...
    println!(
        ...
        serde_json::to_string(
            &Decimal::from_str_exact(("1.2300".to_string()).as_str())
                .unwrap_or_else(|__e| unreachable!())
        )
        .unwrap_or_default()
    );
    ...
    let baseline_tmp_d: Decimal = Decimal::from_str_exact(("1.2345".to_string()).as_str())
        .unwrap_or_else(|__e| unreachable!())
        * Decimal::from_str_exact(("3.0".to_string()).as_str())
            .unwrap_or_else(|__e| unreachable!());
    let baseline_d: String = format!(
        "{}",
        baseline_tmp_d.round_dp_with_strategy(
            {
                let __scale = 3 as i64;
    ...
    let baseline_bd: String = format!(
        ...
        bigdecimal::Context::default()
            ...
            .unwrap_or_else(|| bigdecimal::Context::default()
                .with_rounding_mode(bigdecimal::RoundingMode::HalfEven))
            .round_decimal_ref(
                &(("1.2345678901234567890123456789".to_string())
                    .parse::<BigDecimal>()
                    .unwrap_or_else(|__e| unreachable!())
                    .clone()
                    + ("0".to_string())
    ...
    let mut i: i64 = 0 as i64;
    while i < (20 as i64) {
        let loop_tmp_d: Decimal = Decimal::from_str_exact(("1.2345".to_string()).as_str())
            .unwrap_or_else(|__e| unreachable!())
            * Decimal::from_str_exact(("3.0".to_string()).as_str())
                .unwrap_or_else(|__e| unreachable!());
        assert!(
            format!(
                "{}",
                loop_tmp_d.round_dp_with_strategy(
                    {
                        let __scale = 3 as i64;
                        (if __scale < 0 { 0 } else { __scale }) as u32
                    },
