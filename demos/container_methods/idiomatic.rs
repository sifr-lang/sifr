...
fn main() {
    ...
    words.extend("xy".chars().map(|__sifr_char| __sifr_char.to_string()));
    ...
    println!(
        ...
        format!(
            "{}",
            mapping.remove(&"missing".to_string()).unwrap_or(7 as i64)
        )
    );
    let mut seen: HashSet<i64> = HashSet::from([1 as i64]);
    seen.extend(vec![2 as i64, 3 as i64]);
    seen.extend(4 as i64..6 as i64);
    {
        let __other = (vec![3 as i64, 9 as i64])
            .into_iter()
            .collect::<std::collections::HashSet<_>>();
        seen = seen
            .symmetric_difference(&__other)
            .cloned()
            .collect::<std::collections::HashSet<_>>();
        ()
    };
    ...
    println!(
        ...
        if (2 as i64) < 0 {
            ...
            "aaaa"
                .to_string()
                .replacen(&"a".to_string(), &"b".to_string(), (2 as i64) as usize)
        }
    );
}
