...
fn collect_path_class_actual() -> Vec<bool> {
    ...
    let base: String = format!(
        "{}{}",
        "/tmp/sifr_m30_1e_pathlib_demo_".to_string(),
    ...
    let __sifr_try_res: Result<(), IOError> = (|| {
        ...
        path_flow_ok =
            (((filep.exists()) && (filep.is_file())) && (content == "hello".to_string()));
...
fn collect_missing_path_actual() -> Vec<bool> {
    ...
    let __sifr_try_res: Result<(), IOError> = (|| {
        let _bad: String =
            Path::new("/tmp/sifr_m30_1e_pathlib_demo_missing.txt".to_string()).read_text()?;
        return Ok(());
    })();
    if let Err(__sifr_try_err) = __sifr_try_res {
        let e = __sifr_try_err.clone();
