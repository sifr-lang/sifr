...
fn main() {
    let wrapper_payload: JsonValue = from_object(&vec![
    ...
    let encoded: String = dumps(&wrapper_payload);
    assert!(encoded == "{\"module\":\"json\",\"items\":[1,2,3]}".to_string());
    let mut decoded_ok: bool = false;
    let __sifr_try_res: Result<(), JSONDecodeError> = (|| {
        let mut decoded: JsonValue = loads(&encoded)?;
        let mut module_value: Option<JsonValue> = decoded.get(&"module".to_string());
        let mut items_value: Option<JsonValue> = decoded.get(&"items".to_string());
        ...
        if let Some(items_value) = items_value {
            let mut second: Option<JsonValue> = items_value.at(1 as i64);
            assert!(second.is_some());
            if let Some(second) = second {
                assert!(second.as_int() == Some(2 as i64));
            }
        }
        return Ok(());
    })();
