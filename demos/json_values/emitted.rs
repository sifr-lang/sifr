...
use crate::sifr_generated_generated_support::{
    SifrGeneratedUnion8X3asequence5X3aunion1X3a719X3a4X3aatom10X3abigdecimal11X3a4X3aatom3X3aint11X3a4X3aatom3X3astr12X3a4X3aatom4X3abool13X3a4X3aatom5X3afloat15X3a4X3aatom7X3adecimal32X3a5X3aclass19X3asifrX2ejsonX2eJsonValue1X3a0,
    dumps, from_array, from_int, from_object, from_str, loads,
};
use ::sifr_runtime::SifrInt;
...
fn main() {
    let wrapper_payload: SifrGeneratedStdlibSifrX2ejsonX2eJsonValue = from_object(&[
        ("module".to_string(), from_str("json")),
        (
            "items".to_string(),
            from_array(&[
    ...
    let sifr_generated_try_res: Result<(), JSONDecodeError> = (|| {
        let decoded: SifrGeneratedStdlibSifrX2ejsonX2eJsonValue = loads(encoded.as_str())?;
        let module_value: Option<SifrGeneratedStdlibSifrX2ejsonX2eJsonValue> =
            decoded.get("module");
        let items_value: Option<SifrGeneratedStdlibSifrX2ejsonX2eJsonValue> = decoded.get("items");
