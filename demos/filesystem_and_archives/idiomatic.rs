...
fn main() {
    let base: String = format!(
        "{}{}",
        "/tmp/sifr_wave_psp_d1_demo_".to_string(),
    ...
    let __sifr_try_res: Result<(), IOError> = (|| {
        ...
        let _w: () = std::fs::write(&source, "hello d1".to_string().as_bytes())
            .map(|_| ())
            .map_err(__io_err)?;
        let __sifr_try_res: Result<(), IOError> = (|| {
            ...
            println!(
                "{}",
                format!("{}{}", "io.read_text = ".to_string(), note_content)
            );
            return Ok(());
        })();
        if let Err(__sifr_try_err) = __sifr_try_res {
            ...
            println!(
                "{}",
                format!("{}{}", "io.read_text error: ".to_string(), e.message)
            );
        }
        let mut note_path: Path = Path::new(format!("{}{}", source, "".to_string()));
        println!(
            "{}",
            format!("{}{}", "pathlib.stem = ".to_string(), note_path.stem())
        );
        println!(
            "{}",
            format!(
                "{}{}",
                "glob(\"*.txt\") = ".to_string(),
                format!("{:?}", glob(&base, &"*.txt".to_string()))
            )
        );
        ...
        println!(
            ...
            format!(
                "{}{}",
                "shutil.move_file exists = ".to_string(),
                format!("{}", Path::new(moved).exists())
            )
        );
        ...
        println!(
            "{}",
            format!("{}{}", "tempfile.mkstemp = ".to_string(), temp_file)
        );
        println!(
            "{}",
            format!("{}{}", "tempfile.mkdtemp = ".to_string(), temp_dir)
        );
        ...
        let __sifr_try_res: Result<(), IOError> = (|| {
            ...
            println!(
                "{}",
                format!("{}{}", "gzip roundtrip = ".to_string(), restored)
            );
            return Ok(());
        })();
        if let Err(__sifr_try_err) = __sifr_try_res {
        ...
        let __sifr_try_res: Result<(), IOError> = (|| {
            ...
            println!(
                ...
                format!(
                    "{}{}",
                    "zipfile.namelist = ".to_string(),
                    format!("{:?}", archive.namelist())
                )
            );
            return Ok(());
        })();
        if let Err(__sifr_try_err) = __sifr_try_res {
            ...
            println!(
                "{}",
                format!("{}{}", "zipfile error: ".to_string(), e.message)
            );
        }
    ...
    if let Err(__sifr_try_err) = __sifr_try_res {
        ...
        println!(
            "{}",
            format!("{}{}", "wave_psp_d1 demo error: ".to_string(), e.message)
        );
    }
}
