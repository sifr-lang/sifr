...
fn main() {
    ...
    let t1: f64 = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .unwrap_or_default()
        .as_secs_f64();
    let t2: f64 = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .unwrap_or_default()
        .as_secs_f64();
    println!("{}", t2 >= t1);
    let m1: f64 = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .unwrap_or_default()
        .as_secs_f64();
    let m2: f64 = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .unwrap_or_default()
        .as_secs_f64();
    println!("{}", m2 >= m1);
    ...
    println!(
        "{}",
        std::path::Path::new(&"/tmp/sifr_polish_demo/a_copy.txt".to_string()).exists()
    );
    let __sifr_try_res: Result<(), IOError> = (|| {
        let _mv: () = move_file(
            &"/tmp/sifr_polish_demo/a_copy.txt".to_string(),
    ...
    println!(
        "{}",
        std::path::Path::new(&"/tmp/sifr_polish_demo/a_moved.txt".to_string()).exists()
    );
    println!(
        "{}",
        std::path::Path::new(&"/tmp/sifr_polish_demo/a_copy.txt".to_string()).exists()
    ...
    let __sifr_try_res: Result<(), IOError> = (|| {
        ...
        let _w4: () = std::fs::write(
            &"/tmp/sifr_polish_demo/sub/nested.txt".to_string(),
            "nested".to_string().as_bytes(),
        )
        .map(|_| ())
        .map_err(__io_err)?;
    ...
    println!(
        "{}",
        std::path::Path::new(&"/tmp/sifr_polish_demo/sub".to_string()).exists()
    );
    println!("=== tomllib ===");
