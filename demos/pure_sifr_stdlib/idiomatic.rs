...
fn main() {
    ...
    let h: String = format!(
        "{:x}",
        (<sha2::Sha256 as sha2::Digest>::digest)(("hello".to_string()).as_bytes())
    );
    ...
    if let Err(__sifr_try_err) = __sifr_try_res {
        ...
        assert!(
            format!("{}", format!("base64 error: {}", err.message))
                == "milestone_stdlib_migration demo: all checks passed!".to_string()
        );
    }
