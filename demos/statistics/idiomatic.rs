...
fn collect_positive_actual() -> Vec<String> {
    ...
    actual.push(format!(
        "{}",
        mean_ok && near(mean_v, 3.0 as f64, 0.0001 as f64)
    ));
    ...
    actual.push(format!(
        "{}",
        median_ok && near(median_v, 3.0 as f64, 0.0001 as f64)
    ));
    ...
    actual.push(format!(
        "{}",
        variance_ok && near(variance_v, 2.5 as f64, 0.0001 as f64)
    ));
    ...
    actual.push(format!(
        "{}",
        stdev_ok && near(stdev_v, 1.5811 as f64, 0.001 as f64)
    ));
    let mut mode_ok: bool = true;
    let mut mode_v: i64 = 0 as i64;
    let __sifr_try_res: Result<(), StatisticsError> = (|| {
        let out_mode: i64 = mode(&vec![
            1 as i64, 2 as i64, 2 as i64, 3 as i64, 3 as i64, 3 as i64,
        ])?;
        mode_v = out_mode;
        return Ok(());
    })();
    ...
    let __sifr_try_res: Result<(), StatisticsError> = (|| {
        let out_q: Vec<f64> = quantiles(
            &vec![
                1.0 as f64, 2.0 as f64, 3.0 as f64, 4.0 as f64, 5.0 as f64, 6.0 as f64, 7.0 as f64,
                8.0 as f64,
            ],
            4 as i64,
        )?;
    ...
    actual.push(format!(
        "{}",
        cov_ok && near(cov_v, 5.0 as f64, 0.0001 as f64)
    ));
    ...
    actual.push(format!(
        "{}",
        corr_ok && near(corr_v, 1.0 as f64, 0.0001 as f64)
    ));
    ...
    actual.push(format!(
        "{}",
        ((lr_ok && ((lr_v.len() as i64) == (2 as i64))) && lr_slope_ok) && lr_intercept_ok
    ));
    ...
    actual.push(format!(
        "{}",
        hmean_ok && near(hmean_v, 3.5555555556 as f64, 0.0001 as f64)
    ));
    ...
    actual.push(format!(
        "{}",
        gmean_ok && near(gmean_v, 6.0 as f64, 0.0001 as f64)
    ));
    return actual;
}
