...
fn main() {
    let __sifr_try_res: Result<(), RegexError> = (|| {
        let mut digits: Box<dyn Iterator<Item = Match>> =
        ...
        let mut word_it: Box<dyn Iterator<Item = Match>> =
    ...
    let base: String = format!(
        "{}{}",
        "/tmp/sifr_phase_psp_ext_3_demo_".to_string(),
    ...
    let __sifr_try_res: Result<(), IOError> = (|| {
        ...
        let _w2: () = std::fs::write(
            &format!("{}{}", base, "/sub/b.txt".to_string()),
        ...
        assert!(
            format!("{:?}", top_txt)
                == format!("{}{}{}", "[\"".to_string(), base, "/a.txt\"]".to_string())
        );
