...
fn main() {
        ...
        format!(
            ...
            Box::new(
                (vec!["x".to_string(), "y".to_string()])
                    .into_iter()
                    .enumerate()
                    .map(|__pair| ((__pair.0 as i64) + (4 as i64), __pair.1))
            )
            .collect::<Vec<_>>()
        ) == "[(4, \"x\"), (5, \"y\")]".to_string()
    );
    assert!(
        format!(
            "{:?}",
            Box::new((vec![9 as i64, 8 as i64, 7 as i64]).into_iter().rev()).collect::<Vec<_>>()
        ) == "[7, 8, 9]".to_string()
    );
    assert!(
        format!(
            "{:?}",
            chain(&vec![vec![1 as i64, 2 as i64], vec![3 as i64]]).collect::<Vec<_>>()
        ) == "[1, 2, 3]".to_string()
    );
    assert!(
        format!("{:?}", repeat(5 as i64, 3 as i64).collect::<Vec<_>>()) == "[5, 5, 5]".to_string()
    );
    assert!(
        format!(
            "{:?}",
    ...
    let combos: Vec<Vec<i64>> = combinations(
