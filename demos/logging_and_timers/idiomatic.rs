...
fn main() {
    ...
    let __sifr_try_res: Result<(), IOError> = (|| {
        let _w: () = std::fs::write(&log_path, "".to_string().as_bytes())
            .map(|_| ())
            .map_err(__io_err)?;
        ...
        let epoch_tm: struct_time = struct_time::new(
            1970 as i64,
            1 as i64,
            1 as i64,
            0 as i64,
        ...
        demo_ok = ((((((content == "INFO:wave3-demo:hello\n".to_string())
            && (gmt.tm_year == (1970 as i64)))
            && (epoch_ok))
            && (elapsed >= (0.0 as f64)))
            && (TIMEZONE == (0 as i64)))
            && ((__const_TZNAME()).0 == "UTC".to_string()));
        return Ok(());
    })();
    if let Err(__sifr_try_err) = __sifr_try_res {
        let e = __sifr_try_err.clone();
    ...
    let __sifr_try_res: Result<(), IOError> = (|| {
        if std::path::Path::new(&log_path).exists() {
            let _rm: () = std::fs::remove_file(&log_path)
                .map(|_| ())
                .map_err(__io_err)?;
        }
        return Ok(());
