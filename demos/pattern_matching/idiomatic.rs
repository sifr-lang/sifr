...
fn classify_http(method: &String) -> String {
    match method {
        __s if (__s.as_str() == "GET".to_string().as_str())
            || (__s.as_str() == "HEAD".to_string().as_str()) =>
        {
            return "read".to_string();
        }
        __s if ((__s.as_str() == "POST".to_string().as_str())
            || (__s.as_str() == "PUT".to_string().as_str()))
            || (__s.as_str() == "PATCH".to_string().as_str()) =>
        {
...
fn main() {
    ...
    println!(
        "{}",
        classify_quadrant(&Point::new(-(1 as i64), -(1 as i64)))
    );
}
