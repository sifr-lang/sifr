    ...
    let __sifr_try_res: Result<(), ValueError> = (|| {
        ...
        assert!(
            format!("{}", format!("from_timestamp_ok = {}", epoch_text))
                == "from_timestamp_ok = 1970-01-01T00:00:00".to_string()
        );
        return Ok(());
    })();
    if let Err(__sifr_try_err) = __sifr_try_res {
        ...
        assert!(
            format!("{}", format!("unexpected_error = {}", e.message))
                == "from_timestamp_invalid = invalid timestamp".to_string()
        );
    }
    let __sifr_try_res: Result<(), ValueError> = (|| {
