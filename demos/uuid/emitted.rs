...
mod sifr_generated_project_nominals {
    use crate::sifr_generated_generated_support::sifr_generated_hex_digit_value;
...
use crate::sifr_generated_generated_support::{
    NAMESPACE_DNS, assert_bool_vector_eq, uuid_from_hex, uuid3, uuid4, uuid4_obj, uuid5,
};
use ::sifr_runtime::SifrInt;
...
fn is_canonical_shape(value: &str) -> bool {
    let sifr_generated_chars_value: Vec<char> = value.chars().collect::<Vec<char>>();
    if sifr_generated_chars_value.len() != SifrInt::from_i64(36) {
        return false;
    }
    let h1: Option<String> = {
        let sifr_generated_string_index = SifrInt::from_i64(8);
...
fn collect_parse_actual() -> Vec<bool> {
    ...
    let sifr_generated_try_res: Result<(), ValueError> = (|| {
        let parsed_v1_value_b1aa999388397088: SifrGeneratedStdlibSifrX2euuidX2eUUID =
            uuid_from_hex("550e8400-e29b-11d4-a716-446655440000")?;
...
fn collect_negative_and_class_actual() -> Vec<bool> {
    ...
    let obj: SifrGeneratedStdlibSifrX2euuidX2eUUID = uuid4_obj();
    actual.push(obj.hex().chars().count() == SifrInt::from_i64(32));
    actual.push(uuid3(&NAMESPACE_DNS(), "python.org").version() == SifrInt::from_i64(3));
    actual.push(uuid5(&NAMESPACE_DNS(), "python.org").version() == SifrInt::from_i64(5));
    actual
}
