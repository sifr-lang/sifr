...
fn uuid3(namespace: &UUID, name: &String) -> UUID {
    return UUID::new({
        let __ns = uuid::Uuid::parse_str(&namespace.to_str()).unwrap_or(uuid::Uuid::nil());
...
fn is_canonical_shape(value: &String) -> bool {
    ...
    return ((((h1 == Some("-".to_string())) && (h2 == Some("-".to_string())))
        && (h3 == Some("-".to_string())))
        && (h4 == Some("-".to_string())));
}
