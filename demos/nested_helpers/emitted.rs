...
fn demo_letter_combinations(digits: &String) -> Vec<String> {
    ...
    let digit_to_chars: HashMap<String, Vec<String>> = HashMap::from([("2".to_string(), vec!["a".to_string(), "b".to_string(), "c".to_string()]), ("3".to_string(), vec!["d".to_string(), "e".to_string(), "f".to_string()])]);
    fn backtrack(i: i64, cur: &String, digit_to_chars: &HashMap<String, Vec<String>>, digits: &String, res: &mut Vec<String>) {
    ...
    __indexed_char.to_string()
        ...
        for ch in digit_to_chars.get(&d).cloned().expect(&"dict index proven by guard".to_string()) {
            backtrack(i + (1 as i64), &format!("{}{}", cur, ch), digit_to_chars, digits, res);
        }
    }
...
fn demo_total_n_queens(n: i64) -> i64 {
    ...
    return backtrack(0 as i64, &mut HashSet::new(), &mut HashSet::new(), &mut HashSet::new(), n);
}
...
fn union_nodes(n1: i64, n2: i64, par: &mut Vec<i64>, rank: &mut Vec<i64>) -> bool {
    ...
    if ((((p1 < (0 as i64)) || (p1 >= (rank.len() as i64))) || (p2 < (0 as i64))) || (p2 >= (rank.len() as i64))) {
        return false;
    }
    if p1 == p2 {
        return false;
...
fn main() {
    assert!(format!("{:?}", demo_letter_combinations(&"23".to_string())) == "[\"ad\", \"ae\", \"af\", \"bd\", \"be\", \"bf\", \"cd\", \"ce\", \"cf\"]".to_string());
    assert!(demo_total_n_queens(4 as i64) == (2 as i64));
    assert!(demo_redundant_connection(&vec![(1 as i64, 2 as i64), (1 as i64, 3 as i64), (2 as i64, 3 as i64)]) == vec![2 as i64, 3 as i64]);
}
