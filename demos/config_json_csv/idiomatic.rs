...
fn main() {
    ...
    let payload: JsonValue = from_object(&vec![
        ("module".to_string(), from_str(&"wave1".to_string())),
        ("version".to_string(), from_int(1 as i64)),
    ]);
    ...
    let __sifr_try_res: Result<(), Error> = (|| {
        let decoded_value: JsonValue =
            (decoder.decode(&encoded)).map_err(|__e| Error::new(__e.to_string()))?;
