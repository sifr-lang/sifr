...
fn collect_io_roundtrip_actual() -> Vec<bool> {
    ...
    let __sifr_try_res: Result<(), IOError> = (|| {
        let _w: () = std::fs::write(&path, "hello".to_string().as_bytes())
            .map(|_| ())
            .map_err(__io_err)?;
        let _a: () = ({
...
fn collect_open_actual() -> Vec<bool> {
    ...
    let __sifr_try_res: Result<(), IOError> = (|| {
        ...
        f.close();
        first_ok = (first).map_or("None".to_string().to_string(), |__v| format!("{}", __v))
            == "hello".to_string();
        second_ok = (second).map_or("None".to_string().to_string(), |__v| format!("{}", __v))
            == "world".to_string();
    ...
    let __sifr_try_res: Result<(), IOError> = (|| {
        let _: FileHandle = (|| {
            let __path = "/tmp/sifr_m30_1e_io_demo_missing.txt"
                .to_string()
                .to_string();
            let __mode = "r".to_string().to_string();
            let __handle_id = __sifr_next_file_handle_id();
