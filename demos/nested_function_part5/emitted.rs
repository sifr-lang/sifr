...
fn combination_sum(candidates: &Vec<i64>, target: i64) -> Vec<Vec<i64>> {
    let mut res: Vec<Vec<i64>> = vec![];
    fn dfs(i: i64, cur: &mut Vec<i64>, total: i64, candidates: &Vec<i64>, res: &mut Vec<Vec<i64>>, target: i64) {
        if total == target {
            res.push(cur.clone());
            return;
        }
    ...
    dfs(0 as i64, &mut vec![], 0 as i64, candidates, &mut res, target);
    return res;
}
...
fn main() {
    ...
    assert!(format!("{:?}", subsets(&vec![1 as i64, 2 as i64, 3 as i64])) == "[[1, 2, 3], [1, 2], [1, 3], [1], [2, 3], [2], [3], []]".to_string());
