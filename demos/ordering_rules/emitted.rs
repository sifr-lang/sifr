...
impl Processor {
    fn new(name: String, transform: impl Fn(i64) -> i64 + 'static) -> Self {
        return Self { name: name, transform: Box::new(transform) };
    }
}
...
fn main() {
    let repeated: Vec<i64> = repeat(7 as i64, 3 as i64).collect::<Vec<_>>();
    println!("{:?}", repeated);
    for val in repeat(42 as i64, 2 as i64) {
        println!("{}", val);
    }
    let chained: Vec<i64> = chain(&vec![vec![1 as i64, 2 as i64], vec![3 as i64, 4 as i64]]).collect::<Vec<_>>();
    println!("{:?}", chained);
    let first_two: Vec<i64> = take(2 as i64, &(vec![10 as i64, 20 as i64, 30 as i64, 40 as i64]).into_iter().collect::<Vec<_>>());
    println!("{:?}", first_two);
    let __sifr_try_res: Result<(), JSONDecodeError> = (|| {
    let parsed: JsonValue = loads(&"{\"key\": 42}".to_string())?;
    println!("{}", parsed);
    return Ok(());
})();
    ...
    let mut p: Processor = Processor::new("tripler".to_string(), triple);
    println!("{}", p.name);
    let result: i64 = (p.transform)(10 as i64);
    println!("{}", result);
}
