        ...
        if let Some(arg) = arg {
            if ((arg == name.clone()) && ((i + (1 as i64)) < (args.len() as i64))) {
                ...
                if let Some(next_val) = next_val {
                    if ((next_val == "--".to_string()) || (next_val.starts_with(&"--".to_string())))
                    {
                    } else {
            ...
            let (inline_has_value, inline_name, inline_value) = _split_inline_option(&arg);
...
fn heapify_copy<T: Clone + std::fmt::Display + PartialOrd + 'static>(data: &Vec<T>) -> Vec<T> {
    let mut result: Vec<T> = vec![];
...
fn nsmallest<T: Clone + std::fmt::Display + PartialOrd + 'static>(n: i64, data: &Vec<T>) -> Vec<T> {
...
fn main() {
    ...
    if let Err(__sifr_try_err) = __sifr_try_res {
        ...
        println!(
            "{}",
            format!("{}{}", "statistics error: ".to_string(), se.message)
        );
        assert!(
            format!(
                "{}",
                format!("{}{}", "statistics error: ".to_string(), se.message)
            ) == "milestone_stdlib_expansion demo: all checks passed!".to_string()
        );
    }
    ...
    let merged: Vec<i64> =
        chain(&vec![vec![1 as i64, 2 as i64], vec![3 as i64, 4 as i64]]).collect::<Vec<_>>();
    assert_eq!(merged.len() as i64, 4 as i64);
    let first3: Vec<i64> = take(
        3 as i64,
        &(vec![10 as i64, 20 as i64, 30 as i64, 40 as i64])
            .into_iter()
            .collect::<Vec<_>>(),
    );
