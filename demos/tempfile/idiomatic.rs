...
fn _collision_message(kind: &String, attempts: i64) -> String {
    return format!(
        ...
        "tempfile.".to_string(),
        kind,
        ": failed to create unique path after ".to_string(),
        format!("{}", attempts),
        " attempts".to_string()
    );
}
fn mkstemp(prefix: &String) -> Result<String, IOError> {
    ...
    return Err(IOError::new(_collision_message(
        &"mkstemp".to_string(),
        max_attempts,
    )));
}
fn mkdtemp(prefix: &String) -> Result<String, IOError> {
    ...
    while attempts < max_attempts {
        ...
        let __sifr_try_res: Result<Result<String, IOError>, IOError> = (|| {
            let md: () = std::fs::create_dir_all(&path)
                .map(|_| ())
                .map_err(__io_err)?;
            return Ok(Ok(path));
            unreachable!("sifr try/except return capture fell through");
...
fn collect_tempfile_actual() -> Vec<bool> {
    ...
    let __sifr_try_res: Result<(), IOError> = (|| {
        ...
        let preview_has_prefix: bool = (((preview_name.chars().count() as i64)
            > ("sifr_m30_1e_preview_".to_string().chars().count() as i64))
            && (String::from_iter(
                (preview_name)
                    .chars()
                    .skip((0 as i64).max(0) as usize)
                    .take(
                        (("sifr_m30_1e_preview_".to_string().chars().count() as i64).max(0)
                            - (0 as i64).max(0))
                        .max(0) as usize,
                    ),
            ) == "sifr_m30_1e_preview_".to_string()));
        let file_has_prefix: bool = (((file_name.chars().count() as i64)
            > ("sifr_m30_1e_tmp_".to_string().chars().count() as i64))
            && (String::from_iter(
                (file_name).chars().skip((0 as i64).max(0) as usize).take(
                    (("sifr_m30_1e_tmp_".to_string().chars().count() as i64).max(0)
                        - (0 as i64).max(0))
                    .max(0) as usize,
                ),
            ) == "sifr_m30_1e_tmp_".to_string()));
        let dir_has_prefix: bool = (((dir_name.chars().count() as i64)
            > ("sifr_m30_1e_tmpd_".to_string().chars().count() as i64))
            && (String::from_iter(
                (dir_name).chars().skip((0 as i64).max(0) as usize).take(
                    (("sifr_m30_1e_tmpd_".to_string().chars().count() as i64).max(0)
                        - (0 as i64).max(0))
                    .max(0) as usize,
                ),
            ) == "sifr_m30_1e_tmpd_".to_string()));
        ...
        let missing_parent_path: String =
            format!("{}{}{}", temp_root, "/".to_string(), missing_parent_name);
        ...
        let cleaned: bool = ((!(std::path::Path::new(&file_path).exists()))
