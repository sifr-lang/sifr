...
fn main() {
    ...
    if let Err(__sifr_try_err) = __sifr_try_res {
        ...
        assert!(
            format!("{}", format!("getcwd error: {}", err.message))
                == "milestone_stdlib_parity demo: all checks passed!".to_string()
        );
    }
    let __sifr_try_res: Result<(), RegexError> = (|| {
        let matches: Vec<String> = regex::Regex::new(&"[0-9]+".to_string())
            .map(|re| {
                re.find_iter(&"abc123def456".to_string())
    ...
    if let Err(__sifr_try_err) = __sifr_try_res {
        ...
        assert!(
            format!("{}", format!("regex error: {}", err.message))
                == "Total stdlib modules: 37".to_string()
        );
    }
    ...
    let sys: String = if cfg!(target_os = "windows") {
