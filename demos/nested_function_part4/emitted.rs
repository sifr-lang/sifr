fn combination_sum(candidates: &Vec<i64>, target: i64) -> Vec<Vec<i64>> {
    ...
    fn dfs(i: i64, cur: &mut Vec<i64>, total: i64, candidates: &Vec<i64>, result: &mut Vec<Vec<i64>>, target: i64) {
        if total == target {
    ...
    dfs(0 as i64, &mut vec![], 0 as i64, candidates, &mut result, target);
    return result;
}
...
fn main() {
    assert!(format!("{:?}", combination_sum(&vec![2 as i64, 3 as i64, 6 as i64, 7 as i64], 7 as i64)) == "[[2, 2, 3], [7]]".to_string());
    assert!(format!("{:?}", subsets(&vec![1 as i64, 2 as i64, 3 as i64])) == "[[], [3], [2], [2, 3], [1], [1, 3], [1, 2], [1, 2, 3]]".to_string());
}
