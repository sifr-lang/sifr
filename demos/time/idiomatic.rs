...
fn collect_clock_actual() -> Vec<bool> {
    ...
    let perf_before: f64 = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .unwrap_or_default()
        .as_secs_f64();
    let mono_before: f64 = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .unwrap_or_default()
        .as_secs_f64();
    {
    ...
    let perf_after: f64 = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .unwrap_or_default()
        .as_secs_f64();
    let mono_after: f64 = std::time::SystemTime::now()
        .duration_since(std::time::UNIX_EPOCH)
        .unwrap_or_default()
        .as_secs_f64();
...
fn collect_format_actual() -> Vec<bool> {
    let mut actual: Vec<bool> = vec![];
    actual.push(
        (strftime(&"%Y-%m-%d %H:%M:%S".to_string(), 0.0 as f64)).as_str()
            == ("1970-01-01 00:00:00".to_string()).as_str(),
    );
    let gmt: struct_time = gmtime_struct(0.0 as f64);
    actual.push(
        (((((gmt.tm_year == (1970 as i64)) && (gmt.tm_mon == (1 as i64)))
            && (gmt.tm_mday == (1 as i64)))
            && (gmt.tm_hour == (0 as i64)))
            && (gmt.tm_min == (0 as i64)))
            && (gmt.tm_sec == (0 as i64)),
    );
...
fn collect_parse_and_safety_actual() -> Vec<bool> {
    ...
    let __sifr_try_res: Result<(), ValueError> = (|| {
        let _bad: String = chrono::NaiveDateTime::parse_from_str(
            &"bad".to_string(),
            &"%Y-%m-%d %H:%M:%S".to_string(),
        )
        .map(|dt| dt.format("%Y-%m-%dT%H:%M:%S").to_string())
