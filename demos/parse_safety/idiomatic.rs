...
fn loads(text: &String) -> Result<TomlValue, TOMLDecodeError> {
    return {
        ...
        fn __sifr_toml_value_from_parsed(value: toml::Value) -> Result<TomlValue, TOMLDecodeError> {
            match value {
                ...
                toml::Value::Table(items) => {
                    ...
                    for entry in items {
                        let entry_key = entry.0;
                        let entry_value = entry.1;
                        let converted_value = __sifr_toml_value_from_parsed(entry_value)?;
...
fn demo_toml() {
    ...
    let __sifr_try_res: Result<(), TOMLDecodeError> = (|| {
        ...
        println!(
            "toml parsed: {}",
            (toml_data.keys().len() as i64) > (0 as i64)
        );
        return Ok(());
    })();
    if let Err(__sifr_try_err) = __sifr_try_res {
...
fn demo_regex() {
    ...
    let __sifr_try_res: Result<(), RegexError> = (|| {
        let matched: bool = regex::Regex::new(&"\\d+".to_string())
            .map(|re| re.is_match(&"abc123".to_string()))
            .map_err(|e| RegexError {
                message: e.to_string(),
                detail: e.to_string(),
            })?;
        println!("match found: {}", matched);
        let found: Option<String> = regex::Regex::new(&"\\d+".to_string())
            .map(|re| {
                re.find(&"hello 42 world".to_string())
        ...
        let replaced: String = regex::Regex::new(&"\\d+".to_string())
            .map(|re| {
                re.replace_all(&"test 1 2 3".to_string(), &*"NUM".to_string())
                    .to_string()
            })
            .map_err(|e| RegexError {
                message: e.to_string(),
        ...
        let all_matches: Vec<String> = regex::Regex::new(&"[a-z]+".to_string())
            .map(|re| {
                re.find_iter(&"Hello World Sifr".to_string())
                    .map(|m| m.as_str().to_string())
                    .collect::<Vec<String>>()
            })
            .map_err(|e| RegexError {
        ...
        let parts: Vec<String> = regex::Regex::new(&",".to_string())
            .map(|re| {
                re.split(&"a,b,c".to_string())
                    .map(|s| s.to_string())
                    .collect::<Vec<String>>()
            })
            .map_err(|e| RegexError {
    ...
    let __sifr_try_res: Result<(), RegexError> = (|| {
        let bad_match: bool = regex::Regex::new(&"[unclosed".to_string())
            .map(|re| re.is_match(&"text".to_string()))
            .map_err(|e| RegexError {
                message: e.to_string(),
                detail: e.to_string(),
            })?;
...
fn demo_base64() {
    ...
    let encoded: String = base64::Engine::encode(
        &base64::engine::general_purpose::STANDARD,
        &"safe decoding!".to_string().as_bytes(),
    );
...
fn demo_bytes() {
    ...
    let __sifr_try_res: Result<(), ParseError> = (|| {
        ...
        let bad_text: String = String::from_utf8(bad_bytes.iter().copied().collect::<Vec<u8>>())
            .map_err(|e| ParseError {
                message: e.to_string(),
            })?;
        println!("should not reach here");
    ...
    let __sifr_try_res: Result<(), ParseError> = (|| {
        ...
        let decoded_hex: String = String::from_utf8(hex_data.iter().copied().collect::<Vec<u8>>())
            .map_err(|e| ParseError {
                message: e.to_string(),
            })?;
