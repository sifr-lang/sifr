...
fn demo_json() {
    ...
    let data: JsonValue = ({
    let __json_input = "{\"language\":\"sifr\",\"safe\":true}".to_string();
    fn __sifr_json_value_from_serde(value: serde_json::Value) -> Result<JsonValue, JSONDecodeError> {
        match value {
            serde_json::Value::Null => {
                return Ok(JsonValue { kind: "null".to_string().to_string(), bool_value: None, int_value: None, float_value: None, str_value: None, array_items: Box::new(vec![]), object_items: Box::new(vec![]) });
            },
            serde_json::Value::Bool(b) => {
                return Ok(JsonValue { kind: "bool".to_string().to_string(), bool_value: Some(b), int_value: None, float_value: None, str_value: None, array_items: Box::new(vec![]), object_items: Box::new(vec![]) });
            },
            serde_json::Value::Number(n) => {
                if let Some(i) = n.as_i64() {
                    return Ok(JsonValue { kind: "int".to_string().to_string(), bool_value: None, int_value: Some(i), float_value: None, str_value: None, array_items: Box::new(vec![]), object_items: Box::new(vec![]) });
                }
                if n.is_u64() {
                    return Err(JSONDecodeError { message: "json integer out of range for sifr int".to_string().to_string(), line: 0, column: 0 });
                }
                if let Some(f) = n.as_f64() {
                    return Ok(JsonValue { kind: "float".to_string().to_string(), bool_value: None, int_value: None, float_value: Some(f), str_value: None, array_items: Box::new(vec![]), object_items: Box::new(vec![]) });
                }
                return Err(JSONDecodeError { message: "unsupported json number representation".to_string().to_string(), line: 0, column: 0 });
            },
            serde_json::Value::String(s) => {
                return Ok(JsonValue { kind: "str".to_string().to_string(), bool_value: None, int_value: None, float_value: None, str_value: Some(s), array_items: Box::new(vec![]), object_items: Box::new(vec![]) });
            },
            serde_json::Value::Array(items) => {
                ...
                return Ok(JsonValue { kind: "array".to_string().to_string(), bool_value: None, int_value: None, float_value: None, str_value: None, array_items: Box::new(converted), object_items: Box::new(vec![]) });
            },
            serde_json::Value::Object(entries) => {
                ...
                return Ok(JsonValue { kind: "object".to_string().to_string(), bool_value: None, int_value: None, float_value: None, str_value: None, array_items: Box::new(vec![]), object_items: Box::new(converted) });
            },
        }
    }
    serde_json::from_str::<serde_json::Value>(__json_input.as_ref()).map_err(|e| JSONDecodeError { message: e.to_string(), line: e.line() as i64, column: e.column() as i64 }).and_then(|parsed| __sifr_json_value_from_serde(parsed))
})?;
    println!("parsed: {}", data);
    return Ok(());
})();
    ...
    let __sifr_try_res: Result<(), JSONDecodeError> = (|| {
    let bad: JsonValue = ({
    let __json_input = "{not valid json".to_string();
    fn __sifr_json_value_from_serde(value: serde_json::Value) -> Result<JsonValue, JSONDecodeError> {
        match value {
            serde_json::Value::Null => {
                return Ok(JsonValue { kind: "null".to_string().to_string(), bool_value: None, int_value: None, float_value: None, str_value: None, array_items: Box::new(vec![]), object_items: Box::new(vec![]) });
    ...
    let dumped: String = serde_json::to_string(&(42 as i64)).unwrap_or_default();
...
fn demo_toml() {
    ...
    let mut toml_data: TomlValue = loads(&"name = \"sifr\"\nversion = 1".to_string())?;
    println!("toml parsed: {}", (toml_data.keys().len() as i64) > (0 as i64));
    return Ok(());
})();
    if let Err(__sifr_try_err) = __sifr_try_res {
        let e = __sifr_try_err.clone();
    ...
    let __sifr_try_res: Result<(), TOMLDecodeError> = (|| {
    let bad_toml: TomlValue = loads(&"[broken toml ===".to_string())?;
    let _: TomlValue = bad_toml;
    println!("should not reach here");
    return Ok(());
})();
    if let Err(__sifr_try_err) = __sifr_try_res {
...
fn demo_regex() {
    ...
    let matched: bool = regex::Regex::new(&"\\d+".to_string()).map(|re| re.is_match(&"abc123".to_string())).map_err(|e| RegexError { message: e.to_string(), detail: e.to_string() })?;
    println!("match found: {}", matched);
    let found: Option<String> = regex::Regex::new(&"\\d+".to_string()).map(|re| re.find(&"hello 42 world".to_string()).map(|m| m.as_str().to_string())).map_err(|e| RegexError { message: e.to_string(), detail: e.to_string() })?;
    if let Some(found) = found {
        println!("found: {}", found);
    }
    let replaced: String = regex::Regex::new(&"\\d+".to_string()).map(|re| re.replace_all(&"test 1 2 3".to_string(), &*"NUM".to_string()).to_string()).map_err(|e| RegexError { message: e.to_string(), detail: e.to_string() })?;
    println!("replaced: {}", replaced);
    let all_matches: Vec<String> = regex::Regex::new(&"[a-z]+".to_string()).map(|re| re.find_iter(&"Hello World Sifr".to_string()).map(|m| m.as_str().to_string()).collect::<Vec<String>>()).map_err(|e| RegexError { message: e.to_string(), detail: e.to_string() })?;
    println!("findall count: {}", all_matches.len() as i64);
    let parts: Vec<String> = regex::Regex::new(&",".to_string()).map(|re| re.split(&"a,b,c".to_string()).map(|s| s.to_string()).collect::<Vec<String>>()).map_err(|e| RegexError { message: e.to_string(), detail: e.to_string() })?;
    ...
    let __sifr_try_res: Result<(), RegexError> = (|| {
    let bad_match: bool = regex::Regex::new(&"[unclosed".to_string()).map(|re| re.is_match(&"text".to_string())).map_err(|e| RegexError { message: e.to_string(), detail: e.to_string() })?;
    println!("should not reach here");
    return Ok(());
})();
    if let Err(__sifr_try_err) = __sifr_try_res {
...
fn demo_base64() {
    ...
    let encoded: String = base64::Engine::encode(&base64::engine::general_purpose::STANDARD, &"safe decoding!".to_string().as_bytes());
    let __sifr_try_res: Result<(), ParseError> = (|| {
    let decoded: String = ({
    let __bytes = base64::Engine::decode(&base64::engine::general_purpose::STANDARD, &encoded.as_bytes()).map_err(|e| ParseError { message: e.to_string() })?;
    ...
    let __sifr_try_res: Result<(), ParseError> = (|| {
    let bad_decoded: String = ({
    let __bytes = base64::Engine::decode(&base64::engine::general_purpose::STANDARD, &"!!!not-base64!!!".to_string().as_bytes()).map_err(|e| ParseError { message: e.to_string() })?;
    String::from_utf8(__bytes).map_err(|e| ParseError { message: e.to_string() })
})?;
    println!("should not reach here");
    return Ok(());
...
fn demo_bytes() {
    ...
    let __sifr_try_res: Result<(), ParseError> = (|| {
    let text: String = String::from_utf8(({
    let __s = "hello sifr".to_string();
    __s.as_bytes().to_vec()
}).iter().copied().collect::<Vec<u8>>()).map_err(|e| ParseError { message: e.to_string() })?;
    println!("utf8: {}", text);
    return Ok(());
})();
    if let Err(__sifr_try_err) = __sifr_try_res {
    ...
    let __sifr_try_res: Result<(), ParseError> = (|| {
    let bad_bytes: Vec<u8> = vec![(255 as i64) as u8, (254 as i64) as u8, (253 as i64) as u8];
    let bad_text: String = String::from_utf8(bad_bytes.iter().copied().collect::<Vec<u8>>()).map_err(|e| ParseError { message: e.to_string() })?;
    println!("should not reach here");
    return Ok(());
})();
    if let Err(__sifr_try_err) = __sifr_try_res {
    ...
    let decoded_hex: String = String::from_utf8(hex_data.iter().copied().collect::<Vec<u8>>()).map_err(|e| ParseError { message: e.to_string() })?;
