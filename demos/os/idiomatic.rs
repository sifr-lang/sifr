...
fn collect_filesystem_actual() -> Vec<bool> {
    ...
    let base: String = format!(
        "{}{}",
        "/tmp/sifr_m30_1e_os_demo_".to_string(),
    ...
    let __sifr_try_res: Result<(), IOError> = (|| {
        let _mk: () = std::fs::create_dir_all(&base)
            .map(|_| ())
            .map_err(__io_err)?;
        let _w: () = std::fs::write(&file_path, "demo".to_string().as_bytes())
            .map(|_| ())
            .map_err(__io_err)?;
        os_flow_ok = ((std::path::Path::new(&base).is_dir())
        ...
        let _rm: () = std::fs::remove_file(&file_path)
            .map(|_| ())
            .map_err(__io_err)?;
...
fn collect_missing_actual() -> Vec<bool> {
    ...
    let __sifr_try_res: Result<(), IOError> = (|| {
        let _bad: () = std::fs::remove_dir(&"/tmp/sifr_m30_1e_os_demo_missing".to_string())
            .map(|_| ())
            .map_err(__io_err)?;
        return Ok(());
    })();
