...
fn demo_specific_subclass() {
    ...
    let __sifr_try_res: Result<(), IOError> = (|| {
        let content: String =
            std::fs::read_to_string(&"/nonexistent/file.txt".to_string()).map_err(__io_err)?;
        return Ok(());
    })();
...
fn demo_regex_fields() {
    ...
    let __sifr_try_res: Result<(), RegexError> = (|| {
        let m: bool = regex::Regex::new(&"[unclosed".to_string())
...
fn demo_mixed_families() {
    ...
    let __sifr_try_res: Result<(), IOError> = (|| {
        let data: String =
            std::fs::read_to_string(&"/nonexistent/data.json".to_string()).map_err(__io_err)?;
        return Ok(());
    })();
    if let Err(__sifr_try_err) = __sifr_try_res {
        if __sifr_try_err.kind == "FileNotFound".to_string().to_string() {
