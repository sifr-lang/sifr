...
fn demo_specific_subclass() {
    ...
    let content: String = std::fs::read_to_string(&"/nonexistent/file.txt".to_string()).map_err(__io_err)?;
    return Ok(());
})();
    if let Err(__sifr_try_err) = __sifr_try_res {
        if __sifr_try_err.kind == "FileNotFound".to_string().to_string() {
            ...
            let e = __sifr_try_err.clone();
            println!("Other IOError: {}", e.message);
        }
    }
...
fn demo_parent_catch_all() {
    ...
    if let Err(__sifr_try_err) = __sifr_try_res {
        let e = __sifr_try_err.clone();
        println!("IOError (catches all subclasses): {}", e.message);
...
fn demo_json_fields() {
    ...
    let result: JsonValue = loads(&"{bad json".to_string())?;
    let _: JsonValue = result;
    return Ok(());
})();
    if let Err(__sifr_try_err) = __sifr_try_res {
        let e = __sifr_try_err.clone();
        println!("JSONDecodeError: {}", e.message);
...
fn demo_regex_fields() {
    ...
    let m: bool = regex::Regex::new(&"[unclosed".to_string()).map(|re| re.is_match(&"test".to_string())).map_err(|e| RegexError { message: e.to_string(), detail: e.to_string() })?;
    return Ok(());
})();
    if let Err(__sifr_try_err) = __sifr_try_res {
        let e = __sifr_try_err.clone();
        println!("RegexError: {}", e.message);
...
fn demo_mixed_families() {
    ...
    let __sifr_try_res: Result<(), IOError> = (|| {
    let data: String = std::fs::read_to_string(&"/nonexistent/data.json".to_string()).map_err(__io_err)?;
    return Ok(());
})();
    if let Err(__sifr_try_err) = __sifr_try_res {
        if __sifr_try_err.kind == "FileNotFound".to_string().to_string() {
