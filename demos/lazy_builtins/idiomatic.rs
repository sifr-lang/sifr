fn main() {
    ...
    println!(
        "{}",
        (rev_it.next()).map_or("None".to_string().to_string(), |__v| format!("{}", __v))
    );
    println!("{:?}", rev_it.collect::<Vec<_>>());
    let mut indexed_it: Box<dyn Iterator<Item = (i64, String)>> = Box::new(
        (vec!["a".to_string(), "b".to_string()])
            .into_iter()
            .enumerate()
            .map(|__pair| ((__pair.0 as i64) + (5 as i64), __pair.1)),
    );
    println!("{:?}", indexed_it.collect::<Vec<_>>());
    let mut zipped_it: Box<dyn Iterator<Item = (i64, String, bool)>> = Box::new(
