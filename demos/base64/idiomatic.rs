...
fn urlsafe_b64encode(s: &String) -> String {
    return base64::engine::general_purpose::URL_SAFE.encode(s.as_bytes());
}
fn urlsafe_b64decode(s: &String) -> Result<String, ParseError> {
    return {
        let __bytes = base64::engine::general_purpose::URL_SAFE
            .decode(s.as_bytes())
            .map_err(|e| ParseError {
                message: e.to_string(),
            })?;
...
fn b16decode(s: &String) -> Result<String, ParseError> {
    let __sifr_try_res: Result<Result<String, ParseError>, ParseError> = (|| {
        ...
        return Ok(
            String::from_utf8(data.iter().copied().collect::<Vec<u8>>()).map_err(|e| ParseError {
                message: e.to_string(),
            }),
        );
        unreachable!("sifr try/except return capture fell through");
