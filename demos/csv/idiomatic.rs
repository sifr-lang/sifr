...
fn collect_parse_actual() -> Vec<bool> {
    ...
    actual
        .push((format!("{:?}", parsed)).as_str() == ("[\"a\", \"b\", \"c\"]".to_string()).as_str());
    actual.push(
        (format_csv(
            &vec![
...
fn collect_object_and_file_actual() -> Vec<bool> {
    ...
    let __sifr_try_res: Result<(), IOError> = (|| {
        ...
        let mut rf: reader = reader_from_path(
            &path,
            &None,
            &",".to_string(),
            &"\"".to_string(),
        ...
        csv_file_ok =
            format!("{:?}", rf.rows()) == "[[\"h1\", \"h2\"], [\"v1\", \"v2\"]]".to_string();
        return Ok(());
    })();
    ...
    let __sifr_try_res: Result<(), IOError> = (|| {
        let mut _missing: reader = reader_from_path(
            &"/tmp/sifr_m30_1e_csv_demo_missing.csv".to_string(),
            &None,
            &",".to_string(),
            &"\"".to_string(),
            &"".to_string(),
