...
fn pairwise<T: Clone + std::fmt::Display + PartialOrd + 'static>(data: &Vec<T>) -> Vec<Vec<T>> {
    let mut result: Vec<Vec<T>> = vec![];
...
fn nlargest<T: Clone + std::fmt::Display + PartialOrd + 'static>(n: i64, data: &Vec<T>) -> Vec<T> {
    if n <= (0 as i64) {
        return vec![];
    }
...
fn main() {
    ...
    let first3_int: Vec<i64> = take(
        3 as i64,
        &(vec![10 as i64, 20 as i64, 30 as i64, 40 as i64, 50 as i64])
            .into_iter()
            .collect::<Vec<_>>(),
    );
    println!("{:?}", first3_int);
    ...
    let nested_int: Vec<Vec<i64>> = vec![
        vec![1 as i64, 2 as i64],
        vec![3 as i64, 4 as i64],
        vec![5 as i64],
    ];
    let flat_int: Vec<i64> = flatten(&(nested_int).iter().cloned().collect::<Vec<_>>());
    println!("{:?}", flat_int);
    println!("=== Generic accumulate ===");
    let sums: Vec<i64> = accumulate(
        &(vec![1 as i64, 2 as i64, 3 as i64, 4 as i64, 5 as i64])
            .into_iter()
            .collect::<Vec<_>>(),
        None,
    )
    ...
    let float_sums: Vec<f64> = accumulate(
        &(vec![1.0 as f64, 2.5 as f64, 3.5 as f64])
            .into_iter()
            .collect::<Vec<_>>(),
        None,
    )
    ...
    let odds: Vec<i64> = filterfalse(
        |__arg0| is_even((__arg0).clone()),
        &(vec![1 as i64, 2 as i64, 3 as i64, 4 as i64, 5 as i64, 6 as i64])
            .into_iter()
            .collect::<Vec<_>>(),
    )
    .collect::<Vec<_>>();
