...
fn main() {
    let zero: timezone = timezone::new(0 as i64);
    assert!(format!("{}", zero) == "UTC".to_string());
    let plus_two_thirty: timezone = timezone::new(9000 as i64);
    let minus_five: timezone = timezone::new(-(18000 as i64));
    assert!(format!("{}", plus_two_thirty) == "UTC+02:30".to_string());
    assert!(format!("{}", minus_five) == "UTC-05:00".to_string());
    let mut current: datetime = now(&None);
    assert!(current.year >= (1970 as i64));
    assert!(((current.month >= (1 as i64)) && (current.month <= (12 as i64))));
    assert!(((current.day >= (1 as i64)) && (current.day <= (31 as i64))));
    assert!((current.isoformat().chars().count() as i64) == (19 as i64));
}
