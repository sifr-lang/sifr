...
fn collect_positive_actual() -> Vec<bool> {
    ...
    let __sifr_try_res: Result<(), JSONDecodeError> = (|| {
        ...
        let parsed_roundtrip: JsonValue =
            loads(&serde_json::to_string(&(7 as i64)).unwrap_or_default())?;
    ...
    actual.push(
        (serde_json::to_string(&"hello".to_string()).unwrap_or_default()).as_str()
            == ("\"hello\"".to_string()).as_str(),
    );
    actual.push(
        (serde_json::to_string(&false).unwrap_or_default()).as_str()
