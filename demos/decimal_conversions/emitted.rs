...
fn main() {
    ...
    if let Err(__sifr_try_err) = __sifr_try_res {
        let e = __sifr_try_err.clone();
        println!("{}", format!("{}{}", "unexpected decimal conversion failure: ".to_string(), e.message));
    }
    println!("{}", serde_json::to_string(&Decimal::from_str_exact(("1.2300".to_string()).as_str()).unwrap_or_else(|__e| unreachable!())).unwrap_or_default());
    println!("{}", serde_json::to_string(&("1.2300".to_string()).parse::<BigDecimal>().unwrap_or_else(|__e| unreachable!())).unwrap_or_default());
    let __sifr_try_res: Result<(), DecimalConversionError> = (|| {
    ...
    if let Err(__sifr_try_err) = __sifr_try_res {
        let e = __sifr_try_err.clone();
        println!("{}", format!("{}{}", "caught conversion error: ".to_string(), e.message));
    }
}
